...
!$OMP CRITICAL (THREAD_UTILS_LOCKSTATE)
...
!$OMP END CRITICAL (THREAD_UTILS_LOCKSTATE)
...
SUBROUTINE startOMPparallelfor(sharepointer, routine, istart, iend, incr)      &
  BIND(c,NAME="startOMPparallelfor")
...
!$OMP SHARED(sharepointer, f_routine, istart, iend, incr)                      &
!$OMP PRIVATE(t_istart, t_iend, blocksize)

IF (incr>0) THEN
  blocksize = (iend-istart+numThreads())/numThreads()
  t_istart = istart + threadID()*blocksize
  t_iend = MIN(iend,t_istart+blocksize-1)
ELSE
  blocksize = (istart-iend+numThreads())/numThreads()
  t_istart = istart - threadID()*blocksize
  t_iend = MAX(iend,t_istart-blocksize+1)
END IF

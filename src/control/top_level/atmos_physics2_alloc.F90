...
REAL, ALLOCATABLE :: alpha1_sea(:,:),alpha1_sice(:,:,:),ashtf_sea(:,:), &
 ashtf(:,:,:),bq_gb(:,:,:),bt_gb(:,:,:),dtrdz_charney_grid(:,:,:),      &
 rdz_charney_grid(:,:,:),dtrdz_u(:,:,:),dtrdz_v(:,:,:),rdz_u(:,:,:),    &
 rdz_v(:,:,:),ustargbm(:,:)
...
INTEGER, ALLOCATABLE :: kent(:,:),kent_dsc(:,:),k_blend_tq(:,:),k_blend_uv(:,:)
! OUT additional variables for MOSES II
! ftl_tile, le_tile, radnet_sice, radnet_tile, fqw_tile, epot_tile
! fqw_ice, ftl_ice
...
! dtstar_tile
! dtstar
REAL, ALLOCATABLE :: hcons(:),emis_tile(:,:),emis_soil(:)
! bottom block
...
! Arrays for neutral wind diagnostics. These will be allocated under
! STASH control in the main routine.
REAL, ALLOCATABLE, TARGET ::                                            &
  cdr10m_n(:,:), cd10m_n(:,:)
! Interpolated equivalents on u and v grids.
REAL, ALLOCATABLE ::                                                    &
  cdr10m_n_u(:,:), cd10m_n_u(:,:), cdr10m_n_v(:,:), cd10m_n_v(:,:)
!
...
ALLOCATE(ashtf_sea(pdims%i_start:pdims%i_end,                           &
                   pdims%j_start:pdims%j_end))
...
! OUT diagnostics required for soil moisture nudging scheme :
ALLOCATE(wt_ext(land_points,sm_levels))  ! cumulative fract of transp'n
...
ALLOCATE(wt_ext_tile(land_points,sm_levels,ntiles))
...
IF ( .NOT. model_type == mt_single_column ) THEN
  ...
  END IF
  ! Note: at present taux_p, tauy_p are only required for diagnostic
  ! purposes, so it would make more sense to only allocate them on
  ...
  ! However, the main purpose of taux_p, tauy_p is for use by new
  ! convection schemes (not yet on the trunk) to be called from
  ...
  ! allocate them once from this routine.  Branches which implement
  ! the convection schemes will add their own code here to set
  ! l_calc_tau_at_p accordingly.
END IF
...
! If bl_w_var is needed by the convection scheme, but not by the microphysics,
! allocate local space for it here (since it doesn't need to be stored
! prognostically as it does for microphsyics)
IF ( l_wvar_for_conv .AND. ( .NOT. l_subgrid_qcl_mp ) ) THEN
  bl_w_var => NULL()
  ALLOCATE( bl_w_var ( tdims%i_start:tdims%i_end,                       &
...
IF (l_quick_ap2) THEN
  ...
  ALLOCATE(bl_ctl_2d(pdims%i_start:pdims%i_end,                        &
                     pdims%j_start:pdims%j_end,4))        ! 2d reals
  ...
  ALLOCATE(tile_save(land_points,ntiles,4))        ! land reals on tiles
  ...
  IF (l_use_dtstar_sea) THEN
    ALLOCATE(sea_save(pdims%i_start:pdims%i_end,                        &
                      pdims%j_start:pdims%j_end, 2))          ! open sea fields
  END IF

...
INTEGER, POINTER, SAVE :: send_map(:,:) => NULL()
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SCATTER_FIELD_GCOM'
...
IF ((global_row_len  /=  old_global_row_len) .OR.                 &
  ...
  IF (grid_type  ==  fld_type_unknown) THEN
    WRITE(umMessage,*) 'SCATTER_FIELD_GCOM : Bad field type'
    CALL umPrint(umMessage,src='scatter_field_gcom')

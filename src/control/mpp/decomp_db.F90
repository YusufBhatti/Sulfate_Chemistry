...
USE field_types
USE decomp_params, ONLY: max_decomps, decomp_smexe
USE yomhook, ONLY: lhook, dr_hook
USE parkind1, ONLY: jprb, jpim
USE UM_ParCore, ONLY: mype, nproc_max
USE UM_ParParams, ONLY: &
    nodomain, pnorth, peast, psouth, pwest, pnortheast,    &
    pnorthwest, psoutheast, psouthwest, ndim_max,          &
    nhalo_max, halo_type_single, halo_type_extended,       &
    halo_type_no_halo, bc_static, bc_cyclic, bc_overpole
...
USE model_domain_mod, ONLY:                                                    &
    mt_global, mt_lam, mt_cyclic_lam, mt_bi_cyclic_lam, mt_smexe
...
INTEGER(KIND=jpim), PARAMETER, PRIVATE :: zhook_out = 1
  ...
  CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='DECOMP_DB'
...
! Method:
! The local data sizes are calculated and stored.
! The boundary conditions are set (cyclic in East/West direction for global).
...
CHARACTER(LEN=*), PARAMETER   :: RoutineName='DECOMPOSE_FULL'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SET_NEIGHBOUR'
...
! Parallel UM: Sets up array of tids of adjacent processes with corners.

! Subroutine interface:

SUBROUTINE set_full_neighbour(decomp_type)

IMPLICIT NONE

! Description:

! This routine adds the diagonal neighbouring processes to the four
! neighbours tid in order to calculate the full neighbourhood. It
! takes account of the boundary condition. This needs to be called
! after set_neighbour
! 
! In order to find the tid of the corner, the implementation only
! needs to check the bound type of the West/East direction; since it
! uses the grid positions of the North and South neighbour.
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SET_FULL_NEIGHBOUR'

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)

! Set the North/East/South/West neighbour
DecompDB(decomp_type)%full_neighbour(pnorth) = &
  DecompDB(decomp_type)%neighbour(pnorth)
DecompDB(decomp_type)%full_neighbour(peast)  = &
  DecompDB(decomp_type)%neighbour(peast)
DecompDB(decomp_type)%full_neighbour(psouth) = &
  DecompDB(decomp_type)%neighbour(psouth)
DecompDB(decomp_type)%full_neighbour(pwest)  = &
    DecompDB(decomp_type)%neighbour(pwest)

! Set North/East and North/West neighbour
tmp_pe =   DecompDB(decomp_type)%full_neighbour(pnorth)

IF (tmp_pe == nodomain) THEN
  DecompDB(decomp_type)%full_neighbour(pnortheast) = nodomain
  DecompDB(decomp_type)%full_neighbour(pnorthwest) = nodomain
ELSE

  ! Set North/Western neighbour

  IF ( DecompDB(decomp_type)%g_gridpos(1,tmp_pe)  >   0) THEN

    ! This processor is not at the left of the LPG
    DecompDB(decomp_type)%full_neighbour(pnorthwest) = tmp_pe - 1

  ELSE IF (DecompDB(decomp_type)%bound(1)  ==  bc_cyclic) THEN

    ! This processor at the left of the LPG, and has cyclic BCs.
    DecompDB(decomp_type)%full_neighbour(pnorthwest) =           &
      tmp_pe + DecompDB(decomp_type)%gridsize(1) - 1

  ELSE
   
    ! This processor at the left of LPG and has static BCs
    DecompDB(decomp_type)%full_neighbour(pnorthwest) =nodomain

  END IF

  ! Set North / Eastern neighbour

  IF ( DecompDB(decomp_type)%g_gridpos(1,tmp_pe)  <               &
    (DecompDB(decomp_type)%gridsize(1)-1) ) THEN

    ! This processor is not at the right of the LPG
    DecompDB(decomp_type)%full_neighbour(pnortheast) = tmp_pe + 1

  ELSE IF (DecompDB(decomp_type)%bound(1)  ==  bc_cyclic) THEN

    ! This processor at the right of the LPG, and has cyclic BCs.
    DecompDB(decomp_type)%full_neighbour(pnortheast) =            &
      tmp_pe - DecompDB(decomp_type)%gridsize(1) + 1

  ELSE
   
    ! This processor at the right of LPG and has static BCs
    DecompDB(decomp_type)%full_neighbour(pnortheast) =nodomain
...
! Set South/East and South/West neighbour
tmp_pe =   DecompDB(decomp_type)%full_neighbour(psouth)

IF (tmp_pe == nodomain) THEN
  DecompDB(decomp_type)%full_neighbour(psoutheast) = nodomain
  DecompDB(decomp_type)%full_neighbour(psouthwest) = nodomain
ELSE

  ! Set South/Western neighbour

  IF ( DecompDB(decomp_type)%g_gridpos(1,tmp_pe)  >   0) THEN

    ! This processor is not at the left of the LPG
    DecompDB(decomp_type)%full_neighbour(psouthwest) = tmp_pe - 1

  ELSE IF (DecompDB(decomp_type)%bound(1)  ==  bc_cyclic) THEN

    ! This processor at the left of the LPG, and has cyclic BCs.
    DecompDB(decomp_type)%full_neighbour(psouthwest) =           &
      tmp_pe + DecompDB(decomp_type)%gridsize(1) - 1
  ...
  ! Set South/Eastern neighbour

  IF ( DecompDB(decomp_type)%g_gridpos(1,tmp_pe)  <               &
    (DecompDB(decomp_type)%gridsize(1)-1) ) THEN

    ! This processor is not at the right of the LPG
    DecompDB(decomp_type)%full_neighbour(psoutheast) = tmp_pe + 1

  ELSE IF (DecompDB(decomp_type)%bound(1)  ==  bc_cyclic) THEN

    ! This processor at the right of the LPG, and has cyclic BCs.
    DecompDB(decomp_type)%full_neighbour(psoutheast) =            &
      tmp_pe - DecompDB(decomp_type)%gridsize(1) + 1
...
END SUBROUTINE set_full_neighbour

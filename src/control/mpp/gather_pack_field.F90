...
SUBROUTINE gather_pack_field(                                   &
                        ...
                        packing, StashItem, packing_type,       &
                        num_out, comp_accrcy, rmdi)
...
INTEGER, INTENT(IN) ::                                            &
  StashItem          ! IN: STASHitem=1000*section+item
...
IF ((global_row_len  /=  old_global_row_len) .OR.                 &
  ...
  IF (grid_type  ==  fld_type_unknown) THEN
    WRITE(umMessage,*) 'GATHER_PACK_FIELD : Bad field type'
    CALL umPrint(umMessage,src='gather_pack_field')
...
local_packing_type = PC_No_Packing
...
IF (local_packing_type /= PC_WGDOS_Packing .AND.                               &
...
! Now we must check if the packing_type is PC_WGDOS_Packing
IF (local_packing_type == PC_WGDOS_Packing) THEN
  ...
  IF (mype == gather_pe) THEN
      ...
      CALL wgdos_compress_field(                                  &
         my_global_buffer(                                        &
                   ...
                   (g_datastart_f(2,grid_type,row_start_pe)-1)*   &
                          ...
                          global_row_len, num_out,                &
                          comp_accrcy, rmdi,                      &
                          StashItem, icode)

      global_field = TRANSFER(icomp, global_field)
    ELSE      ! Still doing 2 stage gather
      CALL wgdos_compress_field(                                  &
    ...
    IF (icode /= 0) THEN
      cmessage='Error in wgdos_compress_field'
      CALL ereport(routinename, icode, cmessage)
    END IF
...
END IF !packing_type is PC_WGDOS_Packing
...
! Deallocate the temporary buffer for WGDOS processed data
IF (ALLOCATED(icomp)) DEALLOCATE(icomp)

! Do synchronisation if required to stop message congestion
IF (gather_sync) THEN

...
CHARACTER(LEN=*), PARAMETER :: RoutineName='GATHER_FIELD_GCOM'
...
IF ((global_row_len  /=  old_global_row_len) .OR.                 &
  ...
  IF (grid_type  ==  fld_type_unknown) THEN
    WRITE(umMessage,*) 'GATHER_FIELD_GCOM : Bad field type'
    CALL umPrint(umMessage,src='gather_field_gcom')
    WRITE(umMessage,*) 'Field will not be gathered.'
    CALL umPrint(umMessage,src='gather_field_gcom')

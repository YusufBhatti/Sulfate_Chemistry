...
! *****************************COPYRIGHT*******************************
!  Gathers a 32-bit field from many processors to one processor

! Subroutine Interface:
SUBROUTINE gather_field_mpl32(local_field,    global_field,             &
...
REAL(KIND=real32), INTENT(IN)     :: local_field(local_row_len*local_rows)
                                       ! local part of field
...
CHARACTER (LEN=*), PARAMETER :: RoutineName='GATHER_FIELD_MPL32'
...
global_field(1:global_rows*global_row_len) =         &
             local_field(1:global_rows*global_row_len)

#else

IF (grid_type  ==  fld_type_unknown) THEN
  WRITE(umMessage,'(A)') 'GATHER_FIELD_MPL32 : bad field type'
  CALL umPrint(umMessage,src='gather_field_mpl32')
  WRITE(umMessage,'(A)') 'field will not be gathered.'
  CALL umPrint(umMessage,src='gather_field_mpl32')
  WRITE (cmessage,'(A)') 'GATHER_FIELD_MPL32 : bad field type'
  icode = 1
...
IF (proc_group /= gc_all_proc_group) THEN
  WRITE (cmessage, '(A)') 'Can only gather in an all processor group'
  icode = 2
...
IF ((global_row_len  /=  old_global_row_len) .OR.                 &
  ...
  CALL mpl_type_vector(blsize(2, grid_type),  blsize(1, grid_type),   &
                       local_row_len,  mpl_real4, send_type, info)
...
CALL mpl_gatherv(local_field(ipos), 1, send_type,                    &
                 global_field_copy, counts, displacements, mpl_real4,&
...
IF (mype == gather_pe) THEN
  ...
  IF (ipos /= global_rows * global_row_len + 1) THEN
    WRITE(cmessage, '(A)') 'Addressing gone wrong in field copy'
    icode = 3
...
RETURN
END SUBROUTINE gather_field_mpl32

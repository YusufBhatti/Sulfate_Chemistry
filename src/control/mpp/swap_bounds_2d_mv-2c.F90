...
! at once
MODULE swap_bounds_2d_mv_mod
IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='SWAP_BOUNDS_2D_MV_MOD'
...
INTEGER :: full_rows             ! rows including halos
...
DO i_field=1, n_multi
  ...
  IF (input_fields(i_field) % levels /= 1) THEN
    cmessage='2D Multi-variate halo exchanges cant have >1 levels'
    errorstatus = 20
...
IF (halo_y > 0) THEN
  ...
  ALLOCATE( send_buffer_n(row_length, halo_y, n_multi) )
  ALLOCATE( send_buffer_s(row_length, halo_y, n_multi) )
  ALLOCATE( recv_buffer_n(row_length, halo_y, n_multi) )
  ALLOCATE( recv_buffer_s(row_length, halo_y, n_multi) )
      ...
      DO i_field=1, n_multi
        ...
        DO j=1,halo_y
          DO i=1 ,row_length
            ...
            ! Fill southern halo
            field(i+halo_x,j)=field(i+halo_x,south_halo_source+j-1)

            ! Fill northern halo
            field(i+halo_x,rows+halo_y+j)=field(i+halo_x,north_halo_source+j-1)
    ...
    ELSE ! If there is more than 1 processor north_south
      ...
      DO i_field=1, n_multi
        ...
        DO j=1,halo_y
          DO i=1, row_length
            ...
            send_buffer_s(i,j,i_field) = field(i+halo_x,halo_y+j)

            ! Copy stuff from the northern side of the grid
            send_buffer_n(i,j,i_field) = field(i+halo_x,rows+j)
      ...
      IF (neighbour(pnorth) /= nodomain) THEN
        ...
        DO i_field=1, n_multi
          ...
          DO j=1,halo_y
            DO i=1,row_length
              field(i+halo_x,j+halo_y+rows) = recv_buffer_n(i,j,i_field)
      ...
      IF (neighbour(psouth) /= nodomain) THEN
        ...
        DO i_field=1, n_multi
          ...
          DO j=1,halo_y
            DO i=1,row_length
              field(i+halo_x,j) = recv_buffer_s(i,j,i_field)
            END DO
          END DO
        END DO ! loop over fields
  ...
  ELSE                 !!! bc_cyclic in NS
      ...
      DO i_field=1, n_multi
        ...
        DO j=1,halo_y
          DO i=1,row_length
            ...
            send_buffer_n(i,j,i_field) =                                    &
              field(i+halo_x,rows-north_off(i_field)+j)
    ...
    IF (at_extremity(psouth)) THEN
      ...
      ELSE IF (neighbour(psouth) /= nodomain) THEN
        ...
        DO i_field=1, n_multi
          ...
          IF (change_sign(i_field)) THEN
            DO j=1,halo_y
              DO i=1,row_length
                field(i+halo_x,1-j+halo_y) = -recv_buffer_s(i,j,i_field)
              END DO
            END DO
          ELSE ! don't change sign
            DO j=1,halo_y
      ...
      ELSE IF (neighbour(pnorth) /= nodomain) THEN ! not LAM
        ...
        DO i_field=1, n_multi
          ...
          IF (change_sign(i_field)) THEN
            DO j=1,halo_y
              DO i=1,row_length
                field(i+halo_x,rows+halo_y+j)=                            &
                   -recv_buffer_n(i,halo_y-j+1,i_field)
              END DO
            END DO
          ELSE ! don't change sign
            DO j=1,halo_y
...
IF (halo_x > 0) THEN
  ALLOCATE( send_buffer_e(halo_x, full_rows, n_multi) )
  ALLOCATE( send_buffer_w(halo_x, full_rows, n_multi) )
  ALLOCATE( recv_buffer_e(halo_x, full_rows, n_multi) )
  ALLOCATE( recv_buffer_w(halo_x, full_rows, n_multi) )
  ...
  ELSE ! If there is more than 1 processor East-West
    ...
    DO i_field=1, n_multi
      ...
      DO j=1,rows+2*halo_y
        DO i=1,halo_x
          ...
          send_buffer_w(i,j,i_field) = field(halo_x+i,j)

          ! Copy stuff from the Eastern side of the grid
          send_buffer_e(i,j,i_field) = field(row_length+i,j)
    ...
    IF (neighbour(peast) /= nodomain) THEN
      ...
      DO i_field=1, n_multi
        ...
        DO j=1,rows + 2*halo_y
          DO i=1,halo_x
            field(row_length+halo_x+i,j) = recv_buffer_e(i,j,i_field)
          END DO
        END DO
      END DO ! loop over fields
    ...
    IF (neighbour(pwest) /= nodomain) THEN
      ...
      DO i_field=1, n_multi
        ...
        DO j=1,rows + 2*halo_y
          DO i=1,halo_x
            field(i,j) = recv_buffer_w(i,j,i_field)
          END DO
        END DO
      END DO ! loop over fields
...
END MODULE swap_bounds_2d_mv_mod

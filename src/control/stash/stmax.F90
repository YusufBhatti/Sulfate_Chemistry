...
!    Routine: STMAX ----------------------------------------------------
!
!    Purpose: Computes the point-by-point maximum in time of a field
!             by comparing the field at the current time with the
!             maximum so far (STASH TEMPORAL service routine)
!
...
SUBROUTINE stmax(fieldin,RESULT,fsize,masking)
!
...
REAL :: RESULT(fsize)       ! OUT output field (maximum)
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='STMAX'
!-----------------------------------------------------------------------
IF (lhook) CALL dr_hook(RoutineName,zhook_in,zhook_handle)
!
!  1.1 loop over array size, if either result or fieldin is MDI set
!      result to MDI, else set result to maximum of fieldin and result
!
...
ELSE
  !
  !  1.3 loop over array size, set result to maximum of fieldin and result

...
USE dump_headers_mod, ONLY: fh_CoordSystem_RegLonLat
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='READHEAD'
! -------------------------------------------------------------
...
! DEPENDS ON: read_flh
CALL read_flh (nftin,fixhd,len_fixhd, icode,cmessage)
IF (icode/=0) THEN
...
IF (fixhd(110) >  0 .AND. len1_levdepc /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' LEVEL DEPENDENT CONSTANTS'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(111)*fixhd(112)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(115) >  0 .AND. len1_rowdepc /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      ...
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(116)*fixhd(117)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(120) >  0 .AND. len1_coldepc /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      ...
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(121)*fixhd(122)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(125) >  0 .AND. len1_flddepc /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' FIELD DEPENDENT CONSTANTS'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(126)*fixhd(127)
      CALL umPrint(umMessage,src='readhead')
...
IF (fixhd(130) >  0 .AND. len_extcnst /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      ...
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(131)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(135) >  0 .AND. len_dumphist /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' TEMPORARY HISTORY BLOCK'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(136)
      CALL umPrint(umMessage,src='readhead')
...
IF (fixhd(140) >  0 .AND. len_cfi2 /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' COMPRESSED FIELD INDEX NO 1'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(141)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(142) >  0 .AND. len_cfi2 /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' COMPRESSED FIELD INDEX NO 2'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(143)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(144) >  0 .AND. len_cfi3 /= 0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' COMPRESSED FIELD INDEX NO 3'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(145)
      CALL umPrint(umMessage,src='readhead')
    END IF ! mype  ==  0
  END IF
END IF
...
IF (fixhd(150) >  0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      WRITE(umMessage,'('' '')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' LOOKUP TABLE'')')
      CALL umPrint(umMessage,src='readhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')')fixhd(151)*fixhd(152)
      CALL umPrint(umMessage,src='readhead')

      IF (fixhd(152) <  fixhd_152) THEN
        WRITE(umMessage,'('' '')')
        CALL umPrint(umMessage,src='readhead')
        WRITE(umMessage,'('' '',i6,'' Entries in Look Up Table.'')') fixhd_152
        CALL umPrint(umMessage,src='readhead')
        WRITE(umMessage,'('' '',i6,'' Entries read in.'')') fixhd(152)
        CALL umPrint(umMessage,src='readhead')
      END IF
  ...
  DO k=1,len2_lookup
    IF (lookup(45,k) == 0 .OR. lookup(45,k) == imdi) THEN
      ...
      IF (printstatus >= prstatus_normal) THEN
        WRITE(umMessage,*) 'WARNING: User defined field found - ',      &
                   'STASH code : ', lookup(42,k)
        CALL umPrint(umMessage,src='readhead')
        WRITE(umMessage,*) ' Setting internal model number to atmosphere.'
        CALL umPrint(umMessage,src='readhead')
      END IF
      lookup(45,k)=1

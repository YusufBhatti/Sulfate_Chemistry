...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='WRITHEAD_MOD'
...
REAL(KIND=real32), ALLOCATABLE, TARGET :: tmp_rdata32(:)
REAL, ALLOCATABLE                    :: tmp_rdata_default(:)
INTEGER(KIND=integer32), TARGET      :: lookup_o32(len1_lookup,len2_lookup)
...
IF (fixhd(110) >  0) THEN
  ...
  IF (printstatus >= prstatus_oper) THEN
    IF (mype  ==  0) THEN
      CALL umPrint('',src='writhead')
      CALL umPrint(' LEVEL DEPENDENT CONSTANTS',src='writhead')
      WRITE(umMessage,'('' '',i8,'' 64-bit words long'')') &
          fixhd(111)*fixhd(112)
      CALL umPrint(umMessage,src='writhead')
    END IF ! if mype  ==  0
  END IF
...
IF (fixhd(150) >  0) THEN
  ...
  IF (output_bits == 32) THEN
    ...
    DO i=1,fixhd(152)
      ...
      CALL um_memcpy32(C_LOC(lookup_o32(46,i)), C_LOC(tmp_rdata32(46)),    &
                       INT(19,KIND=C_INT64_T))

...
CHARACTER(LEN=*), PARAMETER :: RoutineName='PR_IFLD'
!--------------------------------------------------------------
...
IF (lookup(lbcode,k) == imdi) THEN
  ...
  CALL umPrint(umMessage,src='pr_ifld')
...
IF (n_cols /= 0 .AND. n_cols /= imdi) THEN
  ...
  DO l=1,ns_pts-1
    WRITE(umMessage,'(1x,i3,'':'',f8.3,'':'',3X,9(I9,3X))')row,lat,       &
...
ELSE
  ...
  WRITE(umMessage,'(14X,9A12)')(dash(j),j=1,ew_print)
  CALL umPrint(umMessage,src='pr_ifld')
  WRITE(umMessage,'(1x,3x,'':'',8x,'':'',3X,9(I9,3X))')                   &
        (id1(j),j=1,ew_print)
  CALL umPrint(umMessage,src='pr_ifld')
  WRITE(umMessage,'(14X,9A12)')(dash(j),j=1,ew_print)
  CALL umPrint(umMessage,src='pr_ifld')
...
WRITE(umMessage,'(A,I8,A,I8,A,I8,A,I8)')                                &

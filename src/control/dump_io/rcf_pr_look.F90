...
CHARACTER(LEN=*), PARAMETER :: RoutineName='RCF_PR_LOOK'
...
stm_record => rcf_exppx(model,section,item,NoFindArg=.TRUE.)
IF ( ASSOCIATED ( stm_record ) ) THEN
  phrase = stm_record % NAME
ELSE
...
CALL umPrint(umMessage,src='rcf_pr_look')
...
CALL umPrint( &
    ...
    src='rcf_pr_look')
WRITE(umMessage,'(1p," ",5e13.4)')(rlookup(i,k),i=46,50)
CALL umPrint(umMessage,src='rcf_pr_look')

CALL umPrint( &
    '          BACC         BLEV        BRLEV        BHLEV       BHRLEV',&
    src='rcf_pr_look')
WRITE(umMessage,'(1p," ",5e13.4)')(rlookup(i,k),i=51,55)
CALL umPrint(umMessage,src='rcf_pr_look')

CALL umPrint( &
    '         BPLAT        BPLON         BGOR          BZY          BDY', &
    src='rcf_pr_look')
WRITE(umMessage,'(1p," ",5e13.4)')(rlookup(i,k),i=56,60)
CALL umPrint(umMessage,src='rcf_pr_look')

CALL umPrint( &
    '           BZX          BDX         BMDI         BMKS',&
    src='rcf_pr_look')
WRITE(umMessage,'(1p," ",4e13.4)')(rlookup(i,k),i=61,64)
CALL umPrint(umMessage,src='rcf_pr_look')
CALL umPrint('',src='rcf_pr_look')

IF (lhook) CALL dr_hook(RoutineName,zhook_out,zhook_handle)
RETURN
END SUBROUTINE rcf_pr_look

...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SET_DUMPFILE_ADDRESS'
...
IF ((fixhd(160)-1)  /=                                            &
  ...
  IF (mype == 0) THEN
    IF (printstatus >= prstatus_diag) THEN
      CALL umPrint('',src='set_dumpfile_address')
      WRITE(umMessage,'(A,i10,A,i10)')                                     &
          'SET_DUMPFILE_ADDRESS: Start of Data Address on disk reset from '&
          ,old_fixhd_160-1,' to ', fixhd(160)-1
      CALL umPrint(umMessage,src='set_dumpfile_address')
    END IF
  END IF
END IF
...
DO i=1, len2_lookup
  ...
  IF (lookup(lbpack,i)-                                         &
    ((lookup(lbpack,i)/10)*10) == PC_Cray32_Packing) THEN
...
IF (mype == 0) THEN
  IF (printstatus >= prstatus_diag) THEN
    ...
    CALL umPrint(&
        ...
        src='set_dumpfile_address')

    WRITE(umMessage,'(I10,A)') &
        fixhd(161),' Words Stored as Data Length in FIXHD(161)'
    CALL umPrint(umMessage,src='set_dumpfile_address')

    WRITE(umMessage,'(I10,A)') &
        number_of_data_words_in_memory,' Words Used in Memory for Data'
    CALL umPrint(umMessage,src='set_dumpfile_address')

    WRITE(umMessage,'(I10,A)') &
        number_of_data_words_on_disk,' Words Used on Disk for Data'
    CALL umPrint(umMessage,src='set_dumpfile_address')

    WRITE(umMessage,'(I10,A)') &
        disk_address-fixhd(160),' Words Used on Disk for Data after Rounding'
    CALL umPrint(umMessage,src='set_dumpfile_address')

    WRITE(umMessage,'(I10,A,I11,A)') &
        disk_address,' Words Used on Disk in Total for the File (', &
        disk_address*i,' Bytes)'
    CALL umPrint(umMessage,src='set_dumpfile_address')
  END IF
END IF
IF (lhook) CALL dr_hook(RoutineName,zhook_out,zhook_handle)

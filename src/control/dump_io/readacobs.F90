...
CHARACTER(LEN=*), PARAMETER :: RoutineName='READACOBS'
...
IF (mype  ==  0) THEN
  CALL umPrint('',src='readacobs')
  WRITE(umMessage,'('' READING ACOBS FILE ON UNIT'',I3)')nftin
  CALL umPrint(umMessage,src='readacobs')
  CALL umPrint(' #####################################',src='readacobs')
  CALL umPrint('',src='readacobs')
...
IF (fixhd(160) >  0) THEN
  ...
  DO k=1,fixhd(152)
    ...
    IF (lookup(lblrec,k) >  0) THEN   !  Any data for this field?
      ...
      IF ((lookup(lbnrec,k) == 0) .OR.                            &
        ...
        IF (k >  1) THEN
          DO l=2,k
            IF (MOD(lookup(lbpack,l-1),10) == PC_Cray32_Packing) THEN
      ...
      .OR. (lookup(data_type,k) <= -1 .AND. lookup(data_type,k) >= -3))&
        ...
        IF ((a /= -1.0) .OR. (len_io /= ipts_to_read)) THEN
          WRITE(umMessage,*)'ERROR READING DUMP ON UNIT ',nftin
          CALL umPrint(umMessage,src='readacobs')
          icode=2
  ...
  IF (mype  ==  0) THEN
    ...
    IF (fixhd(5) >= 6 .AND. fixhd(5) <= 8) THEN ! AC/Var Obs/ Cx file
      WRITE(umMessage,'('' OBSERVATION DATA'')')
      CALL umPrint(umMessage,src='readacobs')
    ELSE
      WRITE(umMessage,'('' MODEL DATA'')')
      CALL umPrint(umMessage,src='readacobs')
    END IF
    WRITE(umMessage,'('' '',i8,'' words long'')')fixhd(161)
    CALL umPrint(umMessage,src='readacobs')
  END IF ! mype  ==  0
...
IF (mype  ==  0) THEN
  CALL umPrint('',src='readacobs')
  WRITE(umMessage,'(A,i9,A,I3)')' INITIAL DATA SUCCESSFULLY READ -',      &
  start_block,' WORDS FROM UNIT',nftin
  CALL umPrint(umMessage,src='readacobs')
  IF (real_start_block /= start_block) THEN
    CALL umPrint('',src='readacobs')
    WRITE(umMessage,'(A,I9)')' Number of Words Read from Disk was ',      &
          real_start_block
    CALL umPrint(umMessage,src='readacobs')
  END IF
END IF ! mype  ==  0

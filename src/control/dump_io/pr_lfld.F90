...
!    SUBROUTINE PR_LFLD----------------------------------------
!
!    Purpose: Prints out selected values from logical data
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='PR_LFLD'
!--------------------------------------------------------------
...
IF (lookup(lbcode,k) == imdi) THEN
  ...
  WRITE(umMessage,*) 'MESSAGE FROM PR_LFLD'
  CALL umPrint(umMessage,src='pr_lfld')
...
IF (n_cols /= 0 .AND. n_cols /= imdi) THEN
  ...
  DO l=1,ns_pts-1
    WRITE(umMessage,'(1x,i3,'':'',f8.3,'':'',3X,9(L9,3X))')row,lat,       &
...
ELSE
  ...
  WRITE(umMessage,'(14X,9A12)')(dash(j),j=1,ew_print)
  CALL umPrint(umMessage,src='pr_lfld')
  WRITE(umMessage,'(1x,3x,'':'',8x,'':'',3X,9(L9,3X))')                   &
        (ld1(j),j=1,ew_print)
  CALL umPrint(umMessage,src='pr_lfld')
  WRITE(umMessage,'(14X,9A12)')(dash(j),j=1,ew_print)
  CALL umPrint(umMessage,src='pr_lfld')

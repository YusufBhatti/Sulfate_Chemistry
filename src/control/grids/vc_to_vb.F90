...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='VC_TO_VB_MOD'

CONTAINS

SUBROUTINE vc_to_vb                                                            &
  ( vc, rows, row_length, n_rows, levels, halo_x, halo_y, global_row_length    &
  , vb, ub_arg )
...
!      field (without halos).
!   4. In the endgame case, compute v at the poles using routine
!      eg_uv_at_poles_Bgrid
...
REAL, INTENT(INOUT) ::                                                         &
                    ub_arg( udims%i_start:udims%i_end                          &
                          , vdims%j_start:vdims%j_end                          &
...
! 2. Populate halos using swapbounds. To west for EG, east for ND.
CALL swap_bounds                                                               &
  ( vc_halo, row_length, n_rows,levels, halo_x, halo_y,                        &
    fld_type_v, swap_field_is_vector                                           &
  , do_east_arg=.FALSE., do_west_arg=.TRUE. )

! 3. Make a simple average of adjacent columns and copy to output
!    field (without halos).
!    On polar rows set every point to the absolute average for the row.
!
!    Here we use udims to define row_length of B grid and vdims to define
!    number of rows.
  ...
  IF (at_extremity(psouth)) THEN
    DO k=1, levels
      ...
      CALL eg_uv_at_poles_Bgrid                                              &
        ( ub, row_length, rows, n_rows, halo_x, halo_y, pole_consts, -1.0    &

...
MODULE p_to_u_sea_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'P_TO_U_SEA_MOD'

CONTAINS

SUBROUTINE p_to_u_sea                                                          &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName = 'P_TO_U_SEA'
  ...
  DO j=outj_start, outj_end
    ...
    DO i=outi_start, outi_end
      ...
      IF (fsea_total >  0.0) THEN
        array_on_u_points(i,j,k) = 1.0/fsea_total*                           &
          ( (1.0-fland_on_p_points(i,j))   * array_on_p_points(i,j,k)        &
          + (1.0-fland_on_p_points(i+1,j)) * array_on_p_points(i+1,j,k) )
      ELSE

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='COAST_AJ_MOD'
...
, index_targ_sea_unres, sea_points_unres, index_targ_land_unres                &
, land_points_unres )
...
INTEGER, INTENT(IN) :: points_lambda_srce  ! Number of lambda points
                                           ! on source grid
INTEGER, INTENT(IN) :: points_phi_srce     ! Number of phi points
                                           ! on source grid
INTEGER, INTENT(IN) :: points              ! Total number of points
                                           ! on target grid
INTEGER, INTENT(IN) :: index_b_l(points)   ! Index of bottom lefthand corner
                                           ! of source gridbox
INTEGER, INTENT(IN) :: index_b_r(points)   ! Index of bottom righthand corner
                                           ! of source gridbox
INTEGER, INTENT(IN) :: index_t_l(points)   ! Index of top lefthand corner
                                           ! of source gridbox
INTEGER, INTENT(IN) :: index_t_r(points)   ! Index of top righthand corner
                                           ! of source gridbox

INTEGER, INTENT(IN) :: land_sea_srce(points_lambda_srce*points_phi_srce)
                                           ! Land/sea mask on source grid

INTEGER, INTENT(INOUT) :: land_sea_targ(points)
                             ! Land/sea mask on target grid. If MASK
                             ! then precalculated land/sea mask on input.

INTEGER, INTENT(OUT) :: coastal_points     ! Number of coastal points
                                           ! on target grid
INTEGER, INTENT(OUT) :: land_points_unres  ! Number of unresolved land pts
                                           ! when MASK=T
INTEGER, INTENT(OUT) :: sea_points_unres   ! Number of unresolved sea pts
                                           ! when MASK=T

INTEGER, INTENT(OUT) :: index_targ(points) ! Index of target coastal points
INTEGER, INTENT(OUT) :: index_srce(points) ! Index of source points mapped
                                           ! onto target coastal points

INTEGER, INTENT(OUT) :: index_targ_sea_unres(points)
                             ! Index of sea pts on target grid which a
                             ! unresolved when MASK=T

INTEGER, INTENT(OUT) :: index_targ_land_unres(points)
                             ! Index of land pts on target grid which
                             ! unresolved when MASK=T


REAL, INTENT(IN) :: weight_t_r(points) ! Weight applied to value at top right
                                       ! hand corner of source gridbox
REAL, INTENT(IN) :: weight_b_l(points) ! Weight applied to value at bot. left
                                       ! hand corner of source gridbox
REAL, INTENT(IN) :: weight_b_r(points) ! Weight applied to value at bot. right
                                       ! hand corner of source gridbox
REAL, INTENT(IN) :: weight_t_l(points) ! Weight applied to value at top left
                                       ! hand corner of source gridbox

LOGICAL, INTENT(IN) :: mask  ! =F, then land/sea mask estimated
                             ! =T, then land/sea mask input as land_sea_targ


! Local arrays
INTEGER :: land_sea_temp(points)  ! Array used to accumulate output land/sea
INTEGER :: index_temp(points,4)   ! Index of 4 sourrounding source points
                                  ! order by distance
INTEGER :: land_sea_coast(points) ! Mask of coastal points on target grid
REAL    :: max_weight(points,4)   ! Linear interpolation weights ordered by ??
...
DO i=1, points
  ...
  land_sea_temp(i) = land_sea_srce (index_b_l(i))                              &
                   ...
                   + land_sea_srce (index_t_l(i))                              &
                   + land_sea_srce (index_t_r(i))
...
IF (mask) THEN
  ...
  DO i=1, points
    logic_test(i) = land_sea_temp(i) /= land_sea_targ(i) .AND.                 &
                    land_sea_coast(i) == 0
  END DO
...
DO i=1, coastal_points
  ...
  index_temp(i,3) = index_t_l(index_targ(i))
  index_temp(i,4) = index_t_r(index_targ(i))
END DO

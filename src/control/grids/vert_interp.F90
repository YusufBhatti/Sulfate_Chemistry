...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'VERT_INTERP_MOD'

CONTAINS

SUBROUTINE vert_interp                                                         &
  ( data_in, data_points, data_levels, desired_r, r_at_data, interp_order      &
  , data_out )
...
INTEGER, INTENT(IN) :: interp_order   ! 1=linear, 3=cubic, 5=quintic
                                      ! 2=linear with no extrapolation
                                      !   at top or bottom of model.
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='VERT_INTERP'
...
DO k=1, data_levels
  DO j=1, data_points
    IF ( (r_at_data(j,k) > desired_r(j)) .AND.                                 &
...
DO j=1, data_points
  ...
  ELSE IF ( level_below(j) == 1 .OR.                                           &
            level_below(j) == data_levels - 1 .OR.                             &
            interp_order   == interp_order_linear .OR.                         &
            interp_order   == interp_order_linear_noex ) THEN
  ...
  ELSE IF ( level_below(j)  ==  2 .OR.                                         &
            level_below(j)  ==  data_levels - 2 .OR.                           &

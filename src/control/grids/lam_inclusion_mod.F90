...
! Description:
!   This checks that the target grid is within the source grid.  Used by 
!   CreateBC and the reconfiguration to make sure interpolations can be 
!   considered safe.
!
...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'LAM_INCLUSION_MOD'

CONTAINS

SUBROUTINE lam_inclusion                                                       &
  ( src_rows , src_row_len , src_halo_x , src_halo_y                           &
...
IF (ipt /= num_pts) THEN
  WRITE(umMessage,'(A,I5,A,I5)') "ipt = ", ipt, ", num_pts = ", num_pts
  CALL umPrint(umMessage,src='lam_inclusion_mod')
...
IF (.NOT. l_same_rotation) THEN
  ...
  CALL rotate_eq_to_latlon                                                     &
    ( phi_box,  lambda_box, phi_box,  lambda_box                               &
  ...
  CALL rotate_latlon_to_eq                                                     &
      ( phi_box, lambda_box, phi_box, lambda_box                               &
      , src_pole_lat, src_pole_lon, num_pts )
...
IF ( src_lambda_min - (targ_lambda_min + grid_tol) > 0.0 ) THEN
  WRITE(cmessage,'(A,F12.7,A,F12.7)')                                          &

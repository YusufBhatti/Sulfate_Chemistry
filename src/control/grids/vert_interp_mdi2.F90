...
! *****************************COPYRIGHT*******************************
! Subroutine vert_interp_mdi2
MODULE vert_interp_mdi2_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'VERT_INTERP_MDI2_MOD'

CONTAINS

SUBROUTINE vert_interp_mdi2                                                    &
  ( data_in, row_length, data_rows, data_levels, desired_r, halo_x1, halo_y1   &
  , halo_x2, halo_y2, r_at_data, interp_order, mdi, data_out )
...
!          This routine is derived from vert_interp_mdi where
!          r can be any quantity that is monotonic increasing with
!          model level, such as height, or potential temperature
!          (assuming that the model is statically stable). In some
!          cases it was being used for non-monotone fields (e.g.
!          PV on theta surfaces) and this routine gives continuity
!          for these cases where the original has been optimised in
!          a way that requires the monotone assumption more explicitly.
...
REAL, INTENT(IN) :: data_in ( 1-halo_x1:row_length+halo_x1                     &
                            , 1-halo_y1:data_rows+halo_y1                      &
                            , data_levels )

REAL, INTENT(IN) :: r_at_data ( 1-halo_x2:row_length+halo_x2                   &
                              , 1-halo_y2:data_rows+halo_y2                    &
                              , data_levels )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName = 'VERT_INTERP_MDI2'
...
DO j=1, data_rows
  DO i=1, row_length
    ...
    ELSE IF ( level_below(i,j)  ==  1 .OR.                                     &
              level_below(i,j)  ==  data_levels - 1 .OR.                       &
              interp_order      ==  interp_order_linear ) THEN
    ...
    ELSE IF ( level_below(i,j)  ==  2 .OR.                                     &
              level_below(i,j)  ==  data_levels - 2 .OR.                       &
              interp_order      ==  interp_order_cubic ) THEN
...
END SUBROUTINE vert_interp_mdi2

END MODULE vert_interp_mdi2_mod

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'T_VERT_INTERP_TO_P_MOD'

CONTAINS

SUBROUTINE t_vert_interp_to_p                                                  &
  ( t, theta, row_length, rows, model_levels, desired_p, off_x, off_y          &
  , halo_i, halo_j, p_theta_levels, boundary_layer_levels, exner_theta_levels  &
  , t_out )
...
!   respectively: isothermal or from the first level above a
!   fixed height (200m)
...
USE planet_constants_mod,  ONLY: g, kappa, p_zero, r, lapse
...
CHARACTER(LEN=*), PARAMETER :: RoutineName = 'T_VERT_INTERP_TO_P'
...
DO j=1, rows
  DO i=1, row_length
    IF ( desired_exner < exner_theta_levels(i,j,model_levels) ) THEN
      level_below(i,j) = -1
    END IF
  END DO
END DO
...
DO j=1, rows
  DO i=1, row_length
    ...
    ELSE IF (level_below(i,j) == 0) THEN
      ! Extrapolate
      t_out(i,j) =                                                             &
        t(i,j,level_extrap(i,j)) *                                             &
        (desired_p / p_theta_levels(i,j,level_extrap(i,j)))**power
    ELSE
      ! Linear interpolation
      t_out(i,j) =                                                             &
        ( ( desired_exner - exner_theta_levels(i,j,level_below(i,j)) )         &
          * t(i,j,level_below(i,j)+1)                                          &
        - ( desired_exner - exner_theta_levels(i,j,level_below(i,j)+1) )       &

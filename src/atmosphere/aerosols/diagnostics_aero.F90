...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='DIAGNOSTICS_AERO_MOD'
...
!$OMP SCHEDULE(STATIC)                                                        &
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,msa,timestep)
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,nh3_dep,timestep)
    DO level=1,model_levels
      DO j=1,rows
        DO i=1,row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dms,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dms,f_dms_to_so2,    &
!$OMP        timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dms,f_dms_to_so4,psi,&
!$OMP        timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dms,f_dms_to_so4,    &
!$OMP        psi,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dry,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_wet,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_wet_o3,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_evap,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_nucl,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_diffuse,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_coag,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dry,timestep,psi)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_dry,psi,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
          work_3d(i,j,level) = deltas_dry(i,j,level)*                          &
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,deltas_merge,timestep)
    DO level=1, model_levels
      DO j=1, rows
        DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,delta_n_chem,timestep)
  DO level=1, model_levels
    DO j=1, rows
      DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,delta_n_nuc,timestep)
  DO level=1, model_levels
    DO j=1, rows
      DO i=1, row_length
...
!$OMP DEFAULT(NONE)                                                           &
!$OMP PRIVATE(level,j,i)                                                      &
!$OMP SHARED(model_levels,rows,row_length,work_3d,delta_n_evap,timestep)
  DO level=1, model_levels
    DO j=1, rows
      DO i=1, row_length

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='SOOTDIFFSCAV_MOD'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SOOTDIFFSCAV'
...
REAL :: p(1-off_x:row_length+off_x,1-off_y:rows+off_y, model_levels)  !pressure
REAL :: t(row_length,rows,model_levels)            !temperature
REAL :: n_droplet(row_length,rows,model_levels)    !droplet concentration
...
REAL ::                                                                        &
  ...
  qctotal(row_length,rows,model_levels),                                       &
  !              Total cloud water
  clear_frac

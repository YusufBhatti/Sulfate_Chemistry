...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='SL3DWASH_MOD'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SL3DWASH'
...
REAL :: rho_r2        (1-off_x:row_length+off_x, 1-off_y:rows+off_y,           &
                           model_levels)
REAL :: ls_rain3d(row_length,rows,model_levels)        ! rain rate (kg/m2/s)
...
DO j=1,rows
  DO i=1,row_length
    ...
    ! Loop over model levels from surface up, and only scavenge at points
      ...
      IF (l_do_scav) THEN
        ...
        IF (ls_rain3d(i,j,k)  >   0) THEN
          ...
          IF (k  <   model_levels) THEN
            ...
            ! Convert DM to DRY density as level is wet

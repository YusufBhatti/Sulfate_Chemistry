...
CHARACTER(LEN=*), PARAMETER :: RoutineName='CARIOLLE_O3_PSC'

IF (lhook) CALL dr_hook(RoutineName,zhook_in,zhook_handle)

IF (PrintStatus  >=  PrStatus_Normal) THEN
  WRITE(umMessage,*) 'Calculating Cariolle Ozone tracer'
  CALL umPrint(umMessage,src='cariolle_o3_psc')
END IF
  ...
  DO j=1,rows
    DO i=1, row_length
      ...
      ELSE
        ...
        level_dso3 = (dp*to3/2.0)*2.152e25/101300.0
      ...
      IF (k==1) THEN
        ...
        ozone_tracer(i,j,k) = ozone_tracer(i,j,k) +              &
                           (timestep*o3_prod_loss_3D(i,j,k)) +   &
                           (timestep*(o3_p_l_temp_3D(i,j,k)*     &
                               (temp-o3_temp_3D(i,j,k))))

      ELSE
        ozone_tracer(i,j,k) = ozone_tracer(i,j,k) +              &
                           (timestep*o3_prod_loss_3D(i,j,k)) -   &
                           (timestep*(o3_vmr_3D(i,j,k)*          &
                                    o3_p_l_vmr_3D(i,j,k))) +     &
                           (timestep*(o3_p_l_temp_3D(i,j,k)*     &
                               (temp-o3_temp_3D(i,j,k)))) +      &
                           (timestep*(o3_p_l_colo3_3D(i,j,k)*    &
                           dso3(i,j)))

      END IF

      ozone_tracer(i,j,k)=ozone_tracer(i,j,k)/(1.0-(timestep*    &
                          o3_p_l_vmr_3D(i,j,k)))

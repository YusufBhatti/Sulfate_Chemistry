...
USE lsprec_mod, ONLY: m0, cx, constp, cff_spread_rate, delta_lambda,          &
                      delta_phi, wind_shear_factor,                           &
                      zero, one, two
...
REAL (KIND=real_lsprec) ::                                                    &
  ...
  m_bi_di(points),                                                            &
                        ...
                        ! Change in ice cloud fraction across tstep
  m_bic_dic(points),                                                          &
                        ! bic+dic'th moment of the ice particle dist.
  lamr(points),                                                               &
                        ! Lambda for rain.
  lamrh1(points),                                                             &
                        ! Lambda for rain + h1r
  ...
  DO c = 1, npts
    ...
    IF (l_rainfall_as) THEN
      ...
      IF (l_warm_new) THEN
        ...
        lamr(i) = (one/(qrain(i)*rho(i)*constp(50)/rainfrac(i)))            &
                                                         **cx(52)
      ELSE
        ! original (wrong) method which ignored rain fraction
        lamr(i) = (one/(qrain(i)*rho(i)*constp(50)))**cx(52)
      END IF
    ...
    ELSE
      ...
      IF (l_warm_new) THEN
        ! flux should be scaled by rain fraction
        fqi_rain(i) = constp(41) * corr(i) / 6.0_real_lsprec *              &
...
DO i = 1, points
  ...
  IF ((qcf_cry(i)+qcf_agg(i))  >   zero) THEN
    ...
    IF (falliceshear_method == original_but_wrong) THEN
      ...
      temp3(i) = MIN(MAX(temp3(i),zero),one)
    ...
    ELSE ! (falliceshear_method /= original_but_wrong)
      ...
      deltacff(i)=MIN(temp3(i) * overhang, one-cff(i))

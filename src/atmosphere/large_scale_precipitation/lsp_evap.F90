...
DO i = 1, points
  ...
  IF (qrain(i)  >   zero .AND. rainfrac(i)  >   zero) THEN
...
DO c = 1, npts
  ...
  IF (.NOT. l_mcr_qrain .AND. l_rainfall_as) THEN
    ...
    prelamnew(i) = qrain(i)*rho(i)*dhir(i)                                  &
               /(rainfrac(i)*constp(42)*corr(i))

    lamnew(i) =  one / (prelamnew(i)**cx(48))
...
DO c = 1, npts
  ...
  dpr(i) = dpr(i) * MAX(-temp7(i)*lheat_correc_liq(i),zero)                 &
...
DO c = 1, npts
  ...
  IF (rain_mix(i) == zero .AND. rain_liq(i) == zero) THEN

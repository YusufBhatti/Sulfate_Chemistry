...
REAL (KIND=real_lsprec) ::                                                    &
  ...
  cffnew(points)        ! New ice cloud fraction
...
!RJJG- should we use EPSILON?
  !    IF ((qrain(i)  <=  qcfmin .AND. qcl(i) <= zero)                      &
  ...
  cffnew(i) = MAX(MIN(cff(i),one),zero)
  cflnew(i) = MAX(MIN(cfl(i),one),zero)
  cfnew (i) = MAX( MAX(cflnew(i),cffnew(i)) , cf(i) )
  cfnew (i) = MIN( MIN(cflnew(i)+cffnew(i),one), cfnew(i) )
  ...
  IF (l_mcr_qcf2 .AND.                                                        &
    ...
    rftransfer(i) = rftransfer(i) +                                           &
      MAX( MAX(frac_ice_above(i),cff(i))-rainfrac(i) , zero)                  &
      * one_over_tsi

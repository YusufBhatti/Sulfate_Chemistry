...
! IN fields
  u_p, v_p, rho_wet_tq, z_uv, z_tq, z0m, zh, Rib, sil_orog_land,        &
! OUT fields
  tau_fd_x, tau_fd_y                                                    &
  )
...
USE jules_surface_mod, ONLY: fd_stab_dep, orog_drag_param
...
DO l = 1, land_pts
  ...
  ELSE
    ...
    fp_x(l)=0.5*rho_wet_tq(i,j,1)*orog_drag_param*                      &
            sil_orog_land(l)*rib_fn*tausx
    fp_y(l)=0.5*rho_wet_tq(i,j,1)*orog_drag_param*                      &

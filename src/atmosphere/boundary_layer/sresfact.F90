...
!           For JULES tiled land surface bdy lyr versions,
!           adapted from RESTILE.
...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'SRESFACT_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SRESFACT'
...
!   set to in SF_EXCH, but we repeat it here for safety.
...
DO n = 1, ntiles
  DO k = 1, tile_pts(n)
    ...
    IF (gs_tile(l,n)  >   cond_lim) THEN
      rs_tile(l,n) = 1.0 / gs_tile(l,n)
    ELSE
      rs_tile(l,n) = 1.0 / cond_lim
    END IF
  END DO
END DO
    ...
    DO k = 1, tile_pts(n)
      ...
      IF ( (catch(l,n)  >   0.01) .AND.                                 &
        ...
        IF ( canopy(l,n)  <=  catch(l,n) ) THEN
          damp_factor(l,n) = 1.0 - 0.66667*canopy(l,n)/catch(l,n)
        ELSE
...
DO n = 1, ntiles
  DO k = 1, tile_pts(n)
    ...
    IF (snow_tile(l,n) >  0.0 .AND.                                     &
      ...
      snow_f = 1.0 - EXP(-asnow*snow_tile(l,n))
      str_resist_s = 1.0/                                               &
          (snow_f/r_snow + (1.0-snow_f)/str_resist_s)
    END IF

...
! IN fields
 ...
 ntpar,ntml_nl,ntdsc,nbdsc,u_p,v_p,v_s,fb_surf,qw,tl,                   &
! IN/OUT fields
 cumulus,weight_1dbl,                                                   &
! OUT fields
...
USE bl_option_mod, ONLY:  WeightLouisToLong, Variable_RiC, cbl_op,      &
   ...
   blending_option, blend_except_cu, extended_tail, sg_shear_enh_lambda
USE gen_phys_inputs_mod, ONLY: l_mr_physics
USE planet_constants_mod, ONLY: vkman, c_virtual, grcp, g
...
USE turb_diff_ctl_mod, ONLY: rneutml_sq, delta_smag
...
IF (l_rp2) THEN
  g0=g0_rp
  ritrans=1.0/g0
  a_ri = 0.0       ! a_ri and b_ri need to be rewritten like this
  b_ri = 2.0 * g0  ! for bit-reproducibility
ELSE
  g0=10.0
  a_ri = (1.0-g0*ritrans)/(1.0-g0*ritrans/2.0)**2.0
  b_ri =         (g0/2.0)/(1.0-g0*ritrans/2.0)**2.0
END IF
  ...
  DO k = 2, bl_levels
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start, pdims%i_end
        ...
        BL_weight(i,j,k) = 0.5*( 1.0 - TANH(3.0*(zpr-1.0) ) )
      END DO
    END DO
  END DO
!$OMP END DO NOWAIT
...
!$OMP& SHARED(bl_levels,pdims,topbl,ri,RiCrit,mbl,local_fa,             &
!$OMP& ntml_local,zh_local,z_uv)
DO k = 2, bl_levels
!$OMP DO SCHEDULE(STATIC)
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
...
!-----------------------------------------------------------------------
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( pdims, ishear_bl, ntml_local, ntpar, cumulus, ntml_nl,   &
!$OMP         zh_local, z_uv )                                         &
!$OMP PRIVATE( i, j )
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
  ...
  CASE (sharpest)
    ...
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start, pdims%i_end
        ...
        ELSE
          func(i,j) = 1.0 / ( a_ri + b_ri*ri(i,j,k) )
        END IF
...
!$OMP         k, g0 )   PRIVATE( i, j )
    DO j = pdims%j_start, pdims%j_end
      DO i = pdims%i_start, pdims%i_end
        ...
        ELSE
          sharp(i,j) = 1.0 / ( a_ri + b_ri*ri(i,j,k) )
        END IF
...
!$OMP& lambdah_rho,vkz,f_log,zz,zht,zfa,beta,fh,rtmri,km)               &
!$OMP& SHARED(k,sbl_op,bl_levels,pdims,g0,ri,ric,                       &
!$OMP& ricinv,subg,flandg,ntml_local,ntml_nl,subb,dh,z_tq,              &
!$OMP& BL_weight,sg_orog_mixing,sigma_h,pr_n,                           &
!$OMP& l_rp2,lambda_min,par_mezcla,zh_local,                            &
!$OMP& h_blend,turb_length,k_log_layr,                                  &
!$OMP& z_uv,z0m,elm,elh,elh_rho,blending_option,cumulus,zhpar,          &
!$OMP& ntdsc,weight_1dbl,delta_smag,rneutml_sq,BL_diag,                 &
!$OMP& r_pr_n,cbl_op,subc,dm,h_tkeb,v_s,MOsurf,rho_wet_tq,              &
!$OMP& l_subfilter_vert,l_subfilter_horiz,fm_3d,fh_3d,rhokm,            &
  ...
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      ...
      IF (blending_option /= off) THEN
        ...
        IF (cumulus(i,j)) zht = MAX( zht, zhpar(i,j) )
        ...
        ! Relevant scale in cumulus layers is cloud top height, ntpar
        IF (cumulus(i,j)) z_scale = MAX( z_scale, zhpar(i,j) )
        ...
        ELSE
          weight_1dbl(i,j,k) =                                          &
           1.0 - TANH( beta*z_scale/delta_smag(i,j)) * MAX( 0.0,        &
               MIN( 1.0, (linear0-delta_smag(i,j)/z_scale)*rlinfac) )
        END IF
      ...
      IF (fb_surf(i,j) <= 0.0) THEN !only for stable/neutral cases
        ...
        DO k = 2, ntml_esbl(i,j)+1
          ...
          IF (zz <  hh) THEN !below ZH
            ...
            km =pkm *ll*USequil
            kh =pkh *ll*USequil
              ! Note "RHO" here is always wet density (RHO_WET_TQ)
            rhokm(i,j,k) = rho_wet_tq(i,j,k-1) * km
            IF (l_mr_physics) THEN
              ! Note "RHO" here is always wet density (RHO_WET_TQ) so
              ! save multiplication of RHOKH to after interpolation
              rhokh(i,j,k) = kh
            ELSE
              rhokh(i,j,k) = rho_wet_tq(i,j,k-1) * kh
            END IF
            dbdu=-1.0
            IF ((dvdzm(i,j,k) >  1.0e-10)                               &
                .AND. (dbdz(i,j,k) >  1.0e-10))                         &
            ...
            IF (L_SBLco .AND. (dbdu >  0.0)) THEN
              ...
              ! Note "RHO" here is always wet density (RHO_WET_TQ)
              rhokm(i,j,k) = rho_wet_tq(i,j,k-1)*(kuu+kut*dbdu)
              IF (l_mr_physics) THEN
                ! Note "RHO" here is always wet density (RHO_WET_TQ) so
                ! save multiplication of RHOKH to after interpolation
                rhokh(i,j,k) = (ktu/dbdu+ktt)
              ELSE
                rhokh(i,j,k) = rho_wet_tq(i,j,k-1)*(ktu/dbdu+ktt)
              END IF
            END IF

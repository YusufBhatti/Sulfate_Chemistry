...
USE bl_option_mod, ONLY:                                                &
    on, off, dec_thres_cloud, dec_thres_cu, ng_stress,                  &
    BrownGrant97, BrownGrant97_limited, flux_grad, Locketal2000,        &
    HoltBov1993, LockWhelan2006, entr_smooth_dec,                       &
    kprof_cu, klcl_entr, buoy_integ, max_cu_depth, bl_res_inv,          &
    a_ent_shr_nml, one_third, two_thirds, l_reset_dec_thres
USE model_domain_mod, ONLY: model_type, mt_single_column
USE missing_data_mod, ONLY: rmdi
USE planet_constants_mod, ONLY: vkman, g
...
REAL, INTENT(IN) ::                                                     &
 ...
 fq_nt (pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end,            &
                bl_levels+1)  !      moisture (rho*m/s) fluxes
...
LOGICAL :: scbase(pdims%i_start:pdims%i_end,                            &
                  pdims%j_start:pdims%j_end),                           &
                              ! Flag to signal base of CML reached
           test_well_mixed(pdims%i_start:pdims%i_end,                   &
                           pdims%j_start:pdims%j_end),                  &
                              ! Flag to test wb integration
                              ! for a well-mixed layer
           ksurf_iterate(pdims%i_start:pdims%i_end,                     &
                         pdims%j_start:pdims%j_end),                    &
                              ! Flag to perform iteration to
                              ! find top of Ksurf
           ktop_iterate(pdims%i_start:pdims%i_end,                      &
                        pdims%j_start:pdims%j_end)
                              ! Flag to perform iteration to
                              ! find base of Ktop
REAL ::                                                                 &
...
REAL :: wbmix( pdims%i_start:pdims%i_start+scmrowlen-1                  &
             , pdims%j_start:pdims%j_start+scmrow-1                     &
             , pdims%k_start:pdims%k_end )
                              ! WB*DZ if were diag as mixed
REAL :: wbend( pdims%i_start:pdims%i_start+scmrowlen-1                  &
             , pdims%j_start:pdims%j_start+scmrow-1                     &
             , pdims%k_start:pdims%k_end )
                              ! WB*DZ after dec diag

REAL:: w_m_top(pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end),    &
...
IF ( l_rp2 .AND. i_rp_scheme == i_rp2b ) THEN
  ...
  IF ( a_ent_1_rp_min < 0.23 .AND. a_ent_1_rp_max > 0.23 ) THEN
    a_ent_shr =                                                         &
     ...
     ( ( a_ent_shr_rp_max*                                              &
         ( a_ent_1 - 0.23 ) / ( a_ent_1_rp_max - a_ent_1_rp_min ) )     &
     - ( a_ent_shr_rp*                                                  &
         ( a_ent_1 - a_ent_1_rp_max ) / ( 0.23 - a_ent_1_rp_min ) ) )
  ELSE
    ! Case a_ent_1_rp_min = a_ent_1 = a_ent_1_rp_max
    a_ent_shr = a_ent_shr_rp * a_ent_1 / 0.23
  END IF
ELSE
  a_ent_shr = a_ent_shr_nml * a_ent_1 / 0.23   ! Entrainment parameter.
END IF
          ...
          IF ( (k >= ksurf(i,j)+1) .AND. (k <= ntop(i,j)) ) THEN
            ...
            IF (z_pr  >   0.0 .AND. z_pr  <   zinv_pr(i,j)) THEN
              ...
              IF (flux_grad  ==  LockWhelan2006) THEN
                khtop(i,j) = 3.6 * vkman * rho_mix(i,j,k) *v_ktop(i,j)  &
                                 * zinv_pr(i,j) * (z_ratio**3)          &
                                 * ( (1.0-z_ratio)*(1.0-z_ratio) )
                f2 = rho_mix(i,j,k) * 0.5 * z_ratio                     &
                                         * 2.0**( z_ratio**4 )
                IF ( v_ksum(i,j)  >   0.0 ) THEN
                  fsc = rho_mix(i,j,k) * 3.5*(v_ktop(i,j)/v_ksum(i,j))  &
  ...
  DO i = pdims%i_start, pdims%i_end
    IF ( test_well_mixed(i,j) ) THEN
      ...
      ELSE   ! buoyancy flux threshold violated
          ...
          !---------------------------------
        IF ( .NOT. cumulus(i,j) .OR. kprof_cu == buoy_integ)            &
                 ksurf_iterate(i,j) = .TRUE.
        IF ( cumulus(i,j) .AND. kprof_cu == buoy_integ)                 &
  ...
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      IF ( cumulus(i,j) .AND. .NOT. dsc(i,j) .AND.                      &
        ...
        z_bot_lim(i,j)=z_lcl(i,j)          ! ...the LCL and ...
        z_top_lim(i,j)=z_lcl(i,j) + 1000.0 ! ...1km above the LCL
        ntop(i,j)=ntml(i,j)
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    ...
    IF ( ktop_iterate(i,j) ) THEN
      ...
      z_inc(i,j) = (z_top_lim(i,j)-z_bot_lim(i,j)) / REAL(n_steps)
        ...
        DO ic = jj, MIN(jj+bl_segment_size-1, c_len_i)
          ...
          IF ((k >= ksurf(i1,j1)+1) .AND. (k <= ntop(i1,j1))) THEN
            ...
            IF (z_pr >   0.0 .AND. z_pr <  zinv_pr(i1,j1)) THEN
              ...
              IF (flux_grad  ==  LockWhelan2006) THEN
                khtop(i1,j1) = 3.6 * vkman * rho_mix(i1,j1,k)           &
                             * v_top_dsc(i1,j1) * zinv_pr(i1,j1)        &
                  * (z_ratio**3) * ( (1.0-z_ratio)*(1.0-z_ratio) )
                f2 = rho_mix(i1,j1,k) * 0.5 * z_ratio                   &
                                     * 2.0**( z_ratio**4 )
                IF ( v_sum_dsc(i1,j1)  >   0.0 ) THEN
                  fsc = 3.5 * rho_mix(i1,j1,k)                          &
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    zsml_top(i,j) = zh(i,j)
    IF (bl_res_inv == on .AND. dzh(i,j) > 0.0)                          &
  ...
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      IF ( cumulus(i,j) ) THEN
        ...
        zsml_top(i,j) = MAX( 1.1*z_lcl(i,j), zsml_top(i,j) )
        ! ...but no higher than parcel top:
        zsml_top(i,j) = MIN( zhpar(i,j), zsml_top(i,j) )

        cu_depth_scale(i,j) = (zsml_top(i,j)-zh(i,j))/3.0
      ...
      DO i = pdims%i_start, pdims%i_end
        ...
        IF ( zk_uv  <   zh(i,j) .AND.                                   &
          ...
          IF (flux_grad  ==  LockWhelan2006) THEN
            ...
            rhokh_top(i,j,k) = 3.6*vkman * rho_mix(i,j,k) * v_top(i,j)  &
                               * zh_pr * (z_ratio**3)                   &
                               * (( 1.0 - z_ratio )**2)

            IF ( .NOT. coupled(i,j) ) THEN
              rhof2(i,j,k)  = rho_mix(i,j,k) * 0.5 * z_ratio            &
                                          * 2.0**( z_ratio**4 )
              IF ( v_sum(i,j)  >   0.0 ) THEN
                rhofsc(i,j,k) = 3.5 * rho_mix(i,j,k)                    &
                                * (v_top(i,j)/v_sum(i,j))               &
                                * (z_ratio**3) * (1.0-z_ratio)
              END IF
            END IF
        ...
        IF ( zk_uv  <   zhsc(i,j) .AND.                                 &
          ...
          IF (flux_grad  ==  LockWhelan2006) THEN
            ...
            rhokh_top(i,j,k) = 3.6*vkman * rho_mix(i,j,k)               &
...
IF (flux_grad  ==  LockWhelan2006) THEN
  ...
  pr_neut = 0.75
  pr_conv = 0.6
  c_tke = 1.33/(vkman*c_ws**two_thirds)
        ...
        DO i = pdims%i_start, pdims%i_end
          ...
          IF (fb_surf(i,j)  >=  0.0) THEN
            ...
            IF (zk_uv  <=  0.1*zh(i,j)) THEN
              ...
              w_s_cubed_uv = 10.0*c_ws * zk_uv * fb_surf(i,j)
            ELSE
            ...
            IF (zk_tq  <=  0.1*zh(i,j)) THEN
              !             Surface layer calculation
              w_s_cubed_tq = 10.0*c_ws * zk_tq * fb_surf(i,j)
            ELSE
              !             Outer layer calculation
            ...
            ELSE IF ( kprof_cu == klcl_entr .AND. cumulus(i,j) ) THEN
              IF ( zk_uv < zsml_top(i,j) ) THEN
                ...
                rhokh(i,j,k) = rhokh_lcl(i,j) *                         &
                       ...
                       (1.0-(zk_uv-zh(i,j))/(zsml_top(i,j)-zh(i,j)))
              END IF
            END IF
            IF ( zk_tq  <   zh(i,j) ) THEN
              !---------------------------------------------------------
            ...
            ELSE IF ( kprof_cu == klcl_entr .AND. cumulus(i,j) ) THEN
              IF ( zk_tq < zsml_top(i,j) ) THEN
                ...
                rhokm(i,j,k) = prandtl_top(i,j) * rhokh_lcl(i,j) *      &
                       ...
                       (1.0-(zk_tq-zh(i,j))/(zsml_top(i,j)-zh(i,j)))
              END IF
            END IF
            IF (BL_diag%l_tke) THEN
              ! save 1/timescale for TKE diag, completed in bdy_expl2
...
IF ( l_scmdiags(scmdiag_bl) .AND.                                       &
  ...
  CALL scmoutput(wbmix,'wb_mix',                                        &
       'Buoyancy flux if a mixed layer','m2/s3',                        &
       t_avg,d_bl,default_streams,'',routinename)

  CALL scmoutput(wbend,'wb_end',                                        &
       'Final estimate of buoyancy flux','m2/s3',                       &
       t_avg,d_bl,default_streams,'',routinename)

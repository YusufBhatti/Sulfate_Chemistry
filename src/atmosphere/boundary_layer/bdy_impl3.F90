...
USE atm_fields_bounds_mod, ONLY:                                        &
 udims, vdims, udims_s, vdims_s, pdims, pdims_s, tdims, tdims_l
...
REAL ::                                                                 &
 ...
 cu_prod(udims%i_start:udims%i_end,udims%j_start:udims%j_end),          &
                ...
                ! for implicit coupling at level k_blend_uv
...
!$OMP& ct_prod, cu_prod, cv_prod,k_blend_tq,k_blend_uv,                 &
!$OMP& GAMMA,cq_cm_u,cq_cm_v,du_nt,dv_nt,rhokm_v,lcrcp,lsrcp)           &
...
omp_block = udims%j_len
!$ omp_block=CEILING(udims%j_len/REAL(omp_get_num_threads()))
  ...
  DO jj = udims%j_start, udims%j_end, omp_block
    DO k = blm1, 2, -1
      ...
      DO j = jj, MIN(jj+omp_block-1, udims%j_end)
        DO i = udims%i_start, udims%i_end
          ! Copy gamma into an array defined on u-points
          ! In principle, gamma should be interpolated, but sensitivity
          ! is expected to be small so the adjacent p-point is used to
          ! avoid message passing. This makes endgame formulation
      ...
      CALL oneover_v(l, temp_u, temp_u_out)

      l = 0
      DO j = jj, MIN(jj+omp_block-1, udims%j_end)
        DO i = udims%i_start, udims%i_end
          l = l + 1
          du(i,j,k) = temp_u_out(l) * du(i,j,k)
          cq_cm_u(i,j,k) = temp_u_out(l) * cq_cm_u(i,j,k)
        END DO
...
DO j = udims%j_start, udims%j_end
  DO i = udims%i_start, udims%i_end
    ...
    du_1(i,j) = du(i,j,k_blend_uv(i,j))
    cu_prod(i,j) = cq_cm_u(i,j,k_blend_uv(i,j))

    DO k = k_blend_uv(i,j)-1, 1, -1

      du_1(i,j) = du_1(i,j) + ( (-1) ** (k_blend_uv(i,j) + k) ) *       &
                  du(i,j,k) * cu_prod(i,j)

      cu_prod(i,j) = cu_prod(i,j) * cq_cm_u(i,j,k)

    END DO

    cq_cm_u_1(i,j) = ( (-1) ** ( k_blend_uv(i,j) + 1 ) ) *              &
                     cu_prod(i,j)
  END DO
END DO
!$OMP END DO
...
!$OMP DO SCHEDULE(STATIC)
  DO j = vdims%j_start, vdims%j_end
    DO i = vdims%i_start, vdims%i_end
      ! Copy gamma into an array defined on v-points
      ! In principle, gamma should be interpolated, but sensitivity is expected
      ! to be small so the adjacent p-point is used to avoid message passing
      ! This makes endgame formulation consistent with the current formulation
...
omp_block = vdims%j_len
!$ omp_block=CEILING(vdims%j_len/REAL(omp_get_num_threads()))
...
!$OMP DO SCHEDULE(STATIC)
  DO jj = vdims%j_start, vdims%j_end, omp_block
    DO k = blm1, 2, -1
      ...
      CALL oneover_v(l, temp_v, temp_v_out)

      l = 0
      DO j = jj, MIN(jj+omp_block-1, vdims%j_end)
        DO i = vdims%i_start, vdims%i_end
          l = l + 1
          dv(i,j,k) = temp_v_out(l) * dv(i,j,k)
          cq_cm_v(i,j,k) = temp_v_out(l) * cq_cm_v(i,j,k)
        END DO
      END DO
    END DO ! loop over 2,BLM1
  END DO
...
!-----------------------------------------------------------------
!! Calculate new coefficients for calculation of implicit v momentum
!! fluxes, i.e. Upward sweep of matrix (to k_blend)
!! NOTE: Gives coeffs for coupling to the bottom model level
!! when k_blend = 1 (i.e. when blend_height_opt switch set to blend_level1)
!-----------------------------------------------------------------
...
DO j = vdims%j_start, vdims%j_end
  DO i = vdims%i_start, vdims%i_end
    ...
    dv_1(i,j) = dv(i,j,k_blend_uv(i,j))
    cv_prod(i,j) = cq_cm_v(i,j,k_blend_uv(i,j))

    DO k = k_blend_uv(i,j)-1, 1, -1

      dv_1(i,j) = dv_1(i,j) + ( (-1) ** ( k_blend_uv(i,j) + k ) ) *     &
                  dv(i,j,k) * cv_prod(i,j)
      cv_prod(i,j) = cv_prod(i,j) * cq_cm_v(i,j,k)

    END DO

    cq_cm_v_1(i,j) = ( (-1) ** ( k_blend_uv(i,j) + 1 ) ) *              &
                     cv_prod(i,j)
  END DO
END DO
!$OMP END DO

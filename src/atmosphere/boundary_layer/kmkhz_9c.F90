...
! INOUT fields
 ftl,fqw,zh,dzh,cumulus,ntml,w,etadot,t1_sd,q1_sd,                      &
! OUT fields
...
USE atm_fields_bounds_mod, ONLY: pdims, tdims,                          &
                                 ScmRowLen, ScmRow, tdims_l, pdims_s
USE bl_diags_mod, ONLY: strnewbldiag
USE tuning_segments_mod, ONLY:  bl_segment_size
USE nlsizes_namelist_mod, ONLY: model_levels
USE bl_option_mod, ONLY:                                                &
    entr_enhance_by_cu, Buoyrev_feedback, on, l_new_kcloudtop,          &
    relax_sc_over_cu, a_grad_adj, max_t_grad, flux_grad, Locketal2000,  &
    HoltBov1993, LockWhelan2006, entr_smooth_dec, kprof_cu,             &
    bl_res_inv, pr_max, one_third
...
USE planet_constants_mod, ONLY: r, c_virtual, g, etar, grcp, lcrcp, lsrcp
...
REAL :: qs(pdims%i_start:pdims%i_end,pdims%j_start:pdims%j_end,bl_levels),&
...
REAL :: db_ga_dry(pdims%i_start:pdims%i_end,                            &
...
REAL :: bflux_surf(pdims%i_start:pdims%i_end,                           &
        ...
        btc_dsct(pdims%i_start:pdims%i_end,                             &
                 pdims%j_start:pdims%j_end),                            &
...
! For SCM diagnostics
REAL :: TmpScm (ScmRowLen,ScmRow)
REAL :: TmpScm2(ScmRowLen,ScmRow)
REAL :: TmpScm3(ScmRowLen,ScmRow,model_levels)
...
INTEGER ::  k_cloud_top(pdims%i_start:pdims%i_end,                      &
            ...
            k_plume(pdims%i_start:pdims%i_end,                          &
                    pdims%j_start:pdims%j_end),                         &
...
REAL ::                                                                 &
 ...
 rhokh_inv,                                                             &
                    ! Estimate of rho*KH within resolved inversion
 svl_lapse_rho,                                                         &
...
INTEGER :: iScm, jScm ! Counters for Scm arrays
...
!$OMP& zdsc_cbase, cfl_ml, cff_ml, kmax, db_disc,k_cff, qw_plume, dqcl, &
!$OMP& dqcf, k_rad_lim, z_rad_lim, cu_depth_fac, grad_t_adj_inv_rdz,    &
!$OMP& t_parc, q_liq_parc, q_liq_env, q_vap_parc,svl_plume,sl_plume,    &
!$OMP& t_dens_parc, t_dens_env, dpar_bydz, denv_bydz, quad_a, quad_bm,  &
!$OMP& quad_c, svl_lapse, svl_lapse_base, z_surf, monotonic_inv,        &
!$OMP& w_curv_nm,w_del_nm,w_curv,jj,svl_diff,grad_q_adj_inv_rdz)
...
!$OMP DO SCHEDULE(STATIC)
    DO k = 1, bl_levels
      CALL qsat_mix_new(qs(:,:,k),t(:,:,k),p(:,:,k),pdims%i_end,pdims%j_end)
    END DO
!$OMP END DO
  ELSE
!$OMP DO SCHEDULE(STATIC)
    DO k = 1, bl_levels
      CALL qsat_new(qs(:,:,k),t(:,:,k),p(:,:,k),pdims%i_end,pdims%j_end)
    END DO
!$OMP END DO
  END IF
ELSE
!$OMP DO SCHEDULE(STATIC)
  DO k = 1, bl_levels
    ...
    CALL qsat_mix(qs(1,1,k),t(1,1,k),p(1,1,k),                            &
                  pdims%i_end*pdims%j_end,l_mr_physics)
  END DO
!$OMP END DO
END IF
      ...
      DO i = pdims%i_start, pdims%i_end
        ...
        IF ( cloud_base(i,j) .AND. .NOT. dsc(i,j) .AND.                 &
          ...
          ELSE IF ( (svl(i,j,k-1)-svl(i,j,k-2))                         &
            ...
            ! ------------------------------------------------------------
            IF (t(i,j,k) >  tm) THEN
              q_liq_parc = MAX( 0.0, ( qw_plume - qs(i,j,k) -           &
                dqsdt(i,j,k)*                                           &
  ...
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      ...
      IF ( dsc(i,j) .AND. ntdsc(i,j)+2 <= bl_levels ) THEN
        z_rad_lim = MAX( z_tq(i,j,ntdsc(i,j)+2)+0.1, 1.2*zhsc(i,j) )

        k = ntml(i,j)+2
        DO WHILE (z_tq(i,j,k) < z_rad_lim .AND. k < bl_levels)
          IF ( z_tq(i,j,k) > 0.5*zhsc(i,j)                              &
        ...
        IF ( k >  1 .AND. k < bl_levels -1 ) THEN
          IF (dflw_over_cp(i,j,k+1) < 1.5*dflw_over_cp(i,j,k+2))        &
  ...
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      ...
      DO k = MAX(1,k_rad_lim), MIN(bl_levels,ntdsc(i,j)+2)
        ...
        IF ( dsc(i,j) .AND. z_tq(i,j,k) > 0.5*zhsc(i,j)                 &
              .AND. dflw_over_cp(i,j,k) > df_dsct_over_cp(i,j) ) THEN
          k_cloud_dsct(i,j) = k
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    ...
    IF ( bl_res_inv == on .AND. .NOT. cumulus(i,j) .AND.                &
         dzh(i,j) > 1.0 .AND. ntml(i,j)+2 <= bl_levels ) THEN
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    DO k = MIN(bl_levels, ntml(i,j)), 1, -1
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    DO k = MIN(bl_levels,ntdsc(i,j)), 1, -1
      IF ( .NOT. cloud_base(i,j) .AND.                                  &
...
DO k = 2, bl_levels
  DO j = pdims%j_start, pdims%j_end
    ...
    DO i = pdims%i_start, pdims%i_end
      ...
      ELSE
        grad_t_adj_inv_rdz(i) = 0.0
        grad_q_adj_inv_rdz(i) = 0.0
      END IF
...
!DIR$ VECTOR ALWAYS
    DO i = pdims%i_start, pdims%i_end
      ...
      db_noga_cld(i,j,k)  = - g *                                       &
        ...
        !-------------------------------------------------------
      db_ga_cld(i,j,k) = db_ga_dry(i,j,k)*(1.0-cf_for_wb(i)) +          &
                         db_ga_cld(i,j,k)*cf_for_wb(i)
      db_noga_cld(i,j,k)  = db_noga_dry(i,j,k)*(1.0-cf_for_wb(i)) +     &
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    ...
    IF ( sml_disc_inv(i,j) == 1 ) THEN
      ...
      IF (cf_sml(i,j)  >   0.0) THEN
        ...
        cfl_ml = cf_sml(i,j)*cfl(i,j,kmax)                              &
                               /(cfl(i,j,kmax)+cff(i,j,kmax)+1.0e-10)
    ...
    chi_s_top(i,j) = MAX( 0.0, MIN( chi_s_top(i,j), 1.0) )
  ...
  DO i = pdims%i_start, pdims%i_end
    IF ( dsc(i,j) ) THEN
      ...
      chi_s_dsct(i,j) = -qcl_ic_top(i,j) /                              &
                           (a_qsm(i,j,km)*dqw - a_dqsdtm(i,j,km)*dsl)
      chi_s_dsct(i,j) = MAX( 0.0, MIN( chi_s_dsct(i,j), 1.0) )
    ...
    IF (dsc(i,j)) THEN
      ...
      ELSE
        ...
        IF ( entr_enhance_by_cu == Buoyrev_feedback                     &
             ...
             .AND. d_siems_dsc(i,j) > 1.0e-14 ) THEN
          ...
          cu_depth_fac = 0.5*( 1.0+                                     &
                    TANH( ((zhpar(i,j)-zh(i,j))-700.0)/100.0) )
...
!$OMP DO SCHEDULE(DYNAMIC)
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      k_rad_lim = ntml(i,j)+1
      z_rad_lim = MAX( z_tq(i,j,k_rad_lim)+0.1, 1.2*zh(i,j) )
      k = 1
      DO WHILE (z_tq(i,j,k) <  z_rad_lim .AND. k < bl_levels)
...
! DEPENDS ON: excf_nl_9c
CALL excf_nl_9c (                                                       &
! IN levels/switches
   bl_levels,BL_diag,nSCMDpkgs,L_SCMDiags,                              &
! IN fields
   rdz,z_uv,z_tq,rho_mix,rho_wet_tq,rhostar_gb,v_s,fb_surf,zhpar,z_lcl, &
...
!$OMP  PARALLEL DEFAULT(SHARED)                                         &
!$OMP& PRIVATE (i, j, k, kl, wstar3, w_m, w_h, ml_tend, fa_tend,        &
!$OMP& inv_tend, tothf_efl, moisten, totqf_efl, w_s_ent, w_s_cubed,     &
!$OMP& Prandtl, rhokh_inv, recip_svl_lapse, svl_lapse_rho)
  ...
  DO i = pdims%i_start, pdims%i_end
    ...
    ELSE   ! NOT specifying entrainment flux but KH
      ...
      IF (res_inv(i,j) == 1) THEN
        Prandtl = MIN( rhokm(i,j,k)/(1.0e-10+rhokh_surf_ent(i,j)),      &
                       pr_max )
        ...
        DO WHILE ( z_uv(i,j,kl) < zh(i,j)+dzh(i,j) .AND.                &
          ...
          recip_svl_lapse = (z_tq(i,j,kl)-z_tq(i,j,kl-1))/              &
                            MAX( 0.01, svl(i,j,kl)-svl(i,j,kl-1) )
          rhokh_inv = rhokh_surf_ent(i,j) * svl_lapse_rho *             &
                 rho_mix(i,j,kl) * recip_svl_lapse *                    &
                 COS(0.5*pi*(z_uv(i,j,kl)-zh(i,j))/dzh(i,j))
          rhokh_inv = MIN( rhokh_inv, 1000.0 )
          rhokh(i,j,kl) = MAX( rhokh(i,j,kl), rhokh_inv )
          ! rescale for KM on staggered grid
          rhokm(i,j,kl) = MAX( rhokm(i,j,kl), Prandtl * rhokh_inv       &
...
DO j = pdims%j_start, pdims%j_end
  DO i = pdims%i_start, pdims%i_end
    ...
    t_frac_tr(i,j,1) = 0.0
    zrzi_tr(i,j,1)   = 0.0
    we_lim_dsc(i,j,1)    = 0.0
    t_frac_dsc_tr(i,j,1) = 0.0
    zrzi_dsc_tr(i,j,1)   = 0.0
    we_lim(i,j,3)    = 0.0
    t_frac_tr(i,j,3) = 0.0
    zrzi_tr(i,j,3)   = 0.0
    we_lim_dsc(i,j,3)    = 0.0
    t_frac_dsc_tr(i,j,3) = 0.0
    zrzi_dsc_tr(i,j,3)   = 0.0
  END DO ! i
END DO ! j
!$OMP END DO NOWAIT
  ...
  DO i = pdims%i_start, pdims%i_end
    IF ( unstable(i,j) ) THEN
      ...
      ELSE IF (flux_grad  ==  HoltBov1993) THEN
        ...
        w_m =( v_s(i,j)**3 + 0.6*wstar3 )**one_third
      ...
      ELSE IF (flux_grad  ==  LockWhelan2006) THEN
          ...
          ! Neut limit GAMMA_TH = 7.5*FTL1/(ustar*zh)
        wstar3 = fb_surf(i,j) * zh(i,j)
        w_h =( ((4.0/3.0)*v_s(i,j))**3 + wstar3 )**one_third
  ...
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      ...
      IF (kprof_cu >= on .AND. cumulus(i,j)) THEN
        BL_diag%dzh(i,j)= MAX( 0.0, zh(i,j)-z_lcl(i,j) )
      ELSE
        BL_diag%dzh(i,j)= MAX( 0.0, dzh(i,j) )
      END IF
    END DO
  END DO
!$OMP END DO NOWAIT
...
IF ( l_scmdiags(scmdiag_bl) .AND.                                       &
  ...
  !------------------------------------------------------
  ! Arrays must be synchronized across threads to prevent race conditions
!$OMP BARRIER
!$OMP SINGLE
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      ...
      ! and in cumulus set dzh=zh-z_lcl
      IF (kprof_cu >= on .AND. cumulus(i,j)) THEN
        TmpScm(iScm,jScm)= MAX( 0.0, zh(i,j)-z_lcl(i,j) )
      ELSE
        TmpScm(iScm,jScm)= MAX( 0.0, dzh(i,j) )
      END IF
    END DO
  END DO

  CALL scmoutput(TmpScm,'dzh_bl',                                       &
       'Inversion thickness after BL','m',                              &
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( dsc(i,j) ) THEN
        TmpScm(iScm,jScm) = zhsc(i,j) - dscdepth(i,j)
      ELSE
        TmpScm(iScm,jScm) = zh(i,j)
      END IF
    END DO
  END DO

  CALL scmoutput(TmpScm,'DSCbase',                                      &
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( coupled(i,j) ) THEN
        TmpScm(iScm,jScm)= 1.0
      ELSE
        TmpScm(iScm,jScm)= 0.0
      END IF
    END DO
  END DO

  CALL scmoutput(TmpScm,'coupled',                                      &
       'Weakly coupled DSC Indicator','',                               &
       t_avg,d_sl,default_streams,'',routinename)

  CALL scmoutput(svl_diff_frac,'svl_diff_frac',                         &
       'Decoupling SVL fraction','',                                    &
       t_avg,d_sl,default_streams,'',routinename)
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( dsc(i,j) ) THEN
        TmpScm(iScm,jScm) = zhsc(i,j) - zc_dsc(i,j)
      ELSE
        TmpScm(iScm,jScm) = zh(i,j)   - zc(i,j)
      END IF
    END DO
  END DO

  CALL scmoutput(TmpScm,'ScCldBase',                                    &
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( dsc(i,j) ) THEN
        TmpScm(iScm,jScm)= we_dsc_parm(i,j)
      ELSE
        TmpScm(iScm,jScm)= we_parm(i,j)
      END IF
    END DO
  END DO

  CALL scmoutput(TmpScm,'Entr_BL',                                      &
       'BL-top entrainment rate','m/s',                                 &
       t_avg,d_sl,default_streams,'',routinename)
  !------------------------------------------------------
  CALL scmoutput(d_siems,'Dsiems_sml',                                  &
       'SML-top D_ctei','',                                             &
       t_avg,d_sl,default_streams,'',routinename)
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( dsc(i,j) ) THEN
        TmpScm(iScm,jScm) = d_siems_dsc(i,j)
      ELSE
        TmpScm(iScm,jScm) = d_siems(i,j)
      END IF
    END DO
  END DO

  CALL scmoutput(TmpScm,'Dsiems',                                       &
       'BL top D_ctei','',                                              &
       t_avg,d_sl,default_streams,'',routinename)
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      ...
      IF ( ABS(dqw_sml(i,j)) < SQRT(TINY(1.0)) ) THEN
        TmpScm(iScm,jScm)= rmdi
      ELSE
        TmpScm(iScm,jScm)= 1.0 + dsl_sml(i,j)/(lcrcp*dqw_sml(i,j))
      END IF

      IF ( dsc(i,j) ) THEN
        IF ( ABS(dqw_dsc(i,j)) < SQRT(TINY(1.0)) ) THEN
          TmpScm2(iScm,jScm)= rmdi
        ELSE
          TmpScm2(iScm,jScm)= 1.0 + dsl_dsc(i,j)/(lcrcp*dqw_dsc(i,j))
        END IF
      ELSE
        TmpScm2(iScm,jScm)= TmpScm(iScm,jScm)
      END IF

    END DO
  END DO

  CALL scmoutput(TmpScm,'K_ctei_sml',                                   &
       'SML top CTEI parameter','',                                     &
       t_avg,d_sl,default_streams,'',routinename)
  CALL scmoutput(TmpScm2,'K_ctei',                                      &
       'BL-top CTEI parameter','',                                      &
       t_avg,d_sl,default_streams,'',routinename)
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( dsc(i,j) ) THEN
        TmpScm(iScm,jScm)= chi_s_dsct(i,j)
      ELSE
        TmpScm(iScm,jScm)= chi_s_top(i,j)
      END IF
    END DO
  END DO

  CALL scmoutput(chi_s_top,'CHI_S_sml',                                 &
       'SML top CHI_S parameter','',                                    &
       t_avg,d_sl,default_streams,'',routinename)
  CALL scmoutput(TmpScm,'CHI_S',                                        &
       'BL-top CHI_S parameter','',                                     &
       t_avg,d_sl,default_streams,'',routinename)
  ...
  DO j=pdims%j_start, pdims%j_end
    ...
    DO i=pdims%i_start, pdims%i_end
      iScm = i - pdims%i_start + 1
      IF ( dsc(i,j) ) THEN
        TmpScm(iScm,jScm)= db_dsct(i,j)
      ELSE
        TmpScm(iScm,jScm)= db_top(i,j)
      END IF
    END DO
  END DO

  CALL scmoutput(db_top,'DB_TOP',                                       &
       'SML inversion strength','m2/s3',                                &
       t_avg,d_sl,default_streams,'',routinename)
  CALL scmoutput(TmpScm,'DB_BL',                                        &
       'BL-top inversion strength','',                                  &
       t_avg,d_sl,default_streams,'',routinename)
  !------------------------------------------------------
  CALL scmoutput(qcl_ic_top,'QCL_IC_TOP',                               &
       'BL-top in-cloud water content','',                              &
       t_avg,d_sl,default_streams,'',routinename)
  !------------------------------------------------------
  CALL scmoutput(df_top_over_cp,'df_top',                               &
       'radiative flux difference across BL top','Km/s',                &
       t_avg,d_sl,default_streams,'',routinename)

  ! initialise temporary array to zero
  TmpScm3(:, :, :) = 0.0
  ! note that frad_lw is only calculated on levels 1:bl_levels+1
  DO k=1, bl_levels+1
    DO j=pdims%j_start, pdims%j_end
      jScm = j - pdims%j_start + 1
      DO i = pdims%i_start, pdims%i_end
        iScm = i - pdims%i_start + 1
        TmpScm3(iScm,jScm,k)= frad_lw(i,j,k)
      END DO
    END DO
  END DO

  CALL scmoutput(TmpScm3,'frad_lw',                                     &
       'Net LW radiative flux in BL','Km/s',                            &

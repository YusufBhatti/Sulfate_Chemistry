...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ROW_COL_2NORM'
...
IF ( print_time == timestep_number ) THEN
  ...
  IF (mype == 0) THEN
    WRITE(umMessage,'('' rows '')')
    CALL umPrint(umMessage,src='row_col_2norm')
    DO j = 1, global_rows, 4
      WRITE(umMessage,'(''row '',I4,4E24.16)') j,                         &
               ( global_2norm_rows(i), i = j, j+3 )
      CALL umPrint(umMessage,src='row_col_2norm')
    END DO
    WRITE(umMessage,'('' columns '')')
    CALL umPrint(umMessage,src='row_col_2norm')
    DO j = 1, global_row_length, 4
      WRITE(umMessage,'(''point '',I4,4E24.16)') j,                       &

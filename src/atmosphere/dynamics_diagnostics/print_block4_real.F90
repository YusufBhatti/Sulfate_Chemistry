...
CHARACTER(LEN=*), PARAMETER :: RoutineName='PRINT_BLOCK4_REAL'
...
IF ( print_time == timestep_number ) THEN
  ...
  CALL gc_rsum(SIZE, nproc, info, ll)
  CALL gc_rsum(SIZE, nproc, info, lr)
  CALL gc_rsum(SIZE, nproc, info, ul)
  CALL gc_rsum(SIZE, nproc, info, ur)
  ...
  DO k = 1, num_lev
    ...
    CALL umPrint(umMessage,src='print_block4_real')
    ii = dom_w
    IF (ix == 4) THEN
      WRITE(umMessage,'(''column             '', i4,                      &
      CALL umPrint(umMessage,src='print_block4_real')
      ''                     '', i4,''                   '', i4,  &

...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ARRAY_L2NORM'
...
IF ( L_do_rims ) THEN
  ...
  IF ( L_do_halos ) THEN
    ...
    j_start = 1 - halo_j
    j_stop = rows + halo_j
    ipoints = (global_row_length + 2 * nproc_x * halo_i ) *       &
              (global_rows + 2 * nproc_y * halo_j )
  END IF ! L_do_halos
...
ELSE  ! do not do halos or rims
  ...
  IF ( L_do_halos ) THEN
    ...
    CALL umPrint(umMessage,src='array_l2norm')
  END IF ! L_do_halos
...
! Whatever options chosen in the gui/namelist we want to force this sum to

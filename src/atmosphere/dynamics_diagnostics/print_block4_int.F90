...
! Purpose:
!          To print (sub-)domain corner values of an integer field
...
!     WRITE(6,*)' *** i_out  ***'
!! DEPENDS ON: print_block4_int
!      CALL print_block4_int(
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='PRINT_BLOCK4_INT'
...
IF ( print_time == timestep_number ) THEN
  ...
  ELSE IF (dom_e + 1 > datastart(1) .AND.                         &
    ...
    ie = ix - iee + ies - 1
  END IF !  dom_e - ix + 1 > datastart(1) + row_length - 1
  ...
  ll = 0      ! lower left values
  lr = 0      ! lower right values
  ul = 0      ! upper left values
  ur = 0      ! upper right values
  ...
  CALL gc_isum(SIZE, nproc, info, ll)
  CALL gc_isum(SIZE, nproc, info, lr)
  CALL gc_isum(SIZE, nproc, info, ul)
  CALL gc_isum(SIZE, nproc, info, ur)
  ...
  DO k = 1, num_lev
    ...
    CALL umPrint(umMessage,src='print_block4_int')
    ii = dom_w
    IF (ix == 4) THEN
      WRITE(umMessage,'(''column          '',I4,''        '', i4,         &
      CALL umPrint(umMessage,src='print_block4_int')
      ''        '',i4,''        '',i4)') ii, ii+1, ii+2, ii+3
    ELSE IF (ix == 3) THEN
      WRITE(umMessage,'(''column          '',I4,''        '', i4,         &
      CALL umPrint(umMessage,src='print_block4_int')
      ''        '',i4)') ii, ii+1, ii+2
    ELSE IF (ix == 2) THEN
      WRITE(umMessage,'(''column          '',I4,''        '', I4)')ii,ii+1
      CALL umPrint(umMessage,src='print_block4_int')
    ELSE
      WRITE(umMessage,'(''column          '',I4)')ii
      CALL umPrint(umMessage,src='print_block4_int')
    END IF !  ix == 4
    DO j = 1, jy
      jn = dom_n - j + 1
      WRITE(umMessage,'(''row '',I4,4I12)') jn, (ul(i,j,k), i = 1, ix)
      CALL umPrint(umMessage,src='print_block4_int')
    END DO
    ...
    DO j = 1, jy
      jn = dom_n - j + 1
      WRITE(umMessage,'(''row '',I4,4I12)') jn, (ur(i,j,k), i = 1, ix)
      CALL umPrint(umMessage,src='print_block4_int')
    END DO
    ...
    DO j = 1, jy
      jn = dom_s + jy - j
      WRITE(umMessage,'(''row '',I4,4I12)') jn, (ll(i,j,k), i = 1, ix)
      CALL umPrint(umMessage,src='print_block4_int')
    END DO
    ...
    DO j = 1, jy
      jn = dom_s + jy - j
      WRITE(umMessage,'(''row '',I4,4I12)') jn, (lr(i,j,k), i = 1, ix)
      CALL umPrint(umMessage,src='print_block4_int')
    END DO
...
RETURN
END SUBROUTINE print_block4_int

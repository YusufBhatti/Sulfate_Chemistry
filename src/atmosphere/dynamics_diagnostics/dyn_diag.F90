...
PRIVATE
PUBLIC Dyn_diag
CHARACTER(LEN=*), PRIVATE, PARAMETER :: ModuleName = 'DYN_DIAG_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY: kappa, p_zero, recip_kappa,       &
                                planet_radius
...
USE UM_ParVars
USE UM_ParParams, ONLY: halo_type_single
...
USE latlon_eq_rotation_mod, ONLY: vector_eq_to_latlon
USE rot_coeff_mod, ONLY: rot_coeff1, rot_coeff2
                   ! for LAM wind rotations (on B grid)
USE vc_to_vb_mod, ONLY: vc_to_vb
! The following are for PWS diagnostics which depend on dynamics diagnostics
USE pws_diags_mod, ONLY: pws_wind_ub_200, pws_wind_vb_200, pws_wind_ub_250, &
                         pws_wind_ub_300, pws_wind_vb_250, &
                         pws_wind_vb_300, flag_cat_turb
...
,qtrue_density
                         ! flag for true unscaled density
...
LOGICAL :: v_valid  ! Is v diagnostic calculation possible?
...
LOGICAL :: l_mdi = .TRUE.  ! T: check for MDI
...
REAL, ALLOCATABLE :: work_u_3d(:,:,:)   ! 3d u workspace
REAL, ALLOCATABLE :: work_v_3d(:,:,:)   ! 3d v workspace
...
v_valid = .TRUE.  ! Are v diagnostics valid?
...
IF (qucomB_m) THEN
  ...
  ALLOCATE(work_u_3d(udims%i_start:udims%i_end,                   &
                     udims%j_start:udims%j_end, ucomB_m_levs) )
  DO  k=1,ucomB_m_levs
    ...
    DO j=udims%j_start, udims%j_end
      DO i=udims%i_start, udims%i_end
        work_u_3d(i,j,k) = u(i,j,kk)
      END DO ! i
    END DO ! j
  END DO  ! k model levels loop

  CALL  uC_to_uB(work_u_3d,                                             &
                row_length,rows,n_rows,ucomB_m_levs,offx,offy,          &
                ucomB_m)

  DEALLOCATE(work_u_3d)

END IF ! on STASHflag

IF (qvcomB_m) THEN

  IF (.NOT. qucomB_m) THEN
    ! In the endgame case, v on B grid model levels is
  ...
  ELSE
    ...
    ALLOCATE(work_v_3d(vdims%i_start:vdims%i_end,                     &
                    vdims%j_start:vdims%j_end, vcomB_m_levs) )
    DO  k=1,vcomB_m_levs
      ...
      IF (ucomB_model(k) /= vcomB_model(k)) THEN
        ErrorStatus = -1        ! Warning
        Cmessage='u and v on B grid model levels - '   &
      ...
      DO j=vdims%j_start, vdims%j_end
        DO i=vdims%i_start, vdims%i_end
          work_v_3d(i,j,k) = v(i,j,kk)
        END DO ! i
      END DO ! j
    END DO  ! k model levels loop


    CALL  vC_to_vB(work_v_3d,                                         &
                   rows,row_length,n_rows,vcomB_m_levs,offx,offy,     &
                   global_row_length,                                 &
                   vcomB_m,                                           &
                   ucomB_m  )


    DEALLOCATE(work_v_3d)
  END IF  !.NOT. qucomB_m
...
IF (qucomB_p) THEN
  ...
  ALLOCATE(work_u_3d(udims%i_start:udims%i_end,                   &
                     udims%j_start:udims%j_end, ucomB_p_levs) )
  ...
  CALL  uC_to_uB(work_u_3d,                                       &
                 row_length,rows,n_rows,ucomB_p_levs,offx,offy,   &
                 ucomB_p)

  DO  k=1,ucomB_p_levs
    IF (flag_cat_turb) THEN
      IF (NINT(ucomB_press(k)) == 200) THEN
        DO j = vdims%j_start,vdims%j_end
          DO i = udims%i_start,udims%i_end
            pws_wind_ub_200(i,j) = ucomB_p(i,j,k)
          END DO
        END DO
      ELSE IF (NINT(ucomB_press(k)) == 250) THEN
        DO j = vdims%j_start,vdims%j_end
          DO i = udims%i_start,udims%i_end
            pws_wind_ub_250(i,j) = ucomB_p(i,j,k)
          END DO
        END DO
      ELSE IF (NINT(ucomB_press(k)) == 300) THEN
        DO j = vdims%j_start,vdims%j_end
          DO i = udims%i_start,udims%i_end
            pws_wind_ub_300(i,j) = ucomB_p(i,j,k)
          END DO
        END DO
      END IF
    END IF
...
IF (qvcomB_p) THEN
  ...
  IF (.NOT. qucomB_p) THEN
    ! In the endgame case, v on B grid pressure levels is
  ...
  ELSE
    ...
    DO  k=1,vcomB_p_levs
      ...
      IF (ucomB_press(k) /= vcomB_press(k)) THEN
        ...
        ErrorStatus = -1        ! Warning
        Cmessage='wind diagnostics cannot be generated: u and v '   &
    ...
    ! Perform simple horizontal interpolation from 'C' to 'B' grid
    CALL  vC_to_vB(work_v_3d,                                        &
                   rows,row_length,n_rows,vcomB_p_levs,offx,offy,    &
                   global_row_length,                                &
                   vcomB_p,                                          &
                   ucomB_p )

    IF ( v_valid ) THEN !  V diagnostics possible
      DO  k=1,vcomB_p_levs
        IF (flag_cat_turb) THEN
          IF (NINT(vcomB_press(k)) == 300) THEN
            DO j=vdims%j_start,vdims%j_end
              DO i=udims%i_start,udims%i_end
                pws_wind_vb_300(i,j) = vcomB_p(i,j,k)
              END DO
            END DO
          ELSE IF (NINT(vcomB_press(k)) == 250) THEN
            DO j=vdims%j_start,vdims%j_end
              DO i=udims%i_start,udims%i_end
                pws_wind_vb_250(i,j) = vcomB_p(i,j,k)
              END DO
            END DO
          ELSE IF (NINT(vcomB_press(k)) == 200) THEN
            DO j=vdims%j_start,vdims%j_end
              DO i=udims%i_start,udims%i_end
                pws_wind_vb_200(i,j) = vcomB_p(i,j,k)
              END DO
            END DO
          END IF
        END IF
...
! Rotate winds from model to standard lat-long grid
        IF (lam .AND. rot_uvcomB_p) THEN
          ...
          IF (qucomB_p .AND. ucomB_press(k) == vcomB_press(k)) THEN
            ...
            ! Safe for Cartesian grids with the appropriate rotation
            ! coefficients.
            CALL vector_eq_to_latlon(rot_coeff1,rot_coeff2,work_1,work_2, &
             ucomB_p(udims%i_start,vdims%j_start,k),                      &
             vcomB_p(udims%i_start,vdims%j_start,k),v_field_size,l_mdi)
...
IF (qv50mB_h .OR. qv50m_h) THEN
  ...
  IF (qv50mB_h) THEN
    ...
    IF (global .AND. .NOT. qu50mB_h) THEN
      ! In the endgame case, v on B grid 50m is
    ...
    DO j=1, rows
      DO i=1, row_length
        ...
        height_theta_ml(i,j,k)                                &
                    = r_theta_levels(i,j,kk) - planet_radius
      END DO
    END DO
  END DO
END IF
  ...
  DO k=1, Hrho_m_levs
    DO j=1, rows
      DO i=1, row_length
        kk = Hrho_model(k)
        height_rho_ml(i,j,k)                                  &
             = r_rho_levels(i,j,kk) - planet_radius
      END DO
    END DO
  END DO
END IF
...
IF ( qpress_H ) THEN
  ...
  DO k=1, p_h_levs
    ...
    desired_r = p_height(k) + planet_radius
...
IF ( qtheta_H ) THEN
  DO k=1,theta_h_levs
    desired_r = theta_height(k) + planet_radius
...
IF ( qrho_H ) THEN
  ...
  DO k=1,rho_h_levs
    ...
    desired_r = rho_height(k) + planet_radius
...
IF ( qw_H ) THEN
  DO k=1, w_h_levs
    desired_r = w_height(k) + planet_radius
...
IF ( qu_H ) THEN
  DO k=1, u_h_levs
    desired_r = u_height(k) + planet_radius
...
IF ( qv_H ) THEN
  DO k=1, v_h_levs
    desired_r = v_height(k) + planet_radius
...
IF (spec_w) THEN
  IF (model_type  ==  mt_bi_cyclic_lam) THEN
    ...
    END DO
  ELSE
    WRITE(umMessage,'(A)')'The spectra is not set up for this domain'
    CALL umPrint(umMessage,src='dyn_diag')
  END IF
END IF

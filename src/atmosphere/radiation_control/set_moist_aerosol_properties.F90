...
!
!- ---------------------------------------------------------------------
MODULE set_moist_aerosol_properties_mod

IMPLICIT NONE

CHARACTER(LEN=*),                                                       &
    PARAMETER, PRIVATE :: ModuleName = 'SET_MOIST_AEROSOL_PROPERTIES_MOD'
CONTAINS

SUBROUTINE set_moist_aerosol_properties(ierr                            &
    , n_profile, n_layer                                                &
    , n_aerosol, i_aerosol_parametrization, nhumidity                   &
    , l_mr_physics, water_mix_ratio, t, p, w_cloud, delta_humidity      &
...
!       Pointers to look-up tables
  LOGICAL, INTENT(IN) :: l_mr_physics
!       Mixing ratio with respect to dry air
...
! Calculate the saturated mixing ratio.
  ...
  IF (l_new_qsat_chem_aero) THEN
    IF (l_mr_physics) THEN
      DO i=1, n_layer
        CALL qsat_wat_mix_new(mix_ratio_sat(1:n_profile,i),t(1:n_profile,i),  &
                              p(1:n_profile,i),n_profile)
      END DO
    ELSE
      DO i=1, n_layer
        CALL qsat_wat_new(mix_ratio_sat(1:n_profile,i),t(1:n_profile,i),      &
                          p(1:n_profile,i), n_profile)
      END DO
    END IF
  ELSE
    IF (l_mr_physics) THEN
      DO i=1, n_layer
        ...
        CALL qsat_wat_mix(mix_ratio_sat(1, i), t(1, i), p(1, i),          &
          n_profile, l_mr_physics)
      END DO
    ELSE
      DO i=1, n_layer
        ! DEPENDS ON: qsat_wat
        CALL qsat_wat(mix_ratio_sat(1, i), t(1, i), p(1, i), n_profile)
      END DO
    END IF
  END IF
...
! Compute the clear-sky mean relative humidity
  IF (l_mr_physics) THEN
    DO i = 1, n_layer
      DO l = 1, n_profile
        mean_rel_humidity(l,i) = mix_ratio_clr(l, i)/mix_ratio_sat(l, i)
      END DO
    END DO
  ELSE
    DO i = 1, n_layer
...
END MODULE set_moist_aerosol_properties_mod

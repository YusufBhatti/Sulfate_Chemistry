...
REAL :: sigma_ccw(ilg,lay) ! Normalized cloud condensate std. dev.
...
!$OMP  PARALLEL DO DEFAULT(NONE) PRIVATE (il,k) SCHEDULE(STATIC)  &
!$OMP& SHARED(k_top,k_base,il1,il2,avg_cf,rlc_cf,alpha,zf,        &
!$OMP&    rcorr,rlc_cw)
...
ELSE IF (ioverlap == 3) THEN
  DO i=1,nsubcol
    ...
    ! The first index of the nested loops is "il" so that it can be
    ! parallelised using OpenMP. 
!$OMP  PARALLEL DO DEFAULT(NONE) PRIVATE(il, k, ind1, ind2, rind1,      &
!$OMP& rind2, temp, zcw) SCHEDULE(STATIC)                               &
!$OMP& SHARED(il1,il2,int_x,int_y,rand_seed_x,rand_seed_y,k_top,k_base, &
!$OMP&    avg_cf,i,rm,x1,y1,alpha,x,rcorr,y,ipph,c_cloud,n1,sigma_ccw,  &
!$OMP&    n2,xcw,c_ratio,sigma_qcw,ls_ratio,l_cld,clw_sub_full,i_loc)
    DO il = il1, il2

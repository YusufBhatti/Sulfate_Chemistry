...
!-----------------------------------------------------------------------
MODULE compute_aod_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'COMPUTE_AOD_MOD'
CONTAINS
...
!  computed absorption aerosol optical depth
REAL :: aaod(npd_profile, npd_aod_wavel)
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='COMPUTE_AOD'
  ...
  DO jj = 1, n_aer_in_type
    ...
    IF (i_aerosol_parametrization(j)  ==                           &
      ...
      DO k = 1, n_aod_wavel
        ...
        absorption = aod_absorption(1, j, k)
        DO i = 1, n_layer
          DO l = 1, n_profile
            aod(l, k) = aod(l, k) +                               &

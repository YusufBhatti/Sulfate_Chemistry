...
SUBROUTINE solinc(row_length, rows, cos_zenith_angle, sol_azimuth,             &
  cosz_beg, cosz_end)

USE conversions_mod, ONLY: pi
USE solinc_data, ONLY:                                                         &
  orog_corr, slope_aspect, slope_angle,                                        &
...
IF (l_skyview) THEN
  ...
  rel_horiz_aspect = MODULO( horiz_aspect - &
    SPREAD(sol_azimuth,3,n_horiz_ang), pi*2.0)
  min_sub=MINLOC(rel_horiz_aspect, dim=3)
  max_sub=MAXLOC(rel_horiz_aspect, dim=3)
  DO j=1,rows
    DO i=1,row_length
      cosz_hor(i,j) = COS( (horiz_ang(i,j,1,min_sub(i,j))                      &
        *(pi*2.0 - rel_horiz_aspect(i,j,max_sub(i,j)))                         &
        +horiz_ang(i,j,1,max_sub(i,j))*rel_horiz_aspect(i,j,min_sub(i,j)))     &
        /(pi*2.0 - rel_horiz_aspect(i,j,max_sub(i,j))                          &
  ...
  WHERE (cos_zenith_angle /= 0.0)
    ...
    ELSEWHERE (cosz_hor > cosz_beg .AND. cosz_hor < cosz_end)
      ...
      orog_corr = lit_frac*(cosz_lit+SIN(ACOS(cosz_lit))*TAN(slope_angle)*     &
        COS(sol_azimuth-slope_aspect))/cos_zenith_angle
      orog_corr = MAX(orog_corr,EPSILON(orog_corr))
    ELSEWHERE (cosz_hor < cosz_beg .AND. cosz_hor > cosz_end)

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'SKYVIEW_MOD'
CONTAINS
...
USE solinc_data, ONLY:                                                &
  ...
  sky, n_horiz_layer, n_horiz_ang
USE conversions_mod, ONLY: pi
USE horizon_1d_mod, ONLY: horizon_1d
IMPLICIT NONE
...
ALLOCATE(horiz_ang(row_length,rows,n_horiz_layer,n_horiz_ang))
...
horiz_aspect_rot(:,:,ip_NW) = pi*2.0-delta_ang
...
horiz_aspect_rot(:,:,ip_NNW) = pi*2.0-delta_ang
...
horiz_aspect_rot(:,:,ip_WNW) = pi*2.0-delta_ang
...
DO k=1,n_horiz_ang
  DO j=1,rows
    DO i=1,row_length
      horiz_aspect(i,j,k) =                                           &
        MODULO(horiz_aspect_rot(i,j,k) + bear_rot_NP(i,j), pi*2.0)
    END DO
  END DO
END DO
...
DO j=1,rows
  ...
  horiz_ang(:,j,1,ip_W) = horiz_ang1(1:row_length)
  horiz_ang(:,j,1,ip_E) = horiz_ang2(1:row_length)
END DO
...
DO i=1,row_length
  ...
  horiz_ang(i,:,1,ip_S) = horiz_ang1(1:rows)
  horiz_ang(i,:,1,ip_N) = horiz_ang2(1:rows)
END DO
...
DO k=1,n_horiz_ang
  DO j=1,rows
    DO i=1,row_length
      ...
      horiz_ang(i,j,1,k) = MIN( horiz_ang(i,j,1,k), tilt_ang(i,j,k) )
    END DO
  END DO
END DO
...
DO j=1,rows
  DO i=1,row_length
    sky(i,j)=SUM(SIN(horiz_ang(i,j,1,:)+pi_over_2-tilt_ang(i,j,:))**2)  &
      /(REAL(n_horiz_ang)*COS(slope_angle(i,j)))
  END DO
END DO

END SUBROUTINE skyview
END MODULE skyview_mod

...
MODULE horizon_1d_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'HORIZON_1D_MOD'
CONTAINS
  ...
  DO
    ...
    IF ( orog(i-k) > orog(horiz_point(i)) ) THEN
      ...
      h_ang = ATAN2( (planet_radius+orog(i-k))*SIN(ang),              &
        orog(i-k)*COS(ang) - orog(i) - planet_radius*(1.0-COS(ang)) )

...
!- ---------------------------------------------------------------------
MODULE r2_re_mrf_umist_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'R2_RE_MRF_UMIST_MOD'
CONTAINS
...
USE rad_pcf
USE rad_input_mod, ONLY: l_use_ndrop, lrad_ccrad, l_use_liu_spec, &
                         aparam, bparam
...
USE r2_column_droplet_conc_mod, ONLY: r2_column_droplet_conc
...
REAL ::                                                           &
          !, INTENT(IN)
     condensed_mix_ratio(npd_profile, id_ct:npd_layer             &
        , npd_cloud_component)                                    &
...
REAL ::                                                           &
          !, INTENT(OUT)
     condensed_re(npd_profile,id_ct:npd_layer,npd_cloud_component)
!             EFFECTIVE RADII OF CONDENSED COMPONENTS OF CLOUDS

REAL ::                                                           &
          !, INTENT(OUT)
     CDNC(npd_profile,id_ct:npd_layer,npd_cloud_component)
!             CDNC array for subsequent calculation of CDNC
!             at cloud top
...
!             NUMBER DENSITY OF DROPLETS
         , kparam_st, kparam_cnv, kparam_arr(npd_profile)         &
!             RATIO OF CUBES OF VOLUME RADIUS TO EFFECTIVE RADIUS
         , beta, beta_cnv(npd_profile), beta_st(npd_profile)      &
...
! Limits for the beta calculation
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='R2_RE_MRF_UMIST'
    ...
    DO l=1, n_profile
      ...
      so4_ccn_diag_g(l, i)=                                    &
         (aitken_sulphate(sulphate_ptr_a, sulphate_ptr_b)      &
          +accum_sulphate(sulphate_ptr_a, sulphate_ptr_b)      &
           +diss_sulphate(sulphate_ptr_a, sulphate_ptr_b))     &
           * density_air(l, i) * (96.0/132.0) * 1.0e+09

      IF (l_seasalt_ccn) THEN
        seasalt_ptr_a=col_list(l)
        seasalt_ptr_b=row_list(l)
        seasalt_ptr_c=n_layer+1-i
        sea_salt_film_g(l,1,i) =                                   &
                sea_salt_film(seasalt_ptr_a, seasalt_ptr_b, seasalt_ptr_c)
        sea_salt_jet_g(l,1,i) =                                    &
                sea_salt_jet(seasalt_ptr_a, seasalt_ptr_b, seasalt_ptr_c) 
      END IF
...
ELSE IF (l_easyaerosol_cdnc) THEN

  DO i=n_layer+1-nclds, n_layer
    DO l=1, n_profile
      n_drop(l, i) = easyaerosol_cdnc%cdnc(col_list(l), row_list(l), &
                                           n_layer+1-i)
    END DO
  END DO
...
DO i=n_layer+1-nclds, n_layer
   DO l=1, n_profile
      CDNC(l, i, ip_clcmp_cnv_water)=n_drop(l, i)   
      CDNC(l, i, ip_clcmp_st_water)=n_drop(l, i)   
   END DO
END DO
  ...
  IF (l_use_liu_spec) THEN
    ...
    DO i=n_layer+1-nclds, n_layer
      ...
      DO l=1, n_profile
        ...
        beta = aparam                                               & 
          *((MAX(eps,total_mix_ratio_st(l))*density_air(l,i)*1.0e-3 &
          /(n_drop(l, i)*1e-06))**(bparam))
        ...
        condensed_re(l, i, ip_clcmp_st_water)  = MAX(0.0e+00,       &
          ...
          /(4.0e+00*pi*rho_water*(beta**(-3))*n_drop(l, i)))        &
          **(1.0e+00/3.0e+00)
...
ELSE
  ...
  DO i=n_layer+1-nclds, n_layer
    ...
    ! Calculate spectral dispersion term
    IF (l_use_liu_spec) THEN
      DO l = 1, n_profile
        beta_cnv(l) = aparam                                         & 
          *((MAX(eps,total_mix_ratio_cnv(l))*density_air(l,i)*1.0e-3 &
          /(n_drop(l, i)*1e-06))**(bparam))
    ...
    IF (i_cld_vn == i_cld_pc2) THEN
      DO l=1, n_profile
        IF (l_use_liu_spec) THEN
          kparam_st  = beta_st(l)**(-3)
          kparam_cnv = beta_cnv(l)**(-3)
        ELSE IF (land_g(l)) THEN
          kparam_st  = kparam_land
          kparam_cnv = kparam_land
        ELSE
          kparam_st  = kparam_sea
          kparam_cnv = kparam_sea
        END IF
        temp                                                      &
         =(3.0e+00*total_mix_ratio_cnv(l)*density_air(l, i)       &
         /(4.0e+00*pi*rho_water*kparam_cnv*n_drop(l, i)))
        IF (temp  >=  0.0) THEN
        ...
        temp                                                      &
         =(3.0e+00*total_mix_ratio_st(l)*density_air(l, i)        &
         /(4.0e+00*pi*rho_water*kparam_st*n_drop(l, i)))
        IF (temp  >=  0.0) THEN
...
END MODULE r2_re_mrf_umist_mod

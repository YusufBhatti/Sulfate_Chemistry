...
!    then written to a temporary file
!
!    On each timestep the observations are read in from the temporary 
!    file and assimilated. This is done until the AC Observation files
...
!    then be written to the temporary file
!
...
! separate from REALs and that the obs_info_int_len and obs_info_real_len
...
INTEGER, PARAMETER :: max_num_acob_files = 20 ! was 100 
INTEGER, PARAMETER :: num_ob_file_types  = 10 

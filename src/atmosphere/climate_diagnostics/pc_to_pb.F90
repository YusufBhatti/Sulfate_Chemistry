...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='PC_TO_PB_MOD'

CONTAINS

SUBROUTINE pc_to_pb(pc,row_length,rows,n_rows,levels,offx,offy,pb)
...
REAL, INTENT(IN) ::  pc(row_length,rows,levels) ! Field on p points on C grid

REAL, INTENT(OUT) :: pb(udims%i_start:udims%i_end,                &
                        vdims%j_start:vdims%j_end,                &
                        levels)                ! Field on uv points on B grid

! Local parameters:
CHARACTER(LEN=*), PARAMETER :: RoutineName='PC_TO_PB'
...
REAL :: pc_halo(1-offx:row_length+offx,1-offy:rows+offy,levels)

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='PHY_DIAG_MOD'
...
! Flags to request each diagnostic output field: in
      ...
      ,qhts_rho                                                         &
      ,qRHwat_p                                                         &
      ,qPWS_Thickness500,qPWS_Thickness850,qPWS_Snow_Prob               &
...
USE atm_fields_bounds_mod, ONLY:                                  &
    ...
    tdims_s, wdims_l, tdims_l, pdims_l

USE level_heights_mod, ONLY:                                      &
          r_theta_levels, r_rho_levels

USE planet_constants_mod, ONLY: planet_radius, kappa, p_zero
...
! The following are for PWS diagnostics which depend on physics diagnostics
USE pws_diags_mod, ONLY: pws_geopht_1000, pws_geopht_850, pws_geopht_500, &
                         pws_snow_prob
...
,qhts_rho                                                         &  
             ! Flag for geopotential heights   on rho levels
,qPWS_Thickness500                                                & 
             ! Flag for PWS section Thickness 1000-500mb
,qPWS_Thickness850                                                &
             ! Flag for PWS section Thickness 1000-850mb
,qPWS_Snow_Prob
             ! Flag for PWS section Snow Probability
...
,qc(row_length,rows,model_levels)                                 &
                             ! cloud water content on q levels
,qT(row_length,rows,model_levels)                                 &
...
IF (qhts_p .OR. qhts_rho) THEN
  ...
  DO k = 1, model_levels
    DO j = 1, rows
      DO i = 1, row_length
        height(i,j,k) = r_rho_levels(i,j,k) - planet_radius
      END DO ! i
    END DO ! j
  END DO ! k
...
IF (qhts_theta) THEN
  ...
  DO k = 1, model_levels
    DO j = 1, rows
      DO i = 1, row_length
        hts_theta(i,j,k) = r_theta_levels(i,j,k) - planet_radius
      END DO ! i
    END DO ! j
  END DO ! k
END IF ! on relevant STASHflags
...
IF (qhts_p) THEN
  DO k = 1, hts_p_levs
    pressure_pa = hts_p_press(k)*100.0 ! convert to Pascals
    CALL vert_h_onto_p(                                            &
...
! Store standard geopotential hts, 
! if PWS thickness or snow probability diagnostics requested
    IF (hts_p_press(k) == 1000.0) THEN
      IF (qPWS_Thickness500 .OR. qPWS_Thickness850 .OR.  &
          qPWS_Snow_Prob) THEN
        pws_geopht_1000(:,:) = hts_p(:,:,k) 
      END IF
    END IF
    IF (hts_p_press(k) == 850.0) THEN
      IF (qPWS_Thickness850 .OR. qPWS_Snow_Prob) THEN
        pws_geopht_850(:,:) = hts_p(:,:,k)
      END IF
    END IF
    IF (hts_p_press(k) == 500.0) THEN
      IF (qPWS_Thickness500) THEN
        pws_geopht_500(:,:) = hts_p(:,:,k)
      END IF
    END IF
...
IF (qt_p) THEN
  DO k = 1, t_p_levs
    pressure_pa = t_p_press(k)*100.0 ! convert to Pascals
...
IF (qRHice_p) THEN
  ...
  DO k = 1, model_levels
    ...
    CALL qsat(rh(:,:,k),t(:,:,k),p_at_theta(:,:,k),               &
              row_length,rows)
    !  And convert to relative humidity
    DO j = 1, rows
      DO i = 1, row_length
        rh(i,j,k) = q(i,j,k)/rh(i,j,k)*100.0
...
IF (qRHwat_p) THEN
  ...
  DO k = 1, model_levels
    !  Find humidity saturation at theta points - store in rh
    CALL qsat_wat(rh(:,:,k),t(:,:,k),p_at_theta(:,:,k),           &
              row_length,rows)
    !  And convert to relative humidity
    DO j = 1, rows
      DO i = 1, row_length
...
IF (qwbpt_p) THEN
  ...
  DO k = 1, wbpt_p_levs
    ...
    pressure_pa = wbpt_p_press(k)*100.0 ! convert to Pascals
...
IF (qp_MSL) THEN
  ...
  CALL Calc_PMSL(theta, exner_theta_levels, p                    &
                ...
                ,gc_all_proc_group, delta_lambda, delta_phi      &

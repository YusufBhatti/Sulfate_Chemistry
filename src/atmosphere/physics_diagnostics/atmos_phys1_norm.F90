...
MODULE atmos_phys1_norm_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='ATMOS_PHYS1_NORM_MOD'

CONTAINS
SUBROUTINE atmos_phys1_norm(                                            &
...
USE atm_fields_bounds_mod,  ONLY: tdims, tdims_s, udims_s, vdims_s
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ATMOS_PHYS1_NORM'
...
REAL, INTENT (IN) :: cff_star                                  &
                           (tdims%i_start:tdims%i_end,         &
                            tdims%j_start:tdims%j_end,         &
                            tdims%k_start:tdims%k_end)
REAL, INTENT (IN) :: u_inc                                     &
                        (udims_s%i_start:udims_s%i_end,        &
                         udims_s%j_start:udims_s%j_end,        &
                         udims_s%k_start:udims_s%k_end)
REAL, INTENT (IN) :: v_inc                                     &
...
! Radiation fields 
REAL, INTENT(IN) ::  dOLR(row_length, rows)  ! TOA - surface upward LW
...
IF ( level_start == level_end ) THEN
  ...
  DO k =  start_level, end_level
    ...
    num = num + 1
    CALL array_norm(                                                 &
                    u_inc, row_length, rows,                         &
                    model_levels, k, k,                              &
                    offx, offy, 0, 0, L_do_halos, L_do_rims,         &
                    rim_n, rim_s, rim_e, rim_w, two_norm(num))
    num = num + 1
    CALL array_norm(                                                 &
                    v_inc, row_length, n_rows,                       &
                    model_levels, k, k,                              &
                    offx, offy, 0, 0, L_do_halos, L_do_rims,         &
                    rim_n, rim_s, rim_e, rim_w, two_norm(num))
    ...
    IF ( L_print_pe .OR. mype == 0 ) THEN
      ...
      CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
      num = num + 1
      WRITE(umMessage, '(A, I5, A, E23.16)')                         &
      ...
      num = num + 1
      WRITE(umMessage, '(A, I5, A, E23.16)')                         &
              ' Level ', k, ' u_inc Two_Norm =' , two_norm(num)
      CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
      num = num + 1
      WRITE(umMessage, '(A, I5, A, E23.16)')                         &
              ' Level ', k, ' v_inc Two_Norm =' , two_norm(num)
      CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
...
ELSE   !  3d norms from level_start to level_end
  ...
  num = num + 1
  CALL array_norm(                                               &
                  ls_rain, row_length, rows, 1, 1, 1,            &
                  0, 0, 0, 0, .FALSE., L_do_rims,                &
                  rim_n, rim_s, rim_e, rim_w, two_norm(num) )
  num = num + 1
  CALL array_norm(                                               &
                  ls_snow, row_length, rows, 1, 1, 1,            &
                  0, 0, 0, 0, .FALSE., L_do_rims,                &
                  rim_n, rim_s, rim_e, rim_w, two_norm(num) )
  num = num + 1
  CALL array_norm(                                               &
                  dOLR, row_length, rows, 1, 1, 1,               &
                  0, 0, 0, 0, .FALSE., L_do_rims,                &
                  rim_n, rim_s, rim_e, rim_w, two_norm(num) )
  ...
  IF ( L_print_pe .OR. mype == 0 ) THEN
    ...
    WRITE(umMessage, '(A, I5, A, I5, A, E23.16)')                &
                  ' Levels', start_level, ' to', level_end,      &
                  ' u_inc Two_Norm =' , two_norm(num)
    CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
    num = num + 1
    WRITE(umMessage, '(A, I5, A, I5, A, E23.16)')                &
                  ' Levels', start_level, ' to', level_end,      &
                  ' v_inc Two_Norm =' , two_norm(num)
    CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
    num = num + 1
    WRITE(umMessage, '(A, E23.16)')                              &
                     ' ls_rain Two_Norm =' , two_norm(num)
    CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
    num = num + 1
    WRITE(umMessage, '(A, E23.16)')                              &
                     ' ls_snow Two_Norm =' , two_norm(num)
    CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
    num = num + 1
    WRITE(umMessage, '(A, E23.16)')                              &
                     ' dOLR Two_Norm =' , two_norm(num)
    CALL umPrint(umMessage, src='ATMOS_PHYS1_NORM')
...
END SUBROUTINE atmos_phys1_norm
END MODULE atmos_phys1_norm_mod

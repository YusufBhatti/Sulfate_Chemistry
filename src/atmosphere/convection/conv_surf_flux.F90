  ...
  IF (tv1_sd_opt == 2) THEN
    ...
    IF ( l_new_qsat_conv ) THEN
      IF ( l_mr_physics ) THEN
        CALL qsat_mix_new(qs_star,tstar_sea,pstar,row_length,rows)
        CALL qsat_mix_new(qs_star_sice,tstar_sice,pstar,row_length,rows)
      ELSE
        CALL qsat_new(qs_star,tstar_sea,pstar,row_length,rows)
        CALL qsat_new(qs_star_sice,tstar_sice,pstar,row_length,rows)
      END IF
    ELSE
      ! DEPENDS ON: qsat_mix
...
!$OMP END PARALLEL
  ...
  ELSE  ! tv1_sd_opt /= 2
    ...
    IF ( l_new_qsat_conv ) THEN
      IF ( l_mr_physics ) THEN
        CALL qsat_mix_new(qs_star,tstar,pstar,row_length,rows)
      ELSE
        CALL qsat_new(qs_star,tstar,pstar,row_length,rows)
      END IF
    ELSE
      ! DEPENDS ON: qsat_mix
      CALL qsat_mix(qs_star,tstar,pstar,row_length*rows,l_mr_physics)
    END IF

...
USE planet_constants_mod, ONLY:                                         &
    cp, r, c_virtual, g, planet_radius
...
REAL, INTENT(IN) :: conv_prog_precip( tdims_s%i_start:tdims_s%i_end,     &
...
REAL :: column_rh(nunstable)          ! Column integrated relative humidity
REAL :: column_rh_bl(nunstable)       ! Column integrated relative humidity in BL
REAL :: column_q(nunstable)           ! Column integrated q
REAL :: qsat_c(nunstable, model_levels) ! Qsat
...
REAL :: zcld_thresh   ! threshold depth for diagnosis
...
REAL ::TmpScm3d(ScmRowLen,ScmRow,model_levels) ! Full   level field

!
! Model constants:
!

REAL :: zcld                  ! Depth of cloud layer
...
ELSE IF (icvdiag >= 2 .AND. icvdiag < 15 ) THEN
  ...
  ELSE IF (icvdiag == 6) THEN
    ...
    DO k= 1,model_levels-1
      DO ii=1, nunstable
        i = index_i(ii)   
        j = index_j(ii)   
        IF (conv_prog_precip(i,j,k) > 1.0e-10) THEN
  ...
  ELSE IF (icvdiag == 10 ) THEN ! Ben's diagnosis
    ...
    IF ( l_scmdiags(scmdiag_conv) .AND.                                     &
         model_type == mt_single_column ) THEN

      DO ii=1,nunstable
        i=index_i(ii)
        j=index_j(ii)
        TmpScm2d(i,j) = column_rh(ii)
      END DO
      CALL scmoutput(TmpScm2d,'column_rh',                                  &
           'Column Integrated RH','-',                                      &
      ...
      DO ii=1,nunstable
        i=index_i(ii)
        j=index_j(ii)
        TmpScm2d(i,j) = column_rh_bl(ii)
      END DO
      CALL scmoutput(TmpScm2d,'column_rh_bl',                               &
           'Column Integrated RH in BL','-',                                &
           t_inst,d_point,default_streams,'',routinename)
      ...
      DO ii=1,nunstable
        i=index_i(ii)
        j=index_j(ii)
        TmpScm2d(i,j) = column_q(ii)
      END DO
      CALL scmoutput(TmpScm2d,'column_q',                                   &
           'Column Integrated q','-',                                       &
           t_inst,d_point,default_streams,'',routinename)
      ...
      DO k= 1,model_levels
        DO ii=1,nunstable
          i=index_i(ii)
          j=index_j(ii)
          TmpScm3d(i,j,k) = qsat_c(ii,k)
        END DO
      END DO
      CALL scmoutput(TmpScm3d,'qsat_c',                                     &
           'qsaturation on convective points','-',                          &
           t_inst,d_wet,default_streams,'',routinename)

    END IF ! scmdiag_conv / model_type


  ELSE

    error = 3     ! fatal return code
      ...
      IF ( cumulus_c(ii) ) THEN
        ...
        IF ( shmin(ii) .AND. zcld < zcld_thresh ) THEN
          ...
          IF ( (z_full_c(ii,kshmin(ii)) - z_full_c(ii,ntpar_l))          &
            ...
            ELSE IF (zcld <=  2000.0 .AND. zcld >  500.0) THEN

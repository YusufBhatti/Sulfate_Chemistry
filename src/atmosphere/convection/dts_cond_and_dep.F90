...
MODULE dts_cond_and_dep_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_COND_AND_DEP_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_COND_AND_DEP'
...
DO k=2,ntparmax
  DO i_dp=1,n_dp
    ...
    IF (k <= dts_ntpar(i_dp) .AND. z_rho(i_dp,k) >= zlcl(i_dp)          &
      ...
      ! the lcl
      IF (qsat_moist_ad(i_dp,k) > 0.0) THEN
...
END MODULE dts_cond_and_dep_mod

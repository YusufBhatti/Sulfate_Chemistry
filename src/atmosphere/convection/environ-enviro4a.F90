...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'ENVIRON_MOD'
CONTAINS
...
LOGICAL, INTENT(IN) ::  &
 ...
 ,l_mom_gk_stable       & ! Switch for stabilized Gregory Kershaw scheme
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ENVIRON'
...
REAL,ALLOCATABLE :: up_ref(:) ! Par. U to be referred for momentum flux
                              ! at k+1/2
REAL,ALLOCATABLE :: vp_ref(:) ! Par. V to be referred for momentum flux
                              ! at k+1/2
...
REAL, PARAMETER ::        &
  ...
  wcgrow = 5.0e-4            ! Water content of new detrained cloud
...
DO i=1,npnts
  ...
  ! Need to initialise as otherwise unset

  dqekp1_nonpc2(i)  = 0.0
  dthekp1_nonpc2(i) = 0.0
  ...
  IF (l_calc_dxek) THEN  
    ...
    IF (i_pc2_conv_coupling == pc2_conv_original       .OR.             &
      ...
      IF (ABS(tempdcfxk)  >   tolerance) THEN
        ...
        tempdcfxk = MAX(tempdcfxk, (-1.0 * cflek(i) / timestep))
        tempdcfxk = MIN(tempdcfxk, ((1.0 - cflek(i)) / timestep))
      ELSE
      ...
      IF (ABS(tempdcfxk)  >   tolerance) THEN
        ...
        tempdcfxk = MAX(tempdcfxk, (-1.0 * cffek(i) / timestep))
        tempdcfxk = MIN(tempdcfxk, ((1.0 - cffek(i)) / timestep))
      ELSE
        tempdcfxk = 0.0                        ! Zero rate
      END IF
    ...
    ELSE ! (i_pc2_conv_coupling == pc2_conv_improved .OR.
      ...
      ELSE IF (dbydt_of_qclek_det < -1.0e-12) THEN
        ...
        tempdcfxkd = dbydt_of_qclek_det * cflek(i) /                    &
                       MAX( wcgrow, qclek(i)/MAX(1.0e-6,cflek(i)) )
      ELSE
      ...
      IF (dbydt_of_qcfek_det > 1.0e-12) THEN
        tempdcfxkd =  dbydt_of_qcfek_det*(1.0 - cffek(i))/wcgrow
      ELSE IF (dbydt_of_qcfek_det < -1.0e-12) THEN
        tempdcfxkd = dbydt_of_qcfek_det * cffek(i) /                    &
                       MAX( wcgrow, qcfek(i)/MAX(1.0e-6,cffek(i)) )
      ELSE
        tempdcfxkd = 0.0
      END IF

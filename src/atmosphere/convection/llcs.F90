...
INTEGER :: a, b, c, dim1, dim2, nlevels
...
IF ( l_new_qsat_conv ) THEN
  CALL qsat_wat_new(q_sat,temperature,p_full(current_level))
ELSE
...
(q_start(current_level)  >=  q_crit)) THEN
  ...
  IF (q_start(current_level)  >=  0.0) THEN
    ...
    DO j = current_level , nlevels
      ...
      IF ( l_new_qsat_conv ) THEN
        CALL qsat_wat_new(q_sat,temperature,p_full(j))
      ELSE
        ! DEPENDS ON: qsat_wat_mix
  ...
  DO j = cloudbase , nlevels
    IF ((theta_adjust(j-1)  >=  theta_adjust(j)) .OR.                   &
      ...
      IF (q_adjust(j-1)  >=  0.0) THEN
        ...
        IF ( l_new_qsat_conv ) THEN
          CALL qsat_wat_new(q_sat,temperature(j),p_full(j))
        ELSE
          ! DEPENDS ON: qsat_wat_mix
        ...
        IF ( l_new_qsat_conv ) THEN
          CALL qsat_wat_mix_new(r_sat,temperature(j),p_full(j))
        ELSE
          ! DEPENDS ON: qsat_wat_mix
        ...
        IF ( l_new_qsat_conv ) THEN
          CALL qsat_wat_mix_new(r_sat,temperature(j-1),p_full(j-1))
        ELSE
          ! DEPENDS ON: qsat_wat_mix

...
MODULE congest_conv_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'CONGEST_CONV_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY:                                   &
    r, cp , pref, kappa, repsilon, c_virtual, g

USE cv_run_mod, ONLY:                                             &
    l_mom, bl_cnv_mix, icvdiag, l_cv_conserve_check, l_new_dd,    &
...
USE cv_stash_flg_mod, ONLY:                                               &
    ...
    flg_detr_up, flg_detr_dwn, flg_uw_shall, flg_vw_shall, flg_w_eqn,     &
...
, rho_theta(n_cg,nlev)     & ! wet density on theta levels (kg/m3)
, rho(n_cg,nlev)           & ! wet density on rho levels (kg/m3)
...
REAL, INTENT(OUT) ::      &
 ...
 ,du_dd(n_cg,nlev)        & ! du/dt from DD (m/s/s)
 ,dv_dd(n_cg,nlev)        & ! dv/dt from DD (m/s/s)
 ,area_ud(n_cg,nlev)      & ! fractional area of updraughts
 ,area_dd(n_cg,nlev)        ! fractional area of downdraughts
...
REAL :: zkm1(n_cg)              ! ... k-1   [m]
REAL :: zk(n_cg)                ! ... k     [m]
REAL :: zkp12(n_cg)             ! ... k+1/2 [m]
REAL :: zkp1(n_cg)              ! ... k+1   [m]
...
LOGICAL :: l_mom_gk_stable      ! true if using stabilized Gregory-Kershaw CMT
...
! New downdraught and evaporation scheme also uses
! npossdd to hold the number of real deep convecting points with precipitation
! index_possdd  the locations of these points  

REAL ::                  &
  precip_rain(n_cg,nlev) & ! Precipitation creation rate for rain (kg/m2/s)
 ,precip_snow(n_cg,nlev)   ! Precipitation creation rate for snow (kg/m2/s)
...
REAL :: w2p_km1_c2 (n_cg)       ! ...layer centre k-1 [(m/s)^2]
REAL :: w2p_k_c2   (n_cg)       ! ...layer centre k   [(m/s)^2]
REAL :: w2p_kp1_c2 (n_cg)       ! ...layer centre k+1 [(m/s)^2]

! Compressed arrays for height above surface of model level ...
REAL :: zkm1_c2    (n_cg)       ! ...k-1   [m]
REAL :: zk_c2      (n_cg)       ! ...k     [m]
REAL :: zkp12_c2   (n_cg)       ! ...k+1/2 [m]
REAL :: zkp1_c2    (n_cg)       ! ...k+1   [m]
...
! This routine is congestus
INTEGER, PARAMETER   ::  congestus = 2 
!===============================================================
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='CONGEST_CONV'
...
IF ( l_new_qsat_conv ) THEN
  CALL qsat_new(qse_lcl,t_lcl,p_lcl,n_cg)
ELSE
  ! DEPENDS ON: qsat_mix
...
,               ekp14,ekp34,amdetk)
  ...
  IF (nterm >  0) THEN
    ...
    !-----------------------------------------------------------------------
    ! 3.5  Original downdraught calculation - set up b_dd & b_nodd
    !-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
! 4.0 All congestus convection will terminate at some level. This level
!     has been stored in the main level loop.
!     The convection will either have a down draught or none will be
!     possible.
!-----------------------------------------------------------------------
!     There are a number of different downdraught and evaporation schemes
!     possible.
!                     (a) Emanuel scheme (Not called from congestus)
...
IF (l_new_dd) THEN
    ...
    IF (npossdd > 0) THEN
      ...
      CALL dd_evap_ctl(n_cg, npossdd, nlev, trlev, ntra, kmax_term,        &
...
ELSE 
  ...
  !-----------------------------------------------------------------------
  ! (b)  Downdraught calculation - on all points where convection is
  !      terminating.
...
DO i = 1,n_cg
  ...
  IF (temp1(i)  <   qminincolumn(i) .AND.                            &
    ...
    WRITE(umMessage,'(a20,i6,a9,g26.18,a7,g26.18)') ' negative q cong, i:',  &
          i,' q after ',temp1(i),' dq/dt ',dqbydt(i,k)
    CALL umPrint(umMessage,src='congest_conv')
  END IF
END DO ! n_cg loop
...
IF (bl_cnv_mix == 1) THEN
  ...
  CALL mix_ipert_4a5a(n_cg, n_cg, nlev, nbl, ntml, index1,        &
               p_layer_boundaries, exner_layer_centres,           &
               dthbydt, dqbydt, flx_init,                         &
               thpert, qpert)
ELSE

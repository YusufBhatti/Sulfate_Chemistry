...
!
MODULE cor_engy_5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'COR_ENGY_5A_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='COR_ENGY_5A'
...
IF (nfalse > 0) THEN
  DO i=1,npnts
    IF (.NOT. bcorr(i) ) THEN
      WRITE(umMessage,'(a21,i6,a8,g26.18,a5,6g26.18)')                         &
          'Conv Energy problem: ',i,                                           &
          ' factor ',scalee(i),' err ',terr(i),tspos(i),tsneg(i),              &
          (lc+lf)*qsum(i),(lf)*qclsum(i),qerror(i)
      CALL umPrint(umMessage,src='cor_engy-5a')
      ! No appropriate format as if there is a problem want to know
      ! variables to machine accuracy & nlev varies
      WRITE(umMessage,*) ' dtheta ',(dthbydt(index1(i),k),k=1,nlev)
      CALL umPrint(umMessage,src='cor_engy-5a')
      WRITE(umMessage,*) ' dq   ',(dqbydt(index1(i),k),k=1,nlev)
      CALL umPrint(umMessage,src='cor_engy-5a')
      WRITE(umMessage,*) ' dqcl ',(dqclbydt(index1(i),k),k=1,nlev)
      CALL umPrint(umMessage,src='cor_engy-5a')
      WRITE(umMessage,*) ' dqcf ',(dqcfbydt(index1(i),k),k=1,nlev)
      CALL umPrint(umMessage,src='cor_engy-5a')
    END IF
  END DO
END IF

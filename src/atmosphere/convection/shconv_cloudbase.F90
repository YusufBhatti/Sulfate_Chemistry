...
MODULE shconv_cloudbase_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'SHCONV_CLOUDBASE_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY: cp, c_virtual, rv, recip_epsilon
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SHCONV_CLOUDBASE'
...
DO i = 1,n_sh
  ...
  thetav_plus(i)  = theta_plus(i) *                               &
                   (1.0+r_plus(i)*recip_epsilon)/(1.0+r_plus(i))
  thetav_minus(i) = theta_minus(i)*                               &
                   (1.0+r_minus(i)*recip_epsilon)/(1.0+r_minus(i))
  ...
  wql_cb(i) = 0.4*wthetav_surf(i)/(lc_o_cp/exner_plus(i) -        &
                        (1.0+c_virtual)*theta_plus(i))


  term_a = -0.7*mb(i)*dthetav_cb(i)

  term_c = wql_cb(i)*(1.0+lc_o_cp*drsatdt_topt)/0.4

  term_d = mb(i)*r_rsat_term

  wqt_cb(i) = (term_c - term_d                                    &
           + exner_plus(i)*drsatdt_topt*term_a )/                 &
     (1.0+c_virtual*exner_plus(i)*theta_minus(i)*drsatdt_topt)
...
END MODULE shconv_cloudbase_mod

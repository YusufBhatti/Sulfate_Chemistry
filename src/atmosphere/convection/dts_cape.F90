...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_CAPE_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY: r, cp, kappa, c_virtual, rv
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_CAPE'
...
IF ( l_new_qsat_conv ) THEN
  IF ( lq_mix ) THEN
    CALL qsat_mix_new(q0,tparcel,p,n_dp)
  ELSE
    CALL qsat_new(q0,tparcel,p,n_dp)
  END IF
ELSE
  ! DEPENDS ON: qsat_mix
...
DO k = klev_min,ktop_max
  ...
  IF (l_itime) THEN
    ...
    IF ( l_new_qsat_conv ) THEN
      IF ( lq_mix ) THEN
        CALL qsat_mix_new(rs1,tparcel,p_layer_centres(:,k-1),n_dp)
      ELSE
        CALL qsat_new(rs1,tparcel,p_layer_centres(:,k-1),n_dp)
      END IF
    ELSE
      ! DEPENDS ON: qsat_mix
  ...
  IF ( l_new_qsat_conv ) THEN
    IF ( lq_mix ) THEN
      CALL qsat_mix_new(qnext,tparcel,p_layer_centres(:,k+1),n_dp)
      CALL qsat_mix_new(q1,tparcel-deltat,p_layer_centres(:,k+1),n_dp)
    ELSE
      CALL qsat_new(qnext,tparcel,p_layer_centres(:,k+1),n_dp)
      CALL qsat_new(q1,tparcel-deltat,p_layer_centres(:,k+1),n_dp)
    END IF
  ELSE
    ! DEPENDS ON: qsat_mix
  ...
  IF ( l_new_qsat_conv ) THEN
    IF ( lq_mix ) THEN
      CALL qsat_mix_new(rs2,tparcel,p,n_dp)
    ELSE
      CALL qsat_new(rs2,tparcel,p,n_dp)
    END IF
  ELSE
    ! DEPENDS ON: qsat_mix

...
MODULE dts_sublimation_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_SUBLIMATION_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_SUBLIMATION'
      ...
      IF (temperature(i_dp,k) < tm) THEN
        ...
        IF (qse(i_dp,k) > 0.0) THEN
          sublimation(i_dp,k) = deposition(i_dp,k)*                       &
                                  (1.0-gamma_fac*q(i_dp,k)/qse(i_dp,k))
        ELSE
          WRITE(umMessage,*) 'qisat alarm: ',qse(i_dp,k)
          CALL umPrint(umMessage,src='dts_sublimation')
          sublimation(i_dp,k) = deposition(i_dp,k)
        END IF
...
END MODULE dts_sublimation_mod

...
SUBROUTINE congest_conv_6a(nbl,nlev,ntra,n_cca_lev,n_cg,trlev,    &
                       ...
                       tcw,cca_2d, dt_dd, dq_dd, du_dd, dv_dd,    &
                       area_ud, area_dd)
...
USE cv_run_mod, ONLY:                                             &
    l_mom, bl_cnv_mix, icvdiag, l_new_dd,                         &
...
REAL, INTENT(IN)    :: exner_rho(n_cg,nlev)  ! Exner on rho levels
...
REAL, INTENT(IN) :: conv_prog_precip(n_cg,nlev) ! Surface precipitation based
...
REAL :: w_max(n_cg)             ! dummy variable for maximum w in column (m/s)
...
REAL :: thrk_c2(n_cg)           ! potential temperature of forced detrained air

REAL :: qrk_c2(n_cg)            ! specific humidity of forced detrained air
...
REAL :: flx_init_c2(n_cg) ! Initial parcal mass flux at cloud base (Pa/s)
...
ELSE 
  ...
  !-----------------------------------------------------------------------
  ! (b)  Orignal downdraught calculation - on all points where convection is
  !      terminating.
  !
...
IF (l_cv_conserve_check) THEN
  CALL cor_engy_6a(n_cg, n_cg, nlev, index1,                        &
...
DO i = 1,n_cg
  ...
  IF (temp1(i)  <   qminincolumn(i) .AND.                            &
    ...
    CALL umPrint(umMessage,src='congest_conv_mod-6a')
  END IF
END DO ! n_cg loop

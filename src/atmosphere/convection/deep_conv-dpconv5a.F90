...
!
MODULE deep_conv_5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DEEP_CONV_5A_MOD'
CONTAINS
...
USE dd_all_call_4a5a_mod, ONLY: dd_all_call_4a5a
...
USE mix_ipert_4a5a_mod, ONLY: mix_ipert_4a5a
...
REAL, INTENT(INOUT) :: dtrabydt(n_dp,nlev,ntra) ! Increment to tracer 
...
REAL ::      &
  rh_test    &   ! critical RH value for cldbase_opt_dp=6
 ,rh_fac         ! factor for cldbase_opt_dp=6 calculation
...
! This routine is deep
INTEGER, PARAMETER   ::  deep = 1 
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEEP_CONV_5A'
...
! is reduced or not used. Need to later zero points where failed deep.
...
CASE (6)
  ...
  ! 7 CAPE timescale dependent on large-scale w
  ! 8 CAPE timescale dependent on large-scale w but with a lower limit of a cloud
  !    base mass flux of  a_cb*b_cb*wstar (applied later)
...
CASE (7,8)
  ...
  ! Work out required new cloud base mass flux directly
...
CASE (9)
  ...
  DO i = 1,n_dp
    IF (kterm(i)  >=  ntml(i)) THEN   ! actual deep convection
      IF (wls_mean(i) > 0.0) THEN
        ! closure based on large-scale vertical velocity & wstar
      ...
      END IF
    END IF
    cape_ts_used(i) = 0.0     ! As not using a CAPE closure
  END DO

END SELECT        ! cldbase_opt_dp


IF (cldbase_opt_dp /= 9 ) THEN  ! CAPE based closures
...
IF (cldbase_opt_dp == 8) THEN  ! lower limit to scaling
  DO i = 1,n_dp
    IF (kterm(i)  >=  ntml(i)) THEN   ! actual deep convection
      IF (flx_init_new(i) > 0.0) THEN
        scale_f(i) = flx_init_new(i) / flx_init_term(i)
        IF (scale_f(i) < 1.0) THEN ! use original closure i.e. surface based
          scale_f(i) = 1.0
          flx_init_new(i) =  flx_init_term(i)
        END IF
        ...
        ! all restrictions being applied to cloud base mass flux.
        IF (dcpbydt_term(i)  >   0.0) THEN
          cape_ts_used(i) = cape_out(i)/(scale_f(i) * dcpbydt_term(i))
        END IF
      END IF
    END IF
  END DO
...
IF (l_safe_conv) THEN
  ...
  DO i = 1,n_dp
    IF (scale_f(i) < 1.0e-6 .OR. cape_out(i) < 0.0) THEN   ! No deep convection
      scale_f(i)  = 0.0
      flx_init(i) = 0.0
      flx_init_new(i) = 0.0
    ...
    DO i = 1,n_dp
      IF (flx(i,k) > 0.0) THEN
        ...
        ELSE
          ...
          ! as this will be required for the new DD scheme. 
          IF (k == iccb(i)+1) THEN 
...
END IF
!-----------------------------------------------------------------------
! 6.0 Downdraughts  - now 3 options
...
ELSE
  ...
  ELSE 
    !-----------------------------------------------------------------------
    ! (b) Original mass flux Downdraught scheme
    !   Downdraught calculation - work out b_dd and b_nodd
    !-----------------------------------------------------------------------
    ...
    DO i = 1,n_dp
      ...
      ELSE IF (kterm(i) == nlev - 1) THEN
        ...
        IF (printstatus >= prstatus_normal) THEN
          WRITE(umMessage,*) 'Deep point ',i,' kterm ',kterm(i),' nlcl ',nlcl_uv(i)
          CALL umPrint(umMessage,src='deep_conv-dpconv5a')
          ! The following writes are not formated as nlev may vary with model
          ! run
          WRITE(umMessage,*) ' theta ',(th(i,k),k=1,nlev)
          CALL umPrint(umMessage,src='deep_conv-dpconv5a')
          WRITE(umMessage,*) ' q ',(q(i,k),k=1,nlev)
          CALL umPrint(umMessage,src='deep_conv-dpconv5a')
          WRITE(umMessage,*) ' qcl ',(qcl(i,k),k=1,nlev)
          CALL umPrint(umMessage,src='deep_conv-dpconv5a')
          WRITE(umMessage,*) ' qcf ',(qcf(i,k),k=1,nlev)
          CALL umPrint(umMessage,src='deep_conv-dpconv5a')
...
IF (l_safe_conv) THEN
  ...
  DO i = 1,n_dp
    temp1(i)=q(i,1) + dqbydt(i,1) * timestep
    IF (temp1(i)  <   qmin_conv .AND.                                          &
      printstatus >= prstatus_normal .AND. dqbydt(i,k) /= 0.0) THEN
      WRITE(umMessage,'(a20,i6,a9,g26.18,a7,g26.18,a6,i10,i3)')                &
             ' negative q deep, i:',i,' q after ',temp1(i),' dq/dt ',          &
             dqbydt(i,1),' step ',timestep_number,mype
      CALL umPrint(umMessage,src='deep_conv-dpconv5a')
...
ELSE
  ...
  DO i = 1,n_dp
    ...
    IF (temp1(i)  <   qminincolumn(i) .AND.                                    &
      ...
      WRITE(umMessage,'(a20,i6,a9,g26.18,a7,g26.18)') ' negative q deep, i:',  &
          i,' q after ',temp1(i),' dq/dt ',dqbydt(i,1)
      CALL umPrint(umMessage,src='deep_conv-dpconv5a')
...
ELSE
  ...
  CALL mix_inc(n_dp,n_dp,n_real_dp,nlev,nbl,ntml,dthbydt,dqbydt,       &
...
IF (l_ccrad) THEN
  ...
  ELSE
    ...
    DO i=1, n_dp
      ...
      IF ((iccb(i) > 0) .AND. (icct(i) > 0)                             &
          .AND. (iccb(i) <= n_cca_lev)) THEN

...
!
MODULE dts_w_variance_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_W_VARIANCE_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_W_VARIANCE'
    ...
    IF (k < dts_ntpar(i_dp)) THEN
      ...
      ELSE
        ...
        ww(i_dp,k) =  w2lcl(i_dp)*                                        &
                    (0.5*(1.0001+TANH(6.0*(z_theta(i_dp,k)-0.5*zlcl(i_dp)) &
                                          /(0.5*zlcl(i_dp)) )) )**(2.0/3.0)
        IF (ww(i_dp,k) < 0.0) THEN
          ww(i_dp,k) = 0.0
        END IF
      END IF ! diff < 0.0

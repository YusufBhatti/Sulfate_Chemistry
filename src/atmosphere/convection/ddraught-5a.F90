...
! Downdraught routine
!
MODULE ddraught_5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DDRAUGHT_5A_MOD'
CONTAINS

SUBROUTINE ddraught_5a (npnts, np_full, k, kct, ntra,                        &
                     l_tracer,                                               &
                     thdd_k,qdd_k,the_k,                                     &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DDRAUGHT_5A'
...
!-----------------------------------------------------------------------
!
CALL crs_frzl(npnts, bddwt_km1, exk, exkm1, flx_dd_km1,                 &
...
IF ( l_new_qsat_conv ) THEN
  CALL qsat_new(qsatdd,tdd_km1,p_km1,npnts)
ELSE
  ! DEPENDS ON: qsat
...
END SUBROUTINE ddraught_5a
END MODULE ddraught_5a_mod

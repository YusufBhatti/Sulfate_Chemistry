...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_PC2_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_PC2'
...
DO k=1,nlev-1
  DO i_dp=1,n_dp
    dmdz(i_dp,k) = (dmdz_rho(i_dp,k)*rho(i_dp,k)               &
                    +dmdz_rho(i_dp,k+1)*rho(i_dp,k+1))/2.0      &
                     /rho_theta(i_dp,k)
...
IF (l_calc_dxek) THEN
  DO k=1,nlev-1
    DO i_dp=1,n_dp
      ...
      !       END IF
      dcflbydt(i_dp,k) = detr(i_dp,k)*(1.0-cf_liquid(i_dp,k))&
                                          /rho_theta(i_dp,k)
      !--------------------------------------------------------------
      ! Ice
      ...
      !       END IF
      dcffbydt(i_dp,k) = detr(i_dp,k)*(1.0-cf_frozen(i_dp,k))&
                                            /rho_theta(i_dp,k)
      !--------------------------------------------------------------
  ...
  DO i = 1,n_dp
    IF (dqclbydt(i,k) /= 0.0) THEN
      ...
      IF (temp1(i)  <   0.0) THEN
        ...
        WRITE(umMessage,*) ' negative qcl deep',i,k,dts_ntpar(i), &
                     temp1(i),dqclbydt(i,k)
        CALL umPrint(umMessage,src='dts_pc2')
        WRITE(umMessage,*) 'dqcl',(dqclbydt(i,kk),kk=1,nlev)
        CALL umPrint(umMessage,src='dts_pc2')
        WRITE(umMessage,*) 'qcl',(qcl_env(i,kk),kk=1,nlev)
        CALL umPrint(umMessage,src='dts_pc2')
    ...
    IF (dqcfbydt(i,k) /= 0.0) THEN
      ...
      IF (temp1(i)  <   0.0) THEN
        ...
        ! Only warning print statement not doing any thing
        WRITE(umMessage,*) ' negative qcf deep',i,k,dts_ntpar(i), &
                         temp1(i),dqcfbydt(i,k)
        CALL umPrint(umMessage,src='dts_pc2')
        WRITE(umMessage,*) 'dqcf',(dqcfbydt(i,kk),kk=1,nlev)
        CALL umPrint(umMessage,src='dts_pc2')
        WRITE(umMessage,*) 'qcf',(qcf_env(i,kk),kk=1,nlev)
        CALL umPrint(umMessage,src='dts_pc2')

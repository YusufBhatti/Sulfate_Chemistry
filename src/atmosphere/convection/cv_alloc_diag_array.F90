! ------------------------------------------------------------------------------
! (c) Crown copyright Met Office. All rights reserved.
! For further details please refer to the file COPYRIGHT.txt
! which you should have received as part of this distribution.
! ------------------------------------------------------------------------------
!
! ALLOCATE and initialise arrays for convective diagnostics
!
MODULE cv_alloc_diag_array_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'CV_ALLOC_DIAG_ARRAY_MOD'
CONTAINS

SUBROUTINE cv_alloc_diag_array( row_length, rows, l_cosp,                     &
                               l_dust, l_sulpc_so2, l_sulpc_nh3, l_soot,      &
                               l_biomass, l_ocff, l_nitrate )


! Definitions of prognostic variable array sizes
USE atm_fields_bounds_mod, ONLY:                                          &
   udims, vdims, wdims, tdims, pdims

USE cv_run_mod,  ONLY:                                                    &
   n_conv_calls, l_mom

USE dynamics_input_mod,  ONLY:                                            &
   l_check_moist_inc

USE cv_diagnostic_array_mod , ONLY:                                     &
        precip_deep, precip_shall ,precip_mid ,precip_cong ,cape_out    &
       ,deep_ind ,shallow_ind ,congestus_ind ,congestus_ind2 ,mid_ind   &
       ,ntml_diag ,ntpar_diag ,freeze_diag ,kterm_diag                  &
       ,wstar_up_diag ,wstar_dn_diag ,mb1_diag ,mb2_diag ,wqt_cb        &
       ,wthetal_cb ,wqt_inv ,wthetal_inv ,sh_top ,sh_base ,cg_top       &
       ,cg_base ,cg_term ,cape_ts_diag ,ind_cape_reduced_diag           &
       ,conscav_dust ,conscav_so4ait ,conscav_so4acc ,conscav_so4dis    &
       ,conscav_agedsoot ,conscav_agedbmass ,conscav_agedocff           &
       ,conscav_nitracc ,conscav_nitrdiss ,conwash_so2 ,conwash_nh3     &
       ,uw_dp, vw_dp ,uw_shall ,vw_shall ,uw_mid ,vw_mid ,up_flux       &
       ,dwn_flux ,entrain_up ,detrain_up ,entrain_dwn ,detrain_dwn      &
       ,up_flux_half                                                    &
       ,theta_incr_diag_conv ,T_incr_diag_conv ,q_incr_diag_conv        &
       ,qcl_incr_diag_conv ,qcf_incr_diag_conv                          &
       ,cf_liquid_incr_diag_conv ,cf_frozen_incr_diag_conv              &
       ,bulk_cf_incr_diag_conv                                          &
       ,mf_deep ,mf_congest ,mf_shall ,mf_midlev                        &
       ,dt_deep ,dt_congest ,dt_shall ,dt_midlev                        &
       ,dq_deep ,dq_congest ,dq_shall ,dq_midlev                        &
       ,du_deep ,du_congest ,du_shall ,du_midlev                        &
       ,dv_deep ,dv_congest ,dv_shall ,dv_midlev                        &
       ,wqt_flux_sh ,wql_flux_sh ,wthetal_flux_sh ,wthetav_flux_sh      &
       ,conv_rain_3d ,conv_snow_3d                                      &
       ,t_incr_conv_only,q_incr_conv_only                               &
       ,qcl_incr_inhom_diag ,qcf_incr_inhom_diag                        &
       ,bulk_cf_incr_inhom_diag ,cf_liquid_incr_inhom_diag              &
       ,cf_frozen_incr_inhom_diag, deep_cfl_limited, mid_cfl_limited    &
       ,deep_tops, w2p, ep1, ep2, ep3, dt_dd, dq_dd, du_dd, dv_dd       &
       ,area_ud, area_dd

USE cv_stash_flg_mod, ONLY:                                             &
    l_apply_diag, l_qcl_incr_cinh, l_qcf_incr_cinh, l_cfl_incr_cinh     &
   ,l_cff_incr_cinh, l_bcf_incr_cinh, flg_w_eqn                         &
   ,l_t_incr_conv, l_q_incr_conv, l_qcl_incr_conv, l_qcf_incr_conv      &
   ,l_cfl_incr_conv, l_cff_incr_conv, l_bcf_incr_conv                   &
   ,l_t_conv_only, l_q_conv_only

USE model_domain_mod, ONLY: model_type, mt_single_column

USE yomhook, ONLY: lhook, dr_hook
USE parkind1, ONLY: jprb, jpim
USE stash_array_mod, ONLY: sf

IMPLICIT NONE

! ------------------------------------------------------------------------------
! Description:
!   ALLOCATE and initialise  arrays required by convection for a full model
!   timestep on the first convection substep of a model timestep.
!
! Code Owner: Please refer to the UM file CodeOwners.txt
! This file belongs in section: Convection
!
! Code description:
!   Language: Fortran 90.
!   This code is written to UM programming standards version 8.2.

! ------------------------------------------------------------------------------
! Subroutine arguments
!-------------------------------------------------------------------------------
INTEGER, INTENT(IN) ::  &
  row_length            & ! Local number of points on a row
 ,rows                    ! Local number of rows in a theta field

LOGICAL, INTENT(IN)  ::  &
  l_dust                 & ! Switch for mineral dust
 ,l_sulpc_so2            & ! Switch for sulphur cycle
 ,l_sulpc_nh3            & ! Switch for NH3 in S-cycle
 ,l_soot                 & ! Switch for soot cycle
 ,l_biomass              & ! Switch for biomass aerosol scheme
 ,l_ocff                 & ! Switch for fossil-fuel organic carbon scheme
 ,l_nitrate                ! Switch for ammonium nitrate aerosol

! Switch for COSP
LOGICAL, INTENT(IN)  ::  l_cosp

!-------------------------------------------------------------------------
! Local variables
INTEGER ::               &
  i, j, k                  ! loop counters

REAL ::                  &
  one_over_conv_calls      ! 1/n_conv_calls

! Required by Dr hook
INTEGER(KIND=jpim), PARAMETER :: zhook_in  = 0
INTEGER(KIND=jpim), PARAMETER :: zhook_out = 1
REAL(KIND=jprb)               :: zhook_handle

CHARACTER(LEN=*), PARAMETER :: RoutineName='CV_ALLOC_DIAG_ARRAY'

!------------------------------------------------------------------------------


IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)

!------------------------------------------------------------------------------
! Allocate 2d arrays for diagnostics

ALLOCATE(precip_deep(row_length,rows) )
ALLOCATE(precip_shall(row_length,rows) )
ALLOCATE(precip_mid(row_length,rows) )
ALLOCATE(precip_cong(row_length,rows) )
ALLOCATE(cape_out(row_length,rows) )
ALLOCATE(deep_ind(row_length,rows) )
ALLOCATE(shallow_ind(row_length,rows) )
ALLOCATE(congestus_ind(row_length,rows) )
ALLOCATE(congestus_ind2(row_length,rows) )
ALLOCATE(mid_ind(row_length,rows) )
ALLOCATE(ntml_diag(row_length,rows) )
ALLOCATE(ntpar_diag(row_length,rows) )
ALLOCATE(freeze_diag(row_length,rows) )
ALLOCATE(kterm_diag(row_length,rows) )
ALLOCATE(wstar_up_diag(row_length,rows) )
ALLOCATE(wstar_dn_diag(row_length,rows) )
ALLOCATE(mb1_diag(row_length,rows) )
ALLOCATE(mb2_diag(row_length,rows) )
ALLOCATE(wqt_cb(row_length, rows) )
ALLOCATE(wthetal_cb(row_length, rows) )
ALLOCATE(wqt_inv(row_length, rows) )
ALLOCATE(wthetal_inv(row_length, rows) )
ALLOCATE(sh_top(row_length, rows) )
ALLOCATE(sh_base(row_length, rows) )
ALLOCATE(cg_top(row_length, rows) )
ALLOCATE(cg_base(row_length, rows) )
ALLOCATE(cg_term(row_length, rows) )
ALLOCATE(cape_ts_diag(row_length, rows) )
ALLOCATE(ind_cape_reduced_diag(row_length, rows) )
ALLOCATE(deep_cfl_limited(row_length, rows) )
ALLOCATE(mid_cfl_limited(row_length, rows) )

! Initialise diagnostics

one_over_conv_calls = 1.0/(n_conv_calls*1.0)

!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE ( i, j )
DO j = 1, rows
  DO i = 1, row_length
    ntml_diag(i,j)  = 0.0
    ntpar_diag(i,j) = 0.0
    precip_deep(i,j)  = 0.0
    precip_shall(i,j) = 0.0
    precip_mid(i,j)   = 0.0
    precip_cong(i,j)  = 0.0
    cape_out(i,j) = 0.0
    kterm_diag(i,j) = 0.0
    freeze_diag(i,j) = 0.0
    deep_ind(i,j)       = 0.0
    shallow_ind(i,j)    = 0.0
    congestus_ind(i,j)  = 0.0
    congestus_ind2(i,j) = 0.0
    mid_ind(i,j)        = 0.0
    ind_cape_reduced_diag(i,j) = 0.0
    cape_ts_diag(i,j) = 0.0
    deep_cfl_limited(i,j) = 0.0
    mid_cfl_limited(i,j)  = 0.0

    ! 5A & 6A diagnostics
    wstar_up_diag(i,j) = 0.0
    wstar_dn_diag(i,j) = 0.0
    mb1_diag(i,j) = 0.0
    mb2_diag(i,j) = 0.0
    cg_term(i,j) = 0.0
    cg_top(i,j)  = 0.0
    cg_base(i,j) = 0.0
    wqt_cb(i,j)     = 0.0
    wthetal_cb(i,j) = 0.0
    wqt_inv(i,j)     = 0.0
    wthetal_inv(i,j) = 0.0
    sh_top(i,j)  = 0.0
    sh_base(i,j) = 0.0

  END DO
END DO
!$OMP END PARALLEL DO

IF (l_check_moist_inc) THEN    ! Only required for checking moisture
  ALLOCATE(ep1(row_length, rows) )
  ALLOCATE(ep2(row_length, rows) )
  ALLOCATE(ep3(row_length, rows) )
  DO j = 1, rows
    DO i = 1, row_length
      ep1(i,j) = 0.0
      ep2(i,j) = 0.0
      ep3(i,j) = 0.0
    END DO
  END DO
END IF

! Convective aerosol relative diagnostics - allocation dependent on whether
! active or not.

IF (l_dust) THEN
  ALLOCATE(conscav_dust(row_length, rows, 6))
ELSE
  ALLOCATE(conscav_dust(1,1,1))
END IF

IF (l_soot) THEN
  ALLOCATE(conscav_agedsoot(row_length, rows) )
ELSE
  ALLOCATE(conscav_agedsoot(1,1) )
END IF

IF (l_biomass) THEN
  ALLOCATE(conscav_agedbmass(row_length, rows) )
ELSE
  ALLOCATE(conscav_agedbmass(1,1) )
END IF
IF (l_ocff) THEN
  ALLOCATE(conscav_agedocff(row_length, rows) )
ELSE
  ALLOCATE(conscav_agedocff(1,1) )
END IF

IF (l_nitrate) THEN
  ALLOCATE(conscav_nitracc(row_length, rows) )
  ALLOCATE(conscav_nitrdiss(row_length, rows) )
ELSE
  ALLOCATE(conscav_nitracc(1,1) )
  ALLOCATE(conscav_nitrdiss(1,1) )
END IF
IF (l_sulpc_so2) THEN
  ALLOCATE(conwash_so2(row_length, rows) )
  ALLOCATE(conscav_so4ait(row_length, rows) )
  ALLOCATE(conscav_so4acc(row_length, rows) )
  ALLOCATE(conscav_so4dis(row_length, rows) )
ELSE
  ALLOCATE(conwash_so2(1,1) )
  ALLOCATE(conscav_so4ait(1,1) )
  ALLOCATE(conscav_so4acc(1,1) )
  ALLOCATE(conscav_so4dis(1,1) )
END IF
IF (l_sulpc_nh3 .AND. l_sulpc_so2 ) THEN
  ALLOCATE(conwash_nh3(row_length, rows) )
ELSE
  ALLOCATE(conwash_nh3(1,1) )
END IF


SELECT CASE (model_type)

CASE DEFAULT

  !---------------------------------------------------------------------
  ! Allocate convection increment diagnostics and initialise to zero.
  !---------------------------------------------------------------------

  ! Convection increment diagnostics

  ALLOCATE( theta_incr_diag_conv(1,1,1) )  ! Only output by the SCM

  IF ( l_t_incr_conv ) THEN
    ALLOCATE ( T_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                tdims%j_start:tdims%j_end,    &
                                            1:tdims%k_end))
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, T_incr_diag_conv )                                &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          T_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO

  ELSE
    ALLOCATE ( T_incr_diag_conv(1,1,1) )
  END IF

  IF ( l_q_incr_conv ) THEN
    ALLOCATE (q_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                               tdims%j_start:tdims%j_end,    &
                                           1:tdims%k_end))
    ! Hold input q increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, q_incr_diag_conv )                                &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          q_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE ( q_incr_diag_conv(1,1,1) )
  END IF

  IF ( l_qcl_incr_conv ) THEN
    ALLOCATE (qcl_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                 tdims%j_start:tdims%j_end,    &
                                             1:tdims%k_end))
    ! Hold input qcl increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, qcl_incr_diag_conv )                              &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          qcl_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE (qcl_incr_diag_conv(1,1,1))
  END IF                   ! on STASHflag

  IF ( l_qcf_incr_conv ) THEN
    ALLOCATE (qcf_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                 tdims%j_start:tdims%j_end,    &
                                             1:tdims%k_end))

    ! Hold input qcf increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, qcf_incr_diag_conv )                              &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          qcf_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE (qcf_incr_diag_conv(1,1,1))
  END IF                   ! on STASHflag

  IF ( l_cfl_incr_conv ) THEN
    ALLOCATE(cf_liquid_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                      tdims%j_start:tdims%j_end,    &
                                                  1:tdims%k_end))

    ! Hold input cf_liquid increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, cf_liquid_incr_diag_conv )                        &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          cf_liquid_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE(cf_liquid_incr_diag_conv(1,1,1))
  END IF                   ! on STASHflag

  IF ( l_cff_incr_conv ) THEN
    ALLOCATE(cf_frozen_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                      tdims%j_start:tdims%j_end,    &
                                                  1:tdims%k_end))

    ! Hold input cf_frozen increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, cf_frozen_incr_diag_conv )                        &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          cf_frozen_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE(cf_frozen_incr_diag_conv(1,1,1))
  END IF                   ! on STASHflag

  IF ( l_bcf_incr_conv ) THEN
    ALLOCATE(bulk_cf_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                    tdims%j_start:tdims%j_end,    &
                                                1:tdims%k_end))

    ! Hold input bulk_cf increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, bulk_cf_incr_diag_conv )                          &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          bulk_cf_incr_diag_conv(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE(bulk_cf_incr_diag_conv(1,1,1))
  END IF                   ! on STASHflag

  IF ( l_T_conv_only ) THEN
    ALLOCATE ( T_incr_conv_only(tdims%i_start:tdims%i_end,    &
                                tdims%j_start:tdims%j_end,    &
                                            1:tdims%k_end))
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, T_incr_conv_only )                                &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          T_incr_conv_only(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO

  ELSE
    ALLOCATE ( T_incr_conv_only(1,1,1) )
  END IF

  IF ( l_q_conv_only ) THEN
    ALLOCATE (q_incr_conv_only(tdims%i_start:tdims%i_end,    &
                               tdims%j_start:tdims%j_end,    &
                                           1:tdims%k_end))
    ! Hold input q increment
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, q_incr_conv_only )                                &
!$OMP PRIVATE( i, j, k )
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          q_incr_conv_only(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE ( q_incr_conv_only(1,1,1) )
  END IF

  ! Downdraught only increments as rates not per timestep
  IF (sf(198,5) .AND. L_apply_diag) THEN
    ALLOCATE (dt_dd(tdims%i_start:tdims%i_end,    &
                    tdims%j_start:tdims%j_end,    &
                                1:tdims%k_end))
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          dt_dd(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
  ELSE
    ALLOCATE ( dt_dd(1,1,1) )
  END IF
  IF (sf(199,5) .AND. L_apply_diag) THEN
    ALLOCATE (dq_dd(tdims%i_start:tdims%i_end,    &
                    tdims%j_start:tdims%j_end,    &
                                1:tdims%k_end))

    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          dq_dd(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
  ELSE
    ALLOCATE ( dq_dd(1,1,1) )
  END IF

! du/dt & dv/dt from DD on p-grid not uv-grid
  IF (sf(175,5) .AND. L_apply_diag) THEN
    ALLOCATE (du_dd(tdims%i_start:tdims%i_end,    &
                    tdims%j_start:tdims%j_end,    &
                                1:tdims%k_end))
    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          du_dd(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
  ELSE
    ALLOCATE ( du_dd(1,1,1) )
  END IF

  IF (sf(176,5) .AND. L_apply_diag) THEN
    ALLOCATE (dv_dd(tdims%i_start:tdims%i_end,    &
                    tdims%j_start:tdims%j_end,    &
                                1:tdims%k_end))

    DO k =             1, tdims%k_end
      DO j = tdims%j_start, tdims%j_end
        DO i = tdims%i_start, tdims%i_end
          dv_dd(i,j,k) = 0.0
        END DO ! i
      END DO ! j
    END DO ! k
  ELSE
    ALLOCATE ( dv_dd(1,1,1) )
  END IF

  ! Other model level diagnostics :
  ! setup allocatable arrays to limit memory over head if diagnostics not
  ! required.
  IF ( sf(229,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( area_ud(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          area_ud(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( area_ud(1,1,1) )
  END IF
  IF ( sf(230,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( area_dd(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          area_dd(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( area_dd(1,1,1) )
  END IF

  IF ( ( sf(227,5) .AND. L_apply_diag ) .OR. L_cosp ) THEN
    ALLOCATE ( conv_rain_3d(tdims%i_start:tdims%i_end,        &
                            tdims%j_start:tdims%j_end,        &
                                        1:tdims%k_end) )
    DO k=1, tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i=tdims%i_start,tdims%i_end
          conv_rain_3d(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( conv_rain_3d(1,1,1) )
  END IF
  IF ( ( sf(228,5) .AND. L_apply_diag ) .OR. L_cosp ) THEN
    ALLOCATE ( conv_snow_3d(tdims%i_start:tdims%i_end,        &
                            tdims%j_start:tdims%j_end,        &
                                        1:tdims%k_end) )
    DO k=1, tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i=tdims%i_start,tdims%i_end
          conv_snow_3d(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( conv_snow_3d(1,1,1) )
  END IF

  IF ( (sf(290,5) .OR. sf(429,5) .OR. sf(430,5) .OR.          &
        sf(431,5) .OR. sf(432,5) ) .AND. L_apply_diag ) THEN
    ALLOCATE ( wqt_flux_sh(tdims%i_start:tdims%i_end,         &
                           tdims%j_start:tdims%j_end,         &
                                       1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          wqt_flux_sh(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( wqt_flux_sh(1,1,1) )
  END IF

  IF ( sf(291,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( wql_flux_sh(tdims%i_start:tdims%i_end,        &
                           tdims%j_start:tdims%j_end,        &
                                       1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          wql_flux_sh(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( wql_flux_sh(1,1,1) )
  END IF

  IF ( (sf(292,5) .OR. sf(425,5) .OR. sf(426,5) .OR.          &
        sf(427,5) .OR. sf(428,5) ) .AND. L_apply_diag ) THEN
    ALLOCATE ( wthetal_flux_sh(tdims%i_start:tdims%i_end,     &
                               tdims%j_start:tdims%j_end,     &
                                           1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          wthetal_flux_sh(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( wthetal_flux_sh(1,1,1) )
  END IF

  IF ( sf(293,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( wthetav_flux_sh(tdims%i_start:tdims%i_end,        &
                               tdims%j_start:tdims%j_end,        &
                                           1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          wthetav_flux_sh(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( wthetav_flux_sh(1,1,1) )
  END IF

  IF ( sf(320,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( mf_deep(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          mf_deep(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( mf_deep(1,1,1) )
  END IF
  IF ( sf(321,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( mf_congest(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          mf_congest(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( mf_congest(1,1,1) )
  END IF

  IF ( ( sf(322,5) .OR. sf(417,5) .OR. sf(418,5) .OR.         &
         sf(419,5) .OR. sf(420,5) ) .AND. L_apply_diag ) THEN
    ALLOCATE ( mf_shall(tdims%i_start:tdims%i_end,            &
                        tdims%j_start:tdims%j_end,            &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          mf_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( mf_shall(1,1,1) )
  END IF

  IF ( sf(323,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( mf_midlev(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          mf_midlev(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( mf_midlev(1,1,1) )
  END IF
  IF ( sf(324,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dt_deep(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dt_deep(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dt_deep(1,1,1) )
  END IF
  IF ( sf(325,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dt_congest(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dt_congest(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dt_congest(1,1,1) )
  END IF

  IF ( ( sf(326,5) .OR. sf(409,5) .OR. sf(410,5) .OR.         &
             sf(411,5) .OR. sf(412,5) ) .AND. L_apply_diag ) THEN
    ALLOCATE ( dt_shall(tdims%i_start:tdims%i_end,            &
                        tdims%j_start:tdims%j_end,            &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dt_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dt_shall(1,1,1) )
  END IF

  IF ( sf(327,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dt_midlev(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dt_midlev(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dt_midlev(1,1,1) )
  END IF
  IF ( sf(328,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dq_deep(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dq_deep(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dq_deep(1,1,1) )
  END IF
  IF ( sf(329,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dq_congest(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dq_congest(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dq_congest(1,1,1) )
  END IF

  IF ( ( sf(330,5) .OR. sf(413,5) .OR. sf(414,5) .OR.         &
         sf(415,5) .OR. sf(416,5) ) .AND. L_apply_diag ) THEN
    ALLOCATE ( dq_shall(tdims%i_start:tdims%i_end,            &
                        tdims%j_start:tdims%j_end,            &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dq_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dq_shall(1,1,1) )
  END IF

  IF ( sf(331,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dq_midlev(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dq_midlev(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dq_midlev(1,1,1) )
  END IF
  IF ( sf(332,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( du_deep(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          du_deep(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( du_deep(1,1,1) )
  END IF
  IF ( sf(333,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( du_congest(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          du_congest(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( du_congest(1,1,1) )
  END IF
  IF ( sf(334,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( du_shall(tdims%i_start:tdims%i_end,        &
                        tdims%j_start:tdims%j_end,        &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          du_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( du_shall(1,1,1) )
  END IF
  IF ( sf(335,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( du_midlev(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          du_midlev(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( du_midlev(1,1,1) )
  END IF
  IF ( sf(336,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dv_deep(tdims%i_start:tdims%i_end,        &
                       tdims%j_start:tdims%j_end,        &
                                   1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dv_deep(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dv_deep(1,1,1) )
  END IF
  IF ( sf(337,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dv_congest(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dv_congest(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dv_congest(1,1,1) )
  END IF
  IF ( sf(338,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dv_shall(tdims%i_start:tdims%i_end,        &
                        tdims%j_start:tdims%j_end,        &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dv_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dv_shall(1,1,1) )
  END IF
  IF ( sf(339,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( dv_midlev(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j = tdims%j_start,tdims%j_end
        DO i = tdims%i_start,tdims%i_end
          dv_midlev(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( dv_midlev(1,1,1) )
  END IF

  IF ( (sf(249,5) .OR. sf(246,5) ) .AND. L_apply_diag ) THEN
    ALLOCATE ( up_flux_half(pdims%i_start:pdims%i_end,        &
                            pdims%j_start:pdims%j_end,        &
                            pdims%k_start:pdims%k_end) )
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( pdims, up_flux_half )                                    &
!$OMP PRIVATE( i, j, k )
  DO k = pdims%k_start,pdims%k_end
    DO j = pdims%j_start,pdims%j_end
      DO i = pdims%i_start,pdims%i_end
        up_flux_half(i,j,k) = 0.0
      END DO
    END DO
  END DO
!$OMP END PARALLEL DO
  ELSE
    ALLOCATE ( up_flux_half(1,1,1) )
  END IF

  ! Note always require mass flux up and down as not just required in
  ! atmos_physics2 for output diagnostics

  ALLOCATE ( up_flux(tdims%i_start:tdims%i_end,           &
                     tdims%j_start:tdims%j_end,           &
                                 1:tdims%k_end) )
  ALLOCATE ( dwn_flux(tdims%i_start:tdims%i_end,          &
                      tdims%j_start:tdims%j_end,          &
                                  1:tdims%k_end) )

!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, up_flux, dwn_flux )                               &
!$OMP PRIVATE( i, j, k )
  DO k = 1, tdims%k_end
    DO j = tdims%j_start,tdims%j_end
      DO i = tdims%i_start,tdims%i_end
        up_flux(i,j,k)=0.0
        dwn_flux(i,j,k) = 0.0
      END DO
    END DO
  END DO
!$OMP END PARALLEL DO

  IF ( sf(252,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( entrain_up(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          entrain_up(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( entrain_up(1,1,1) )
  END IF

  IF ( sf(253,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( detrain_up(tdims%i_start:tdims%i_end,        &
                          tdims%j_start:tdims%j_end,        &
                                      1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          detrain_up(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( detrain_up(1,1,1) )
  END IF

  IF ( sf(254,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( entrain_dwn(tdims%i_start:tdims%i_end,        &
                           tdims%j_start:tdims%j_end,        &
                                       1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          entrain_dwn(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( entrain_dwn(1,1,1) )
  END IF

  IF ( sf(255,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( detrain_dwn(tdims%i_start:tdims%i_end,        &
                           tdims%j_start:tdims%j_end,        &
                                       1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          detrain_dwn(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( detrain_dwn(1,1,1) )
  END IF

  IF ( sf(258,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( uw_dp(tdims%i_start:tdims%i_end,        &
                     tdims%j_start:tdims%j_end,        &
                                 1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          uw_dp(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( uw_dp(1,1,1) )
  END IF


  IF ( sf(259,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( vw_dp(tdims%i_start:tdims%i_end,        &
                     tdims%j_start:tdims%j_end,        &
                                 1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          vw_dp(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( vw_dp(1,1,1) )
  END IF

  IF ( sf(260,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( uw_shall(tdims%i_start:tdims%i_end,        &
                        tdims%j_start:tdims%j_end,        &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          uw_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( uw_shall(1,1,1) )
  END IF


  IF ( sf(261,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( vw_shall(tdims%i_start:tdims%i_end,        &
                        tdims%j_start:tdims%j_end,        &
                                    1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          vw_shall(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( vw_shall(1,1,1) )
  END IF

  IF ( sf(263,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( uw_mid(tdims%i_start:tdims%i_end,        &
                      tdims%j_start:tdims%j_end,        &
                                  1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          uw_mid(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( uw_mid(1,1,1) )
  END IF


  IF ( sf(264,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( vw_mid(tdims%i_start:tdims%i_end,        &
                      tdims%j_start:tdims%j_end,        &
                                  1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          vw_mid(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( vw_mid(1,1,1) )
  END IF

  IF ( sf(319,5) .AND. L_apply_diag ) THEN
    ALLOCATE ( deep_tops(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
    DO k=1,tdims%k_end
      DO j=tdims%j_start,tdims%j_end
        DO i= tdims%i_start,tdims%i_end
          deep_tops(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( deep_tops(1,1,1) )
  END IF

  IF ( ( sf(196,5) .OR. sf(197,5) ) .AND. l_apply_diag ) THEN
    ALLOCATE( w2p (tdims%i_start:tdims%i_end,     &
                   tdims%j_start:tdims%j_end,     &
                               1:tdims%k_end) )
    DO k=1, tdims%k_end
      DO j=tdims%j_start, tdims%j_end
        DO i=tdims%i_start, tdims%i_end
          w2p(i,j,k) = 0.0
        END DO
      END DO
    END DO
  ELSE
    ALLOCATE ( w2p(1,1,1) )
    w2p(1,1,1) = 0.0
  END IF

CASE (mt_single_column)

  ALLOCATE( w2p (tdims%i_start:tdims%i_end,                    &
                 tdims%j_start:tdims%j_end,                    &
                             1:tdims%k_end) )

  ALLOCATE( theta_incr_diag_conv(tdims%i_start:tdims%i_end,    &
                                 tdims%j_start:tdims%j_end,    &
                                       1:tdims%k_end) )
  ALLOCATE( T_incr_diag_conv(tdims%i_start:tdims%i_end,        &
                             tdims%j_start:tdims%j_end,        &
                                       1:tdims%k_end) )
  ALLOCATE(q_incr_diag_conv (tdims%i_start:tdims%i_end,        &
                             tdims%j_start:tdims%j_end,        &
                                         1:tdims%k_end) )
  ALLOCATE(qcl_incr_diag_conv(tdims%i_start:tdims%i_end,        &
                              tdims%j_start:tdims%j_end,        &
                                           1:tdims%k_end) )
  ALLOCATE(qcf_incr_diag_conv(tdims%i_start:tdims%i_end,        &
                              tdims%j_start:tdims%j_end,        &
                                          1:tdims%k_end) )
  ALLOCATE(cf_liquid_incr_diag_conv(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                                1:tdims%k_end)  )
  ALLOCATE(cf_frozen_incr_diag_conv(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                                1:tdims%k_end) )
  ALLOCATE(bulk_cf_incr_diag_conv  (tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                                1:tdims%k_end) )
  ALLOCATE(T_incr_conv_only  (tdims%i_start:tdims%i_end,              &
                                    tdims%j_start:tdims%j_end,        &
                                                1:tdims%k_end) )
  ALLOCATE(q_incr_conv_only  (tdims%i_start:tdims%i_end,              &
                                    tdims%j_start:tdims%j_end,        &
                                                1:tdims%k_end) )
  ALLOCATE(dt_dd  (tdims%i_start:tdims%i_end,              &
                   tdims%j_start:tdims%j_end,              &
                               1:tdims%k_end) )
  ALLOCATE(dq_dd  (tdims%i_start:tdims%i_end,              &
                   tdims%j_start:tdims%j_end,              &
                               1:tdims%k_end) )
  ALLOCATE(du_dd  (tdims%i_start:tdims%i_end,              &
                   tdims%j_start:tdims%j_end,              &
                               1:tdims%k_end) )
  ALLOCATE(dv_dd  (tdims%i_start:tdims%i_end,              &
                   tdims%j_start:tdims%j_end,              &
                               1:tdims%k_end) )
  ALLOCATE(area_ud(tdims%i_start:tdims%i_end,              &
                   tdims%j_start:tdims%j_end,              &
                               1:tdims%k_end) )
  ALLOCATE(area_dd(tdims%i_start:tdims%i_end,              &
                   tdims%j_start:tdims%j_end,              &
                               1:tdims%k_end) )

  ! SCM need to initialise diagnostic space

  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        T_incr_diag_conv(i,j,k) = 0.0
        T_incr_conv_only(i,j,k) = 0.0
        dt_dd(i,j,k) = 0.0
        du_dd(i,j,k) = 0.0
        dv_dd(i,j,k) = 0.0
      END DO ! i
    END DO !
  END DO ! k

  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        q_incr_diag_conv(i,j,k)   = 0.0
        q_incr_conv_only(i,j,k)   = 0.0
        qcl_incr_diag_conv(i,j,k) = 0.0
        qcf_incr_diag_conv(i,j,k) = 0.0
        cf_liquid_incr_diag_conv(i,j,k) = 0.0
        cf_frozen_incr_diag_conv(i,j,k) = 0.0
        bulk_cf_incr_diag_conv(i,j,k)   = 0.0
        dq_dd(i,j,k) = 0.0
        area_ud(i,j,k)  = 0.0
        area_dd(i,j,k)  = 0.0
      END DO ! i
    END DO ! j
  END DO ! k

  ALLOCATE(up_flux_half(pdims%i_start:pdims%i_end,        &
                        pdims%j_start:pdims%j_end,        &
                        pdims%k_start:pdims%k_end) )
  up_flux_half(:,:,:) = 0.0

  ALLOCATE(mf_deep(1,1,tdims%k_end) )
  ALLOCATE(mf_congest(1,1,tdims%k_end) )
  ALLOCATE(mf_shall(1,1,tdims%k_end) )
  ALLOCATE(mf_midlev(1,1,tdims%k_end) )
  ALLOCATE(dt_deep(1,1,tdims%k_end) )
  ALLOCATE(dt_congest(1,1,tdims%k_end) )
  ALLOCATE(dt_shall(1,1,tdims%k_end) )
  ALLOCATE(dt_midlev(1,1,tdims%k_end) )
  ALLOCATE(dq_deep(1,1,tdims%k_end) )
  ALLOCATE(dq_congest(1,1,tdims%k_end) )
  ALLOCATE(dq_shall(1,1,tdims%k_end) )
  ALLOCATE(dq_midlev(1,1,tdims%k_end) )
  ALLOCATE(du_deep (1,1,tdims%k_end) )
  ALLOCATE(du_congest(1,1,tdims%k_end) )
  ALLOCATE(du_shall(1,1,tdims%k_end) )
  ALLOCATE(du_midlev(1,1,tdims%k_end) )
  ALLOCATE(dv_deep(1,1,tdims%k_end) )
  ALLOCATE(dv_congest(1,1,tdims%k_end) )
  ALLOCATE(dv_shall(1,1,tdims%k_end) )
  ALLOCATE(dv_midlev(1,1,tdims%k_end) )
  ALLOCATE(wqt_flux_sh(1,1,tdims%k_end) )
  ALLOCATE(wql_flux_sh(1,1,tdims%k_end) )
  ALLOCATE(wthetal_flux_sh(1,1,tdims%k_end) )
  ALLOCATE(wthetav_flux_sh(1,1,tdims%k_end) )
  ALLOCATE(conv_rain_3d(1,1,tdims%k_end) )
  ALLOCATE(conv_snow_3d(1,1,tdims%k_end) )
  ALLOCATE(deep_tops(1,1,tdims%k_end) )

  DO k = 1, tdims%k_end
    conv_rain_3d(1,1,k) = 0.0
    conv_snow_3d(1,1,k) = 0.0
    mf_deep(1,1,k) = 0.0
    mf_congest(1,1,k) = 0.0
    mf_shall(1,1,k) = 0.0
    mf_midlev(1,1,k) = 0.0
    dt_deep(1,1,k) = 0.0
    dt_congest(1,1,k) = 0.0
    dt_shall(1,1,k) = 0.0
    dt_midlev(1,1,k) = 0.0
    dq_deep(1,1,k) = 0.0
    dq_congest(1,1,k) = 0.0
    dq_shall(1,1,k) = 0.0
    dq_midlev(1,1,k) = 0.0
    du_deep (1,1,k) = 0.0
    du_congest(1,1,k) = 0.0
    du_shall(1,1,k) = 0.0
    du_midlev(1,1,k) = 0.0
    dv_deep(1,1,k) = 0.0
    dv_congest(1,1,k) = 0.0
    dv_shall(1,1,k) = 0.0
    dv_midlev(1,1,k) = 0.0
    wqt_flux_sh(1,1,k) = 0.0
    wql_flux_sh(1,1,k) = 0.0
    wthetal_flux_sh(1,1,k) = 0.0
    wthetav_flux_sh(1,1,k) = 0.0
    w2p(1,1,k) = 0.0
    deep_tops(1,1,k) = 0.0
  END DO

  ! Note always require mass flux up and down as not just required in
  ! atmos_physics2 for output diagnostics

  ALLOCATE ( up_flux(tdims%i_start:tdims%i_end,           &
                     tdims%j_start:tdims%j_end,           &
                                 1:tdims%k_end) )
  ALLOCATE ( dwn_flux(tdims%i_start:tdims%i_end,          &
                      tdims%j_start:tdims%j_end,          &
                                  1:tdims%k_end) )

  DO k = 1, tdims%k_end
    DO j = tdims%j_start,tdims%j_end
      DO i = tdims%i_start,tdims%i_end
        up_flux(i,j,k)=0.0
        dwn_flux(i,j,k) = 0.0
      END DO
    END DO
  END DO

  ALLOCATE ( entrain_up(tdims%i_start:tdims%i_end,        &
                        tdims%j_start:tdims%j_end,        &
                                    1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        entrain_up(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( detrain_up(tdims%i_start:tdims%i_end,        &
                        tdims%j_start:tdims%j_end,        &
                                    1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        detrain_up(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( entrain_dwn(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        entrain_dwn(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( detrain_dwn(tdims%i_start:tdims%i_end,        &
                         tdims%j_start:tdims%j_end,        &
                                     1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        detrain_dwn(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( uw_dp(tdims%i_start:tdims%i_end,        &
                   tdims%j_start:tdims%j_end,        &
                               1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        uw_dp(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( vw_dp(tdims%i_start:tdims%i_end,        &
                   tdims%j_start:tdims%j_end,        &
                               1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        vw_dp(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( uw_shall(tdims%i_start:tdims%i_end,        &
                      tdims%j_start:tdims%j_end,        &
                                  1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        uw_shall(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( vw_shall(tdims%i_start:tdims%i_end,        &
                      tdims%j_start:tdims%j_end,        &
                                  1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        vw_shall(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( uw_mid(tdims%i_start:tdims%i_end,        &
                    tdims%j_start:tdims%j_end,        &
                                1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        uw_mid(i,j,k) = 0.0
      END DO
    END DO
  END DO
  ALLOCATE ( vw_mid(tdims%i_start:tdims%i_end,        &
                    tdims%j_start:tdims%j_end,        &
                                1:tdims%k_end) )
  DO k=1,tdims%k_end
    DO j=tdims%j_start,tdims%j_end
      DO i= tdims%i_start,tdims%i_end
        vw_mid(i,j,k) = 0.0
      END DO
    END DO
  END DO

END SELECT ! model_type


! inhomogenous diagnostics

IF (l_qcl_incr_cinh) THEN

  ALLOCATE (qcl_incr_inhom_diag(tdims%i_start:tdims%i_end,        &
                                tdims%j_start:tdims%j_end,        &
                                             1:tdims%k_end) )

  ! Hold input qcl increment (almost certainly zero)
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, rows, row_length, qcl_incr_inhom_diag )           &
!$OMP PRIVATE( i, j, k )
  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        qcl_incr_inhom_diag(i,j,k) = 0.0
      END DO ! i
    END DO ! j
  END DO ! k
!$OMP END PARALLEL DO
ELSE
  ALLOCATE (qcl_incr_inhom_diag(1,1,1))
END IF                   ! on STASHflag

IF (l_qcf_incr_cinh) THEN

  ALLOCATE (qcf_incr_inhom_diag(tdims%i_start:tdims%i_end,        &
                                tdims%j_start:tdims%j_end,        &
                                             1:tdims%k_end) )
  ! Hold input qcf increment (almost certainly zero)
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, rows, row_length, qcf_incr_inhom_diag )  &
!$OMP PRIVATE( i, j, k )
  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        qcf_incr_inhom_diag(i,j,k) = 0.0
      END DO ! i
    END DO ! j
  END DO ! k
!$OMP END PARALLEL DO
ELSE
  ALLOCATE (qcf_incr_inhom_diag(1,1,1))

END IF                   ! on STASHflag

IF (l_bcf_incr_cinh) THEN

  ALLOCATE (bulk_cf_incr_inhom_diag(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                                1:tdims%k_end) )

  ! Hold input bulk_cf increment (almost certainly zero)
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, rows, row_length, bulk_cf_incr_inhom_diag )       &
!$OMP PRIVATE( i, j, k )
  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        bulk_cf_incr_inhom_diag(i,j,k) = 0.0
      END DO ! i
    END DO ! j
  END DO ! k
!$OMP END PARALLEL DO
ELSE
  ALLOCATE (bulk_cf_incr_inhom_diag(1,1,1))

END IF                   ! on STASHflag

IF (l_cfl_incr_cinh) THEN

  ALLOCATE (cf_liquid_incr_inhom_diag(tdims%i_start:tdims%i_end,        &
                                      tdims%j_start:tdims%j_end,        &
                                                  1:tdims%k_end) )
  ! Hold input bulk_cf increment (almost certainly zero)
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, rows, row_length, cf_liquid_incr_inhom_diag )     &
!$OMP PRIVATE( i, j, k )
  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        cf_liquid_incr_inhom_diag(i,j,k) = 0.0
      END DO ! i
    END DO ! j
  END DO ! k
!$OMP END PARALLEL DO
ELSE
  ALLOCATE (cf_liquid_incr_inhom_diag(1,1,1))

END IF                   ! on STASHflag

IF (l_cff_incr_cinh) THEN

  ALLOCATE (cf_frozen_incr_inhom_diag(tdims%i_start:tdims%i_end,        &
                                      tdims%j_start:tdims%j_end,        &
                                                  1:tdims%k_end) )

  ! Hold input bulk_cf increment (almost certainly zero)
!$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP SHARED( tdims, rows, row_length, cf_frozen_incr_inhom_diag )     &
!$OMP PRIVATE( i, j, k )
  DO k=1,tdims%k_end
    DO j=1,rows
      DO i=1,row_length
        cf_frozen_incr_inhom_diag(i,j,k) = 0.0
      END DO ! i
    END DO ! j
  END DO ! k
!$OMP END PARALLEL DO
ELSE
  ALLOCATE (cf_frozen_incr_inhom_diag(1,1,1))

END IF                   ! on STASHflag


!------------------------------------------------------------------------------

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_out,zhook_handle)

!-------------------------------------------------------------------------------
RETURN
END SUBROUTINE cv_alloc_diag_array
END MODULE cv_alloc_diag_array_mod

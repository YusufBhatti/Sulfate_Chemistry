...
MODULE les_shall_func_thlev_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'LES_SHALL_FUNC_THLEV_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='LES_SHALL_FUNC_THLEV'
...
DO k =1,max_cldlev
  DO i = 1,n_sh
    IF (eta(i,k) <= 1.0) THEN    ! in cloud
      wexp10(i,k) = 1.0 - EXP(-10.0*eta(i,k))
      wexp6(i,k)  = EXP(-6.0*eta(i,k))

      g_func(i,k)   = 1.45*(eta(i,k) -0.665*eta(i,k)*eta(i,k)     &
                    + (gg1 - gg2*eta(i,k))*wexp6(i,k) -gg1)

      k_func(i,k)   = 0.115*(1.0 + eta(i,k))*wexp10(i,k)

      f0_func(i,k)  = 1.0 -(0.473*fob_fot(i)+fob(i))*eta(i,k)      &
                   + cc1*fob_fot(i)*(1.0-EXP(-0.64*eta(i,k)))

      f1_func(i,k)  = 9.0*(eta(i,k)**1.5)

      ftheta_func(i,k)  = 2.25 - 1.75*EXP(-4.0*eta(i,k))
...
DO k = 1,rlev
  eta_reg(k) = k*inc_eta
  temp = (1.0+2.0*eta_reg(k))*(1.0+2.0*eta_reg(k))
...
DO k =1,max_cldlev
  DO i = 1,n_sh
    IF (eta(i,k) <= 1.0) THEN  ! in cloud
      ...
      temp = (1+2.0*eta(i,k))*(1+2.0*eta(i,k))
      dfw_at_eta = 20.0*(1.0-EXP(-5.0*eta(i,k)))/temp  -1.0
...
END MODULE les_shall_func_thlev_mod

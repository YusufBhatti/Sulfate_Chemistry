...
MODULE deep_turb_conv_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DEEP_TURB_CONV_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY: cp, c_virtual, r, g, planet_radius
...
USE dts_cape_mod, ONLY: dts_cape
USE dts_cond_and_dep_mod, ONLY: dts_cond_and_dep
USE dts_conv_classify_mod, ONLY: dts_conv_classify
USE dts_deduce_thetaandqv_mod, ONLY: dts_deduce_thetaandqv
USE dts_dthvdz_mod, ONLY: dts_dthvdz
USE dts_evaporation_mod, ONLY: dts_evaporation
USE dts_flux_par_mod, ONLY: dts_flux_par
USE dts_locate_closest_levels_mod, ONLY: dts_locate_closest_levels
USE dts_melt_mod, ONLY: dts_melt
USE dts_mseflux_mod, ONLY: dts_mseflux
USE dts_pc2_mod, ONLY: dts_pc2
USE dts_qflux_mod, ONLY: dts_qflux
USE dts_rainprod_mod, ONLY: dts_rainprod
USE dts_sublimation_mod, ONLY: dts_sublimation
USE dts_update_mod, ONLY: dts_update
USE dts_w_variance_mod, ONLY: dts_w_variance
USE dts_wthv_mod, ONLY: dts_wthv
IMPLICIT NONE
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEEP_TURB_CONV'
...
t40(1) = tm-38.0 !nb or should it actually be -38C?
...
DO i = 1,n_dp
  IF (dts_ntpar(i) < 5) THEN
    WRITE(umMessage,'(A18,2i2,A12,4i3)')                    &
         ' deep turb alison ',CALL_number,i,' ntml ntpar ', &
         klcl(i),dts_ntpar(i),ntml(i),ntpar(i)
    CALL umPrint(umMessage,src='dts_deep_turb_conv')
...
DO i = 1,n_dp
  ...
  IF (temp1(i)  <   qMinInColumn(i)) THEN
    ...
    WRITE(umMessage,'(A16,3i3,6F15.8)')                           &
         ' negative q deep',i,k,kterm(i),temp1(i),dqbydt(i,k),    &
               w_max(i),mb(i),rain(i),snow(i)
    CALL umPrint(umMessage,src='dts_deep_turb_conv')

    WRITE(umMessage,'(A6,F15.8)') ' dq ',   (dqbydt(i,j),j=1,nlev)
    CALL umPrint(umMessage,src='dts_deep_turb_conv')
    WRITE(umMessage,'(A6,F15.8)') ' dqcl ', (dqclbydt(i,j),j=1,nlev)
    CALL umPrint(umMessage,src='dts_deep_turb_conv')
    WRITE(umMessage,'(A6,F15.8)') ' dqcf ', (dqcfbydt(i,j),j=1,nlev)
    CALL umPrint(umMessage,src='dts_deep_turb_conv')
...
IF (model_type == mt_single_column) THEN
  ...
  r_a2 = 1.0/(planet_radius*planet_radius)
  DO i = 1,n_dp
    temp1(i)= 0.0
  END DO!
  ...
  DO k = 1,nlev
    DO i = 1,n_dp
      dqt = dqbydt(i,k)+dqclbydt(i,k)+dqcfbydt(1,k)
      temp1(i) = temp1(i) + dqt*r2rho_th(i,k)*dr_across_th(i,k)
    END DO
  END DO
  DO i = 1,n_dp
    temp1(i)= temp1(i)*r_a2

    WRITE(umMessage,'(A19,i2,3F15.8)')                              &
         'location int moist ',i,temp1(i),rain(i),snow(i)
    CALL umPrint(umMessage,src='dts_deep_turb_conv')
...
DO i=1, n_dp
  ...
  IF ((rain(i) + snow(i)) > 0.0) THEN
    IF (land_mask(i)) THEN
      cca_2d(i)  = a_land + b_land * LOG(rsec_per_day*(rain(i)+snow(i)))
    ELSE
      cca_2d(i)  = a_sea  + b_sea  * LOG(rsec_per_day*(rain(i)+snow(i)))
    END IF
  END IF

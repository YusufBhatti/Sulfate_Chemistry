...
!
MODULE lift_par_5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'LIFT_PAR_5A_MOD'
CONTAINS

SUBROUTINE lift_par_5a (npnts,np_full,                              &
...
USE planet_constants_mod, ONLY: cp, repsilon, kappa, rv
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='LIFT_PAR_5A'
...
DO i=1,npnts
  ...
  ekp14_plus1 = 1.0+ekp14(i)
  repss(i) = 1.0 / ((ekp14_plus1)*(1.0+ekp34(i)))

  thpkp1(i) = ( thpk(i)+ ekp14(i)*thek(i) + ekp34(i)*(ekp14_plus1)*thekp1(i)) &
               *repss(i)

  qpkp1(i) = (  qpk(i) + ekp14(i)*qek(i) + ekp34(i)*(ekp14_plus1)*qekp1(i) )  &
               *repss(i)

  IF (l_q_interact) THEN

    qclpkp1(i)= ( qclpk(i)                                                   &
                  + ekp14(i)*qclek(i) + ekp34(i)*(ekp14_plus1)*qclekp1(i)    &
                    ) * repss(i)

    qcfpkp1(i)= ( qcfpk(i)                                                   &
                  + ekp14(i)*qcfek(i) + ekp34(i)*(ekp14_plus1)*qcfekp1(i)    &
                    ) * repss(i)
  ...
  ELSE ! L_q_interact_if1
    ...
    IF ( bwkp1(i) .AND. .NOT. bwk(i) ) THEN
      thpkp1(i) = thpkp1(i) - lfrcp * xpk(i) / exkp1(i)
...
IF (l_mom_gk) THEN   ! l_mom_gk set according to type of CMT required
  ...
  DO i=1,npnts
    ...
    ekp14_plus1 = 1.0+ekp14(i)
    rekp34_plus1 = 1.0/(1.0+ekp34(i))
    upkp1(i) = (upk(i)+                                             &
                ekp14(i)*uek(i) + ekp34(i)*(ekp14_plus1)*uekp1(i)   &
                  ) *repss(i)

    vpkp1(i) = (vpk(i)+                                             &
                ekp14(i)*vek(i) + ekp34(i)*(ekp14_plus1)*vekp1(i)   &
                  ) *repss(i)
    upkp1(i) = upkp1(i) - (0.7*(uek(i)-uekp1(i))*rekp34_plus1)
    vpkp1(i) = vpkp1(i) - (0.7*(vek(i)-vekp1(i))*rekp34_plus1)
...
IF ( l_new_qsat_conv ) THEN
  CALL qsat_new(qspkp1,tt,pkp1,npnts)
ELSE
  ! DEPENDS ON: qsat
...
RETURN
END SUBROUTINE lift_par_5a
END MODULE lift_par_5a_mod

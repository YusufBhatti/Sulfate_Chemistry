...
!
MODULE detrain_4a5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DETRAIN_4A5A_MOD'
CONTAINS
...
USE det_rate_4a5a_mod, ONLY: det_rate_4a5a
USE thetar_4a5a_mod, ONLY: thetar_4a5a
USE thp_det_4a5a_mod, ONLY: thp_det_4a5a
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DETRAIN_4A5A'
...
IF ( l_new_qsat_conv ) THEN
  CALL qsat_new(xsqkp1,tt,pkp1,npnts)
ELSE
  ! DEPENDS ON: qsat
...
DO i=1,npnts
  ...
  IF ( bgmk(i) .OR. ( (xsqkp1(i)  >   0.0) .AND. brecal(i) ) ) THEN
    bgmkp1(i) = .TRUE.
  ELSE
    bgmkp1(i) = .FALSE.
    xsqkp1(i) = 0.0

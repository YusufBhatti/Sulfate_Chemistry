...
!
MODULE convec2_4a5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'CONVEC2_4A5A_MOD'
CONTAINS
...
USE ukca_option_mod,     ONLY:                                        &
    l_ukca, l_ukca_plume_scav

USE ukca_scavenging_mod, ONLY:                                        &
    ukca_plume_scav

USE yomhook, ONLY: lhook, dr_hook
USE parkind1, ONLY: jprb, jpim

USE environ_mod, ONLY: environ
USE parcel_4a5a_mod, ONLY: parcel_4a5a
IMPLICIT NONE
...
INTEGER, INTENT(IN) :: &
  n_wups                 ! Sets dimension arrays for w-eqn diags

LOGICAL, INTENT(IN) :: &
  l_tracer             & ! Switch for tracers
 ,l_mom_gk             & ! Gregory-Kershaw convective momentum transport switch
 ,l_mom_gk_stable      & ! Stabilized Gregory-Kershaw 
                         ! convective momentum transport switch
...
REAL, INTENT(IN) :: zkm1  (n_wups)       ! ...k-1   [m]
REAL, INTENT(IN) :: zk    (n_wups)       ! ...k     [m]
REAL, INTENT(IN) :: zkp12 (n_wups)       ! ...k+1/2 [m]
REAL, INTENT(IN) :: zkp1  (n_wups)       ! ...k+1   [m]
...
REAL, INTENT(INOUT) :: &
 ...
 ,trapkp1(np_full,ntra)& ! IN  parcel tracer content in layer k+1 (kg/kg)
                         ! OUT - not required, but modified by ukca_plume_scav
...
REAL, INTENT(IN)    :: w2p_km1 (n_wups)   ! ...layer centre k-1 [(m/s)^2]
REAL, INTENT(INOUT) :: w2p_k   (n_wups)   ! ...layer centre k   [(m/s)^2]
REAL, INTENT(OUT)   :: w2p_kp1 (n_wups)   ! ...layer centre k+1 [(m/s)^2]
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='CONVEC2_4A5A'

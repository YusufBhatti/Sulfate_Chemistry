...
!
MODULE shallow_conv_5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'SHALLOW_CONV_5A_MOD'
CONTAINS
...
SUBROUTINE shallow_conv_5a(nbl,nlev,ntra,n_cca_lev,n_sh,trlev,    &
                       ...
                       r_theta,r_rho,rho_theta,                   &
                       rho_dry_theta, rho, rho_dry,               &
                       r2rho_th,r2rho,                            &
...
USE cv_run_mod, ONLY:                                             &
    ...
    cape_timescale, sh_pert_opt, bl_cnv_mix, icvdiag,             &
    cca2d_sh_opt, cca_sh_knob, ccw_sh_knob, limit_pert_opt,       &
    cnv_wat_load_opt, l_ccrad, l_eman_dd, l_new_dd, l_snow_rain,  &
    cldbase_opt_sh
...
USE cv_dependent_switch_mod, ONLY:                                &
    sh_on, mdet_sh_on, sh_ent_on, sh_sdet_on, sh_new_termc
...
REAL, INTENT(IN)    ::      &
 ...
 ,rho_theta(n_sh,nlev)      & ! wet density on theta levels (kg/m3)
 ,rho_dry_theta(n_sh,nlev)  & ! dry density on theta levels (kg/m3)
 ,rho(n_sh,nlev)            & ! wet density on rho levels (kg/m3)
...
REAL :: dcpbydt_term(n_sh)      ! Rate of change of cape (J/kg/s)
...
! Oriignal downdraught scheme variables
...
! This routine is shallow.
INTEGER, PARAMETER ::  shallow = 3 
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SHALLOW_CONV_5A'
...
! Initalisations required by some SCM configurations
DO i=1, n_sh
  depth(i)    = 0.0
END DO
DO k=1, nlev
...
DO i = 1,n_sh
  scale_f(i) = 0.0     ! Need to initialise
END DO
...
DO i = 1,n_sh
  IF (kterm(i)  >=  ntml(i)) THEN   ! actual shallow convection
    ...
    IF (cldbase_opt_sh == sh_grey_closure) THEN
      ...
      IF (dcpbydt_term(i) > 0.0 .AND. z_scale > 0.0 ) THEN
        ...
        flx_init_new(i) = flx_init_new(i)*MAX( 1.0,                     &
                    cape_out(i)/(cape_timescale*dcpbydt_term(i)) )

        IF (z_scale < 1.0) THEN
          !         ! Take linear sum of shallow (flux_init_term) and
          !         ! deep (flux_init_new) closures
                    ! [zscale varies between 0 and 1]
          flx_init_new(i)= z_scale * flx_init_new(i) +                  &
                     (1.0-z_scale) * flx_init_term(i)
        END IF
...
IF (l_safe_conv) THEN
  ...
  DO i = 1,n_sh
    ...
    IF (scale_f(i) < 1.0e-6 .OR. kterm(i) <= ntml(i)) THEN
...
END IF      ! l_ccrad
!-----------------------------------------------------------------------
! 5.0 Downdraughts and evaporation  - now 3 options
!                     (a) Emanuel downdraught scheme
!                     (b) Original mass flux code
!                     (c) New mass flux scheme 
! Note the level at which shallow convection terminates has been stored
...
IF (l_eman_dd .AND. l_snow_rain) THEN
  ! (a) Use revise Emanuel downdraughts
  IF (n_real_sh > 0) THEN   ! only call if real shallow convection
...
ELSE
  ...
  ELSE
  !-----------------------------------------------------------------------
  ! (b) Original mass flux scheme 
  ! 5.1 All shallow convection will terminate at some level. This level
  ...
  !-----------------------------------------------------------------------
    ...
    DO i = 1,n_sh
      IF (kterm(i) >= ntml(i)) THEN
        ...
        IF (deltap_cld >  15000.0 .AND. bgmk_term(i)                      &
    ...
    !-----------------------------------------------------------------------
    ! 5.2  Downdraught calculation - on all points where convection is
    !      terminating.
    !
    ...
    !-----------------------------------------------------------------------
    ! 5.3 Surface precipitation calculation for terminating points with
...
DO i = 1,n_sh
  ...
  IF (temp1(i)  <   qminincolumn(i) .AND.                            &
    ...
    CALL umPrint(umMessage,src='shallow_conv-shconv5a')
  END IF
END DO ! n_sh loop
!-----------------------------------------------------------------------
...
ELSE
  ...
  CALL mix_inc (n_sh,n_sh,n_real_sh,nlev,nbl,ntml,                  &
...
END MODULE shallow_conv_5a_mod

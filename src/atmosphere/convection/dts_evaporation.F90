...
MODULE dts_evaporation_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_EVAPORATION_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY: cp, r
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_EVAPORATION'
  ...
  DO i_dp=1,n_dp
    ...
    IF (z_theta(i_dp,k) < ztop(i_dp) .AND.                             &
      ...
      IF (temperature(i_dp,k) < tm .AND. temperature(i_dp,k) > tm-45.0) THEN
        ...
        zsc = zsc*EXP(-0.5*(3.0*(tm-temperature(i_dp,k))/40.0)**2)
      ...
      ELSE
        evaporation(i_dp,k) = zsc*frac*gamma_evp*(wcld(i_dp)/(10000.0))    &
                              *(qsat_moist_ad(i_dp,k)-q(i_dp,k)) /        &
                                (1.0+(lc/cp)*dqsedt(i_dp,k))
      END IF
...
END MODULE dts_evaporation_mod

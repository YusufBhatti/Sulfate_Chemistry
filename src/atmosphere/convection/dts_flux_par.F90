...
MODULE dts_flux_par_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'DTS_FLUX_PAR_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_FLUX_PAR'
...
DO i_dp=1,n_dp
  IF (cape_below_fr(i_dp) > smallcape) THEN
    wcld(i_dp) = (mb(i_dp)*cape_below_fr(i_dp))**(1.0/3.0)
...
DO i_dp=1,n_dp
  IF (cape_whole_layer(i_dp) > smallcape) THEN
    wall(i_dp) = (mb(i_dp)*cape_whole_layer(i_dp))**(1.0/3.0)
  ELSE
    wall(i_dp) = 0.0
  END IF
END DO
...
DO i_dp=1,n_dp
  IF (cape_above_fr(i_dp) > smallcape) THEN
    wfr(i_dp) = (mfr(i_dp)*cape_above_fr(i_dp))**(1.0/3.0)
  ELSE
    wfr(i_dp) = 0.0
  END IF
END DO

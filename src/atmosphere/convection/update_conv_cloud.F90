...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='UPDATE_CONV_CLOUD_MOD'
...
REAL, INTENT(IN) ::                                                 &
 ...
 ,it_ccw(row_length,rows,model_levels)     &! Cnv.Cld Water (kg/kg)
 ...
 ,it_ccw0(row_length,rows,model_levels)    &! Cnv.Cld Water (kg/kg)
...
REAL, INTENT(INOUT) ::                   &
 ...
 ,ccw(row_length,rows,model_levels)      &! Cnv.Cld Water (kg/kg)
 ...
 ,ccw0(row_length,rows,model_levels)     &! Cnv.Cld Water (kg/kg)
 ...
 ,ccw0_local(row_length,rows,model_levels)  &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='UPDATE_CONV_CLOUD'
...
!$OMP  PARALLEL DEFAULT(NONE) PRIVATE(k, i, j, decay_now)                    &
!$OMP& SHARED(rows, row_length,                                              &
!$OMP&    n_cca_lev, ndecay_steps, n_conv_calls, loop_number,                &
!$OMP&    it_ccb, it_cct, it_lcbase, it_lctop, it_ccb0, it_cct0, it_lcbase0, &
!$OMP&    one_over_conv_calls, timestep, decay_time,                         &  
!$OMP&    p_layer_boundaries, cld_life_3d, g,                                &
!$OMP&    it_cca_2d, it_lcca, it_cca, it_ccw, it_cclwp,                      &
!$OMP&    it_cca0_2d, it_cca0,it_ccw0,                                       &
!$OMP&    it_cca0_dp, it_cca0_md, it_cca0_sh,                                &
!$OMP&    ccb, cct, lcbase,lctop, ccb0, cct0, lcbase0,                       &
!$OMP&    ccb0_local, cct0_local,lcbase0_local,                              &
!$OMP&    lcca,  cca_2d,   cclwp,  cca,  ccw,                                &
!$OMP&    cca0_2d,  cclwp0, cca0, ccw0,                                      &
!$OMP&    cca0_dp, cca0_md, cca0_sh,                                         &
!$OMP&    cca0_local_dp, cca0_local_md, cca0_local_sh,                       &
!$OMP&    it_cclwp0, cclwp0_local, cca0_local, ccw0_local,                   &
!$OMP&    rad_cloud_decay_opt,     cld_life_opt, cca_min,                    &
!$OMP&    fixed_cld_life, l_dcpl_cld4pc2, l_ccrad,                           &
!$OMP&    model_levels, l_cca_dp_prog, l_cca_md_prog, l_cca_sh_prog)
...
!$OMP DO SCHEDULE(STATIC)  
    DO k=1, n_cca_lev
      DO j=1, rows
        DO i=1, row_length
          cca0_dp(i,j,k) = cca0_dp(i,j,k)                         &
                    + one_over_conv_calls * it_cca0_dp(i,j,k)
          cca0_dp(i,j,k) = MIN(cca0_dp(i,j,k), 1.0)
        END DO
      END DO
    END DO
!$OMP END DO NOWAIT
...
!$OMP DO SCHEDULE(STATIC)  
    DO k=1, n_cca_lev
      DO j=1, rows
        DO i=1, row_length
          cca0_md(i,j,k) = cca0_md(i,j,k)                         &
                    + one_over_conv_calls * it_cca0_md(i,j,k)
          cca0_md(i,j,k) = MIN(cca0_md(i,j,k), 1.0)
        END DO
      END DO
    END DO
!$OMP END DO NOWAIT
...
!$OMP DO SCHEDULE(STATIC)  
    DO k=1, n_cca_lev
      DO j=1, rows
        DO i=1, row_length
          cca0_sh(i,j,k) = cca0_sh(i,j,k)                         &
                    + one_over_conv_calls * it_cca0_sh(i,j,k)
          cca0_sh(i,j,k) = MIN(cca0_sh(i,j,k), 1.0)
        END DO
      END DO
    END DO
!$OMP END DO NOWAIT
...
!$OMP DO SCHEDULE(STATIC)  
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            cca0_local_dp(i,j,k) = it_cca0_dp(i,j,k)
          END DO
        END DO
      END DO
!$OMP END DO NOWAIT
...
!$OMP DO SCHEDULE(STATIC)  
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            cca0_local_md(i,j,k) = it_cca0_md(i,j,k)
          END DO
        END DO
      END DO
!$OMP END DO NOWAIT
...
!$OMP DO SCHEDULE(STATIC)  
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            cca0_local_sh(i,j,k) = it_cca0_sh(i,j,k)
          END DO
        END DO
      END DO
!$OMP END DO NOWAIT
...
!$OMP DO SCHEDULE(STATIC)  
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            cca0_local_dp(i,j,k) = cca0_local_dp(i,j,k)           &
              + one_over_conv_calls * it_cca0_dp(i,j,k)
          END DO
        END DO
      END DO
...
!$OMP DO SCHEDULE(STATIC)  
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            cca0_local_md(i,j,k) = cca0_local_md(i,j,k)           &
              + one_over_conv_calls * it_cca0_md(i,j,k)
          END DO
        END DO
      END DO
...
!$OMP DO SCHEDULE(STATIC)  
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            cca0_local_sh(i,j,k) = cca0_local_sh(i,j,k)           &
              + one_over_conv_calls * it_cca0_sh(i,j,k)
          END DO
        END DO
      END DO
      ...
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            ...
            cca0_dp(i,j,k) = MAX( cca0_dp(i,j,k)                &
                                 * (1.0-cld_life_3d(i,j,k))     &
                             , cca0_local_dp(i,j,k) )

            cca0_dp(i,j,k) = MIN( cca0_dp(i,j,k), 1.0 )

            IF (cca0_dp(i,j,k) < cca_min) THEN
              cca0_dp(i,j,k) = 0.0
            END IF
      ...
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            ...
            cca0_md(i,j,k) = MAX( cca0_md(i,j,k)                &
                                 * (1.0-cld_life_3d(i,j,k))     &
                             , cca0_local_md(i,j,k) )

            cca0_md(i,j,k) = MIN( cca0_md(i,j,k), 1.0 )

            IF (cca0_md(i,j,k) < cca_min) THEN
              cca0_md(i,j,k) = 0.0
            END IF
      ...
      DO k=1, n_cca_lev
        DO j=1, rows
          DO i=1, row_length
            ...
            cca0_sh(i,j,k) = MAX( cca0_sh(i,j,k)                &
                                 * (1.0-cld_life_3d(i,j,k))     &
                             , cca0_local_sh(i,j,k) )

            cca0_sh(i,j,k) = MIN( cca0_sh(i,j,k), 1.0 )

            IF (cca0_sh(i,j,k) < cca_min) THEN
              cca0_sh(i,j,k) = 0.0
            END IF
...
!$OMP BARRIER
    ...
    IF (l_dcpl_cld4pc2) THEN
      ...
      !-------------------------------------------------
!$OMP DO SCHEDULE(STATIC)  
      ! The first index of the loop is "j" so that it can be
...
!-------------------------------------------------------------------------------
END SUBROUTINE update_conv_cloud

END MODULE update_conv_cloud_mod

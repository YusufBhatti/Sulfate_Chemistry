...
SUBROUTINE glue_conv_6a(np_field,npnts,nlev,nbl,call_number,seg_num  &
   ...
   ,                 entrain_coef,conv_prog_precip                   &
   ,                 deep_flag,past_precip,past_conv_ht,cape_out     &
   ,                 n_dp,n_cg, n_sh, n_md                           &
...
USE cv_run_mod,  ONLY:                                            &
   ...
   l_3d_cca,     l_conv_hist,  l_conv_prog_precip,                &
   l_new_dd,     l_pc2_diag_sh
...
REAL, INTENT(IN) :: exner_rho_levels(np_field,nlev)   !Exner on rho levels
...
REAL, INTENT(IN) :: conv_prog_precip(np_field,nlev) ! Surface precipitation
                                                    ! based 3d convective 
...
REAL :: conv_prog_precip_dp(n_dp,nlev)
...
REAL, ALLOCATABLE ::        &
 ...
 ,du_dd_temp(:,:)           & ! du from DD
 ,dv_dd_temp(:,:)           & ! dv from DD
...
! Surface precipitation based 3d convective prognostic in kg/m2/s
REAL :: conv_prog_precip_cg(n_cg,nlev)
...
! Surface precipitation based 3d convective prognostic in kg/m2/s
REAL :: conv_prog_precip_sh(n_sh,nlev)
...
! Surface precipitation based 3d convective prognostic in kg/m2/s
REAL :: conv_prog_precip_md(n_md,nlev)
...
IF (l_scm_convss_dg) THEN
  ...
  ! Allocate the diagnostic arrays inside the structures
  DO j = 1, MAX(n_dp, n_sh, n_md)
    CALL scm_convss_dg_allocate( scm_convss_dg_c(j) )
  END DO
...
IF (n_dp  >   0 .AND. iconv_deep >  0 ) THEN
  ...
  ! SCM convective substep diagnostics:
  IF ( l_scm_convss_dg ) THEN
    ! Copy in the data from input, which is already initialised to zero
    ! (this needs to be done so that the diags are zeroed on levels
    !  where convection doesn't act)
    DO j = 1, n_dp
      scm_convss_dg_c(j) = scm_convss_dg(dpi(j))
    END DO
  END IF
  ...
  IF (iconv_deep == 1) THEN  ! 4A like Gregory Rowntree deep conv
    ...
    CALL deep_conv_6a(                                  &
       ...
       qcf_dp,qcl_dp,tracer_dp,w2p_dp,                  &
       scm_convss_dg_c(1:n_dp),                         &
                            !OUT
...
!dir$ ivdep
  ...
  ! Copy data back from compressed array structure for SCM
  ! convection sub-step diagnostics, if used
  IF ( l_scm_convss_dg ) THEN
    ! Copy diagnostics back into full array
    DO j = 1, n_dp
      scm_convss_dg(dpi(j)) = scm_convss_dg_c(j)
    END DO
  END IF
...
IF (n_sh  >   0 .AND. iconv_shallow >  0) THEN
  ...
  IF ( l_scm_convss_dg ) THEN
    ...
    !  where convection doesn't act)
    DO j = 1, n_sh
      scm_convss_dg_c(j) = scm_convss_dg(shi(j))
    END DO
  END IF
...
!dir$ ivdep
  ...
  IF ( l_scm_convss_dg ) THEN
    ! Copy diagnostics back into full array
    DO j = 1, n_sh
      scm_convss_dg(shi(j)) = scm_convss_dg_c(j)
    END DO
  END IF
  ...
  IF (flg_up_flx_half .AND. iconv_shallow == 1) THEN
    ...
    END DO
  END IF
  ! Downdraught mass flux only available from shallow for new DD scheme
...
IF (iconv_mid >  0 .AND. n_md > 0) THEN
  ...
  DO i = 1,n_md
      ...
      ! above where shallow, congestus or deep has terminated (in this
      ! case ntml<kterm_* by definition).
    midtrig(i) = MAX( ntml(mdi(i)), kterm_shall(mdi(i)),              &
                      kterm_congest(mdi(i)), kterm_deep(mdi(i)) ) + 1
  END DO
  ...
  IF ( l_scm_convss_dg ) THEN
    ...
    !  where convection doesn't act)
    DO j = 1, n_md
      scm_convss_dg_c(j) = scm_convss_dg(mdi(j))
    END DO
  END IF
  ...
  IF ( l_scm_convss_dg ) THEN
    ! Copy diagnostics back into full array
    DO j = 1, n_md
      scm_convss_dg(mdi(j)) = scm_convss_dg_c(j)
    END DO
  END IF
...
! Deallocate compressed array for SCM convection substep diagnostics
IF (l_scm_convss_dg) THEN

  ! Deallocate the fields contained in the structures
  DO j = 1, MAX(n_dp, n_sh, n_md)
    CALL scm_convss_dg_deallocate( scm_convss_dg_c(j) )
  END DO

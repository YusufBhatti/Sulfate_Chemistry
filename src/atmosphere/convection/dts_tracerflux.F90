...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='DTS_TRACERFLUX_MOD'
...
USE tridiag_all_n_mod, ONLY: tridiag_all_n
IMPLICIT NONE
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DTS_TRACERFLUX'
...
IF (trlev /= nlev) THEN
  WRITE(umMessage,*) 'WARNING: trlev != nlev',trlev,nlev
  CALL umPrint(umMessage,src='dts_tracerflux')
END IF

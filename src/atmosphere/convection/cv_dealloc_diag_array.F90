...
!
MODULE cv_dealloc_diag_array_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'CV_DEALLOC_DIAG_ARRAY_MOD'
CONTAINS
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='CV_DEALLOC_DIAG_ARRAY'
...
! Note: theta_incr_diag_conv not deallocated here as is needed later
! in atmos_physics2.
...
! Could in future become diagnostics
IF (l_check_moist_inc) THEN
  DEALLOCATE(ep3)
  DEALLOCATE(ep2)
  DEALLOCATE(ep1)
END IF
...
END MODULE cv_dealloc_diag_array_mod

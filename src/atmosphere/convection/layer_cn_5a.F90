...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'LAYER_CN_MOD'
CONTAINS
...
USE cv_run_mod, ONLY:                                            &
    ...
    ent_dp_power, ent_md_power, cldbase_opt_sh

USE cv_param_mod, ONLY: ae2, sh_grey_closure, entcoef

USE cv_dependent_switch_mod, ONLY:                               &
    l_var_entrain, l_new_det, l_const_ent
...
,conv_indicator            ! type of convection for layer calls.
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='LAYER_CN'
...
!      write(6,*) ' layer_cn', conv_indicator
...
IF (conv_indicator == shallow) THEN
  ...
  ELSE             ! original code
    DO i=1,npnts
      ...
      END IF   ! type of convection and level
    END DO
  END IF      ! l_var_entrain and cldbase_opt_sh
...
ELSE IF (conv_indicator == congestus) THEN
  ...
  !-----------------------------------------------------------------------
ELSE IF (conv_indicator == deep) THEN ! Deep
  !-----------------------------------------------------------------------

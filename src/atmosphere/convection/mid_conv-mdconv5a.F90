...
!
MODULE mid_conv_5a_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'MID_CONV_5A_MOD'
CONTAINS

SUBROUTINE  mid_conv_5a(nbl,nlev,ntra,n_cca_lev,npnts,trlev,      &
                       ...
                       q,q1_sd,t1_sd,th,                          &
                       timestep,u,v,w,recip_pstar,qse,            &
...
USE cv_run_mod, ONLY:                                             &
    l_mom, l_eman_dd, l_new_dd, l_safe_conv, l_cv_conserve_check, &
    cldbase_opt_md, cape_min,                                     &
    w_cape_limit, cape_timescale, mid_cmt_opt,                    &
    mid_cnv_pmin, cca2d_md_opt, cca_md_knob, ccw_md_knob,         &
    l_anvil, cnv_wat_load_opt, qmin_conv, l_ccrad, l_3d_cca
...
USE cv_stash_flg_mod, ONLY:                                       &
    ...
    flg_mf_midlev, flg_w_eqn, flg_area_ud
...
USE convec2_4a5a_mod, ONLY: convec2_4a5a
USE cor_engy_5a_mod, ONLY: cor_engy_5a
USE dd_call_4a5a_mod, ONLY: dd_call_4a5a
USE dqs_dth_mod, ONLY: dqs_dth
USE eman_cex_mod, ONLY: eman_cex
USE evap_bcb_nodd_mod, ONLY: evap_bcb_nodd
USE flag_wet_mod, ONLY: flag_wet
USE layer_cn_mod, ONLY: layer_cn
USE lift_par_5a_mod, ONLY: lift_par_5a
USE mid_conv_dif_cmt_mod, ONLY: mid_conv_dif_cmt
USE mix_inc_mod, ONLY: mix_inc
IMPLICIT NONE
...
REAL, INTENT(IN)    ::  &
 ...
 ,q1_sd(npnts)          & ! Standard deviation of
                          ! turbulent flucts. of layer 1 q (kg/kg)
 ,t1_sd(npnts)          & ! Standard deviation of
...
REAL, INTENT(INOUT) ::   & !
  w2p(npnts,nlev)          ! (Parcel vertical velocity)^2 [(m/s)^2]
...
REAL, INTENT(OUT) ::       &
 ...
 ,up_flux_half(npnts,nlev) & ! Updraught mass flux on rho levels(Pa/s)
...
REAL, INTENT(INOUT) ::                &
  rbuoy_p_out (ScmRowLen*ScmRow,nlev) & !buoyancy excess
 ,the_out     (ScmRowLen*ScmRow,nlev) & !th_E in parcel routine
 ,thp_out     (ScmRowLen*ScmRow,nlev) & !th_P in parcel routine
 ,qe_out      (ScmRowLen*ScmRow,nlev) & !q_E in parcel routine
 ,qp_out      (ScmRowLen*ScmRow,nlev)   !q_P in parcel routine
...
REAL, INTENT(OUT) ::      &
 ...
 ,du_dd(npnts,nlev)       & ! du/dt from DD (m/s/s)
 ,dv_dd(npnts,nlev)       & ! dv/dt from DD (m/s/s)
 ,area_ud(npnts,nlev)     & ! fractional area of updraughts
 ,area_dd(npnts,nlev)       ! fractional area of downdraughts
...
REAL ::                  &
  rbuoy_p_here(npnts)    & ! buoyancy excess
 ,the_here(npnts)        & ! th_E in parcel routine
 ,thp_here(npnts)        & ! th_P in parcel routine
 ,qe_here(npnts)         & ! q_E in parcel routine
 ,qp_here(npnts)         & ! q_P in parcel routine
 ,rbuoy_p_old(npnts)       ! buoyancy excess on previous level

! Height above surface of model levels ...
REAL :: zkm1(npnts)              ! ... k-1   [m]
REAL :: zk(npnts)                ! ... k     [m]
REAL :: zkp12(npnts)             ! ... k+1/2 [m]
REAL :: zkp1(npnts)              ! ... k+1   [m]
...
REAL :: flxkp12(npnts,nlev)     ! Mass flux on half level (Pa/s)
...
REAL :: scale_low(npnts)        ! scaling factor lowest mid-level
...
REAL ::                       &
  wls_mean(npnts)             & ! sum of wls over convecting column
 ,mass_mean(npnts)              ! sum of mass over convecting column
...
! New downdraught and evaporation scheme also uses npossdd 
! to hold the number of real mid-level convecting points with precipitation
! index_possdd  the locations of these points  

REAL ::                   &
  precip_rain(npnts,nlev) & ! Precipitation creation rate for rain (kg/m2/s)
 ,precip_snow(npnts,nlev) & ! Precipitation creation rate for snow (kg/m2/s)
 ,tot_precip(npnts)         ! total precipitation rate in a column (kg/m2/s)
...
REAL :: w2p_km1_c2 (npnts)  ! ...layer centre k-1 [(m/s)^2]
REAL :: w2p_k_c2   (npnts)  ! ...layer centre k   [(m/s)^2]
REAL :: w2p_kp1_c2 (npnts)  ! ...layer centre k+1 [(m/s)^2]

! Compressed arrays for height above surface of model level ...
REAL :: zkm1_c2    (npnts)  ! ...k-1   [m]
REAL :: zk_c2      (npnts)  ! ...k     [m]
REAL :: zkp12_c2   (npnts)  ! ...k+1/2 [m]
REAL :: zkp1_c2    (npnts)  ! ...k+1   [m]

INTEGER :: n_wups           ! Sets size of arrays for
                            ! vertical velocity calculation
                            ! for 4a/5a convection schemes
!====================================
...
REAL ::           &
 ...
 ,qsekp1_c(npnts) & ! Saturation mixing ratio of cld. env. in layer k+1 (kg/kg)
...
REAL ::                  &
  traek_c(npnts,ntra)    & ! Tracer content cld. env. in layer k (kgkg-1)
 ,traek_c2(npnts,ntra)   & ! Tracer content cld. env. in layer k (kgkg-1)
 ,traekp1_c(npnts,ntra)  & ! Tracer content of cloud env. in layer k+1 (kg/kg)
 ,traekp1_c2(npnts,ntra) & ! Tracer content of cloud env. in layer k+1 (kg/kg)
 ,trapk_c(npnts,ntra)    & ! Tracer cont. of parcel in layer k (kg/kg)
 ,trapk_c2(npnts,ntra)   & ! Tracer cont. of parcel in layer k (kg/kg)
 ,trapkp1_c(npnts,ntra)  & ! Tracer cont.of parcel in layer k+1 (kg/kg)
 ,trapkp1_c2(npnts,ntra)   ! Tracer cont.of parcel in layer k+1 (kg/kg)

REAL ::           &
  rbuoy_c(npnts)  & ! Buoyancy of parcel at k+1 (Kelvin) at k+1 (Kelvin)
 ,rbuoy_c2(npnts)   ! Buoyancy of parcel at k+1 (Kelvin) at k+1 (Kelvin)

REAL ::             &
  uek_c(npnts)      & ! Model U field on layer k (m/s)
 ,uek_c2(npnts)     & ! Model U field on layer k (m/s)
 ,uekp1_c(npnts)    & ! Model U field on layer k+1 (m/s)
 ,uekp1_c2(npnts)   & ! Model U field on layer k+1 (m/s)
 ,vek_c(npnts)      & ! Model V field on layer k (m/s)
 ,vek_c2(npnts)     & ! Model V field on layer k (m/s)
 ,vekp1_c(npnts)    & ! Model V field on layer k+1 (m/s)
 ,vekp1_c2(npnts)   & ! Model V field on layer k+1 (m/s)
 ,upk_c(npnts)      & ! Parcel U in layer k after entrainment (m/s)
 ,upk_c2(npnts)     & ! Parcel U in layer k after entrainment (m/s)
 ,upkp1_c(npnts)    & ! Parcel U in layer k+1 after entrainment (m/s
 ,upkp1_c2(npnts)   & ! Parcel U in layer k+1 after entrainment (m/s
 ,vpk_c(npnts)      & ! Parcel V in layer k after entrainment (m/s)
 ,vpk_c2(npnts)     & ! Parcel V in layer k after entrainment (m/s)
 ,vpkp1_c(npnts)    & ! Parcel V in layer k+1 after entrainment (m/s)
 ,vpkp1_c2(npnts)     ! Parcel V in layer k+1 after entrainment (m/s)

REAL ::             &
  xsqkp1_c(npnts)   & ! Excess water vapour in parcel at k+1 (kg/kg)
 ,xsqkp1_c2(npnts)    ! Excess water vapour in parcel at k+1 (kg/kg)
...
REAL :: scale_test              ! Used for testing is scaling too big?
...
! layer_cn_5a needs to know which convection type calls it.
! This routine is midlevel
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='MID_CONV_5A'
...
! The version of Convec2 used by the 5a scheme is also
! called by the 4a scheme. Assigning dimensioning of
! W-eqn arrays to be set by n_wups so that n_wups can
! be set to 1 if being use by the 4a scheme
n_wups = npnts
...
,                   ekp14,ekp34,amdetk)
  ...
  CALL lift_par_5a(ncposs,npnts,thpkp1_c,qpkp1_c,xsqkp1_c,        &
  ...
  DO i = 1,ncposs
    ...
    IF (bconv(index1(i)) .AND. k == start_lev(index1(i))) THEN
      ...
      IF (l_q_interact) THEN
        ...
        ekp14_plus1 = 1.0 + ekp14(index1(i))
        repss = 1.0/( (ekp14_plus1) * (1.0 + ekp34(index1(i))) )
        qclpk_c(i)  = qcl(index1(i),k) +                          &
                      ekp14_plus1 * (qcl(index1(i),k+1) - qcl(index1(i),k))
        qclpkp1_c(i) = ( qclpk_c(i) +                             &
               (ekp14(index1(i)) * qcl(index1(i),k)) +            &
               (ekp34(index1(i)) * ekp14_plus1 * qcl(index1(i),k+1)) )*repss

        qcfpk_c(i) = qcf(index1(i),k) +                           &
                     ekp14_plus1 * (qcf(index1(i),k+1) - qcf(index1(i),k))
        qcfpkp1_c(i) = ( qcfpk_c(i) +                             &
               (ekp14(index1(i)) * qcf(index1(i),k)) +            &
               (ekp34(index1(i)) * ekp14_plus1 * qcf(index1(i),k+1)) )*repss
  ...
  IF (nconv  >   0) THEN
    ...
    CALL convec2_4a5a(nconv,npnts,nlev,ntra,k,md_on,md_sdet_on,md_new_termc, &
                      start_lev3c,n_wups,                                    &
                      l_tracer,l_mom_gk,l_mom_gk_stable,                     &
                    ...
                    ! In/out
                      ...
                      w2p_km1_c2, w2p_k_c2, w2p_kp1_c2,                      &
  ...
  IF (nterm >  0) THEN
    ...
    CASE (7)
      ...
      DO j = 1, nterm
        ...
        IF ( dcpbydt(i) > 0.0 ) THEN
          ...
          END IF
          ! Limit CAPE timescale to 4 hours
    ...
    IF (l_safe_conv) THEN
      ...
      DO j = 1,nterm
        ...
        IF (flx_init_new(i)  >   0.0) THEN
          ...
          END IF
        ELSE  ! negative mass flux probably due to a negative cape
          IF (cape(i) < 0.0) THEN
            cape(i) = 0.0  ! reset CAPE to zero as convection not real
          END IF
          flx_init_new(i) = 0.0
          scale_f(i) =0.0
        END IF
      ...
      ! Check that the mid-level mass flux scaling for the updraught will not
      ! result in removing more water vapour than present in the layer

      DO kt = 2,k+1
        DO j = 1,nterm
          i = index_nterm(j)

          IF (kt  >  start_lev(i) .AND. scale_f(i) >   0.0) THEN
            IF (dqbydt(i,kt) < 0.0) THEN
              ...
              IF (scale_test < scale_f(i)) THEN
                ...
                IF (scale_test < scale_low(i)) THEN
                  scale_low(i) = scale_test
                END IF
                flx_init_new(i) = scale_test*flx_init(i)
              END IF
            END IF
          END IF  ! tests on kt
        END DO
      ...
      DO kt = 2, k+1
        ...
        DO j = 1,nterm
          i = index_nterm(j)
          IF (kt  >=  start_lev(i)) THEN  ! scale all terminating convection
            ...
            IF (kt  ==  det_lev(i)) THEN
              IF (scale_f(i) > 0.0) THEN
                ...
                IF (l_tracer) THEN
                  ...
                  END DO
                END IF
              ELSE   ! scale_f=0.0  no real convection occurred zero increments
                ...
                IF (l_tracer) THEN
                  DO ktra = 1,ntra
                    dtrabydt(i,kt,ktra) = 0.0
                  END DO
                END IF
              END IF   ! test on whether
    ...
    ELSE ! previous unsafe code with negative CAPE ascents
      ...
      DO kt = 2, k+1
        ...
        DO j = 1,nterm
          i = index_nterm(j)
          IF (kt  >=  start_lev(i) .AND. flx_init_new(i)  >   0.0) THEN
    ...
    !-----------------------------------------------------------------------
    ! 3.5  Original mass flux downdraught scheme
    !      Downdraught calculation - on all points where convection is
    !      terminating.
    !
    !      Subroutine DD_CALL
    !
    ...
    IF ((.NOT. l_eman_dd) .AND. (.NOT. l_new_dd)) THEN
      ...
      DO j = 1,nterm
        ...
        IF (l_safe_conv) THEN
          ! added scale_f test to remove cases where CAPE may have been negative
          ! so mid-level updraught set to zero.
          IF (iccb(i)  >   0 .AND. scale_f(i) > 0.0) THEN
        ...
        ELSE
          IF (l_safe_conv) THEN
            IF (scale_f(i) > 0.0) THEN ! convection is real
              nnodd = nnodd + 1
              index_nodd(nnodd) = i
            END IF
          ELSE
    ...
    IF (l_safe_conv) THEN
      IF (.NOT. l_ccrad) THEN
        DO j=1, nterm
          i = index_nterm(j)
          ! Extra test on scale_f for real convection
          IF (iccb(i) == icct(i) .OR. scale_f(i) == 0.0) THEN
            iccb   (i) = 0
            icct   (i) = 0
            cca_2d (i) = 0.0
            tcw    (i) = 0.0
            cclwp  (i) = 0.0
          END IF
          ! ensure no cloud settings if scale_f(0) and only mid so far
          IF (lcbase(i) == lctop(i) .OR.                                  &
                   (lcbase(i) == iccb(i) .AND. scale_f(i) == 0.0)) THEN
            lcbase (i) = 0
            lctop  (i) = 0
            lcca   (i) = 0.0
          END IF
    ...
    IF (l_safe_conv) THEN
      DO j = 1,nterm
        i=index_nterm(j)
        ! Only setting termination level if real mid-level convection
        IF (scale_f(i) > 0.0) THEN
          kterm_mid(i) = k
          kterm(i) = k
        END IF
...
! Copy mass flux on half levels to diagnostic array
...
! If so print a warning message. NOTE unable to cause model to stop as
! tests suggest this type of problem is occurring regularly in some of the
! common operational configurations using the 4A convection scheme so may
! also be a problem with the 5A and 6A schemes.

DO i=1,npnts
  IF (kterm_mid(i) == nlev-1 .AND. l_mid_all(i) ) THEN
    IF (printstatus >= prstatus_normal) THEN
      WRITE(umMessage,'(a33,i8)') 'WARNING: mid_conv reached top at ',i
      CALL umPrint(umMessage,src='mid_conv-mdconv5a')
      ...
      WRITE(umMessage,'(11A26)') 'Theta','q','qcl','qcf','up mass flux',  &
         'dq','dqcl','dqcf','dth','du','dv'
      CALL umPrint(umMessage,src='mid_conv-mdconv5a')
      DO k=1,nlev
        WRITE(umMessage,'(11G26.18)') th(i,k),q(i,k),qcl(i,k),qcf(i,k),     &
  ...
  IF (l_safe_conv) THEN
    ! Has mid_level convection failed? i.e. negative CAPE so increments set to
    ! zero
    IF (kterm_mid(i) == 0 .AND. l_mid_all(i) ) THEN
      ! Reset l_mid_all as a case of unreal mid-level convection
      l_mid_all(i) = .FALSE.
    END IF
  END IF
END DO
...
IF (l_eman_dd .AND. nmid > 0) THEN
 ...
 ,                  timestep,scale_low, th, q, qse, tracer ,precip &
...
END IF
!-----------------------------------------------------------------------
! New mass flux downdraught & evaporation scheme
!-----------------------------------------------------------------------
IF (l_new_dd .AND. nmid > 0) THEN
    
  ! Work out whether there is any precipitation from the updraught.
  DO i = 1,npnts
    tot_precip(i) = 0.0
  END DO
  ...
  npossdd = 0
  DO i = 1,npnts
    IF(kterm(i) >  kterm_max .AND. l_mid_all(i)) THEN
      kterm_max = kterm_mid(i)
    END IF
    ! Real deep convection index
    IF (kterm(i) > ntml(i) .AND. l_mid_all(i) .AND. tot_precip(i) > 0.0) THEN
      npossdd = npossdd +1
      index_possdd(npossdd) = i
    END IF
  END DO
  IF (npossdd > 0) THEN
    ...
    CALL dd_evap_ctl(npnts, npossdd, nlev, trlev, ntra, kterm_max,       &
                     index_possdd, iccb, kterm_mid, freeze_lev,          &
...
IF (nmid >  0) THEN
  IF (l_safe_conv) THEN
    ...
    DO j = 1,nmid
      ...
      IF (temp1(i)  <   qmin_conv .AND.                                     &
        ...
        WRITE(umMessage,'(a19,i6,a9,g26.18,a7,g26.18,a6,i10,i3)')           &
                  ' negative q mid, i:',i,' q after ',temp1(j),             &
                  ' dq/dt ',dqbydt(i,k),' step ',timestep_number,mype
        CALL umPrint(umMessage,src='mid_conv-mdconv5a')

        WRITE(umMessage,'(5A26)') 'q inc','qcl inc ','qcf inc ',            &
              'up mass flux','dp/dt'
        CALL umPrint(umMessage,src='mid_conv-mdconv5a')
        DO kk=1,nlev-1
          WRITE(umMessage,'(5G26.18)') dqbydt(i,kk),dqclbydt(i,kk),         &
                    dqcfbydt(i,kk),flx(i,kk),                               &
               (p_layer_boundaries(i,kk)-p_layer_boundaries(i,kk-1))        &
                                                       /timestep
          CALL umPrint(umMessage,src='mid_conv-mdconv5a')
        END DO
  ...
  ELSE
    ...
    DO j = 1,nmid
      qminincolumn(j)=MAX(qmin_conv,qminincolumn(j))
    END DO
    ...
    DO j = 1,nmid
      ...
      IF (temp1(j)  <   0.0 .AND.                              &
        ...
        WRITE(umMessage,'(a19,i6,a9,g26.18,a7,g26.18)') ' negative q mid, i:',     &
                i,' q after ',temp1(j),' dq/dt ',dqbydt(i,k)
        CALL umPrint(umMessage,src='mid_conv-mdconv5a')
        WRITE(umMessage,'(4A26)') 'q inc','qcl inc ','qcf inc ','mass flux'
        CALL umPrint(umMessage,src='mid_conv-mdconv5a')
        DO kk=1,nlev
          WRITE(umMessage,'(4G26.18)') dqbydt(i,kk),dqclbydt(i,kk),         &
                    dqcfbydt(i,kk),flx(i,kk)
          CALL umPrint(umMessage,src='mid_conv-mdconv5a')
        END DO
      END IF
  ...
  ! ---------------------------------------------------------------------
  ! 7.0 Energy correction calculation - involves integral over
  ...
  IF (l_cv_conserve_check) THEN
    CALL cor_engy_5a(npnts,nmid,nlev,index1, r_theta, r_rho                  &
                    ,r2rho_th, r2rho, dr_across_th, dr_across_rh             &
...
ELSE      ! Not CCRAD
  ...
  IF (l_3d_cca) THEN
    IF (l_anvil) THEN
      CALL calc_3d_cca                                                  &
        ( npnts, npnts, nlev, n_cca_lev, nbl, iccb, icct                &
...
IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_out,zhook_handle)
RETURN
END SUBROUTINE mid_conv_5a
END MODULE mid_conv_5a_mod

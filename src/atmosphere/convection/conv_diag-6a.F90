...
USE planet_constants_mod, ONLY: vkman, cp, kappa, r, repsilon
...
USE cv_run_mod, ONLY:                                                    &
   icvdiag, l_jules_flux, cnv_cold_pools, ccp_off
USE cv_param_mod, ONLY: phi_ccp

USE planet_constants_mod, ONLY: g, c_virtual
...
REAL :: TmpScm3d(ScmRowLen,ScmRow,1:tdims%k_end)
...
IF (model_type == mt_single_column) THEN
  ...
  IF (nunstable == 0) THEN
    ...
    IF (l_scmdiags(scmdiag_conv)) THEN
      ...
      ! SCM diagnostics that would be output from inside parcel_ascent
      ! (which is called by conv_diag_comp-6a before that routine's
      !  calls to scmoutput), if a dilute ascent is used
      IF (icvdiag >= 2) THEN

...
MODULE shallow_turb_conv_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'SHALLOW_TURB_CONV_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY:                                         &
   cp, r, repsilon, c_virtual, rv, recip_epsilon, g, planet_radius
...
USE les_shall_func_rhlev_mod, ONLY: les_shall_func_rhlev
USE les_shall_func_thlev_mod, ONLY: les_shall_func_thlev
USE sh_grad_flux_mod, ONLY: sh_grad_flux
USE shallow_grad_h_mod, ONLY: shallow_grad_h
USE shallow_wql_mod, ONLY: shallow_wql
USE shconv_cloudbase_mod, ONLY: shconv_cloudbase
USE shconv_inversion_mod, ONLY: shconv_inversion
USE shconv_precip_mod, ONLY: shconv_precip
USE shconv_qlup_mod, ONLY: shconv_qlup
USE shconv_turb_fluxes_mod, ONLY: shconv_turb_fluxes
USE shtconv_base_stress_mod, ONLY: shtconv_base_stress
USE shtconv_cmt_incr_mod, ONLY: shtconv_cmt_incr
USE shtconv_grad_stress_mod, ONLY: shtconv_grad_stress
IMPLICIT NONE
...
REAL :: TmpScm3d(ScmRowLen,ScmRow,nlev+1)  ! model_levels = nlev+1
        ! Note: this must be 3d in order to satisfy fortran's shape-
        ! matching rules on input to scmoutput.
...
DO i = 1,n_sh
  ...
  dthetav_cld(i) =    theta(i,ntpar(i)+1)*                        &
      (1.0+q_mix(i,ntpar(i)+1)/repsilon)/(1.0+q_mix(i,ntpar(i)+1))  &
                  -   theta(i,ntml(i))*                           &
      (1.0+q_mix(i,ntml(i))/repsilon)/(1.0+q_mix(i,ntml(i)))
...
DO i= 1, n_sh
  ...
  dthetavdz_ad_lcl(i) = temp2*(1.0+0.61*theta(i,ntml(i))*temp1)+    &
...
IF (L_tracer) THEN
  ...
  IF (model_type == mt_single_column) THEN
    ...
    ! Conservation check -left for SCM as a means of
    ! checking code
...
DO k = nlev,2,-1
  DO i = 1,n_sh
    ...
    ((qMinInColumn(i) - q_mix(i,k)) / timestep-dqbydt(i,k))    &
        ...
        ELSE
          WRITE(umMessage,*) 'Problem negative q ',i,k,                &
                     tempx(i),dqbydt(i,k), q_mix(i,k),         &
                     (dqbydt(i,j),j=1,ntpar(i)+1)
          CALL umPrint(umMessage,src='shallow_turb_conv')
...
DO i = 1,n_sh
  ...
  IF (tempx(i)  <   qMinInColumn(i)) THEN
    WRITE(umMessage,*) 'Problem negative q k=1 ',i,tempx(i),         &
...
!---------------------------------------------------------------------
! SCM diagnostics - 
! Note problems if Call convection more then once per timestep
!---------------------------------------------------------------------
...
IF (model_type == mt_single_column) THEN
  IF (i_convection_vn == i_convection_vn_6a) THEN
    ...
    sname='fw_func'
    lname='updraught velocity similarity function, Fw'
    units='-'
    TmpScm3d = RESHAPE( fw_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,                  &
         t_inst,d_wet,default_streams,'',routinename)

    sname='g_func'
    lname='Mass flux similarity function, G'
    units='-'
    TmpScm3d = RESHAPE( g_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,&
         t_inst,d_wet,default_streams,'',routinename)

    sname='k_func'
    lname='Flux eddy diffusivity, K'
    units='-'
    TmpScm3d = RESHAPE( k_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( f0_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( f1_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( ftheta_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( k_hfunc, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( fng_hfunc, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( b_hfunc, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( fw_hfunc, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( g_hfunc, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( fql_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
    ...
    lname=sname
    units='-'
    TmpScm3d = RESHAPE( gql_func, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
    CALL scmoutput(TmpScm3d,sname,lname,units,            &
         t_inst,d_wet,default_streams,'',routinename)
  ...
  Tmpscm2d = RESHAPE( mb, [ScmRowLen,ScmRow] )
  sname='mb_cb'
  ...
  Tmpscm2d = RESHAPE( mb_new, [ScmRowLen,ScmRow] )
  sname='mb_new_cb'
  ...
  Tmpscm2d = RESHAPE( wstar_up, [ScmRowLen,ScmRow] )
  sname='wstar_up'
  lname='wstar_up'
  units='m/s'
  CALL scmoutput(Tmpscm2d,sname,lname,units,            &
  ...
  Tmpscm2d = RESHAPE( wthetal_cb, [ScmRowLen,ScmRow] )
  sname='wthetal_cb'
  ...
  Tmpscm2d = RESHAPE( wqt_cb, [ScmRowLen,ScmRow] )
  sname='wqt_cb'
  ...
  Tmpscm2d = RESHAPE( wql_cb, [ScmRowLen,ScmRow] )
  sname='wql_cb'
  ...
  Tmpscm2d = RESHAPE( wql_inv, [ScmRowLen,ScmRow] )
  sname='wql_inv'
  ...
  Tmpscm2d = RESHAPE( wqr_inv, [ScmRowLen,ScmRow] )
  sname='wqr_inv'
  ...
  Tmpscm2d = RESHAPE( theta_plus *                              &
         (1.0+q_plus*recip_epsilon)/(1.0+q_plus)-theta_minus*   &
         (1.0+q_minus*recip_epsilon)/(1.0+q_minus),             &
                      [ScmRowLen,ScmRow] )
  sname='dthetav_cb'
  ...
  Tmpscm2d = RESHAPE( dthetal_cb, [ScmRowLen,ScmRow] )
  sname='dtheta_cb'
  ...
  Tmpscm2d = RESHAPE( dqt_cb, [ScmRowLen,ScmRow] )
  sname='drv_cb'
  ...
  Tmpscm2d = RESHAPE( wthetal_cb+c_virtual*theta_minus*wqt_cb,  &
                      [ScmRowLen,ScmRow] )
  sname='wthetav_m_cb'
  ...
  Tmpscm2d = RESHAPE( wqt_inv, [ScmRowLen,ScmRow] )
  sname='wqt_inv'
  ...
  Tmpscm2d = RESHAPE( wthetal_inv, [ScmRowLen,ScmRow] )
  sname='wthetal_inv'
  ...
  units='Kg/kg s'
  TmpScm3d = RESHAPE( dwql_dz, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  units='Kg/kg '
  TmpScm3d = RESHAPE( ccw, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  units='m/s'
  TmpScm3d = RESHAPE( wup, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  units='K m/s '
  TmpScm3d = RESHAPE( wthetav, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  units=' '
  TmpScm3d = RESHAPE( wql, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  lname='dqsat_dt'
  units='K m/s'
  TmpScm3d = RESHAPE( dqsatdt_cld, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  sname='wthetal'
  lname='wthetal'
  units='K m/s'
  TmpScm3d = RESHAPE( wthetal, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  lname='wqt'
  units='Kg/kg m/s'
  TmpScm3d = RESHAPE( wqt, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  units='Kg/kgm/s'
  TmpScm3d = RESHAPE( wqr, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
  ...
  units='/s'
  TmpScm3d = RESHAPE( precip_product, [ScmRowLen,ScmRow,nlev+1], PAD=[0.0] )
  CALL scmoutput(TmpScm3d,sname,lname,units,            &
       t_inst,d_wet,default_streams,'',routinename)
...
END MODULE shallow_turb_conv_mod

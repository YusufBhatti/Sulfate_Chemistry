...
USE cv_run_mod, ONLY:                                             &
    l_mom, l_eman_dd, l_new_dd, cldbase_opt_md, cape_min,         &
...
REAL, INTENT(INOUT) :: w2p(npnts,nlev) ! (Parcel vertical velocity)^2 [(m/s)^2]
...
TYPE(scm_convss_dg_type), INTENT(INOUT) :: scm_convss_dg( npnts )
...
REAL :: wsc_o_mb(npnts)         ! Dummy argument for layer_cn
...
INTEGER :: start_lev_c2(npnts)  ! Compressed convection initiation level
...
REAL :: thrk_c2(npnts)          ! potential temperature of forced detrained air

REAL :: qrk_c2(npnts)           ! specific humidity of forced detrained air
...
REAL :: flx_init_c2(npnts)      ! Initial parcal mass flux at cloud base (Pa/s)
...
REAL :: qsat_lcl(npnts)     !qsat at the LCL
...
IF (l_new_dd .AND. nmid > 0) THEN
  ...
  ! Resetting kterm to be sure it holds the highest top and lcbase holds
  ! the lowest bottom for real mid-level convection
  DO k=1,nlev
    DO i = 1,npnts
      IF (flx(i,k) > 0.0 .AND. l_mid_all(i)) THEN
        kterm_mid(i) = k
        IF (lcbase(i) == 0) THEN
          lcbase(i) = k
        END IF 
  ...
  kterm_max = 2
  ! Work out points where precipitation and mid-level convection
  npossdd = 0
  DO i = 1,npnts
    IF(kterm(i) >  kterm_max .AND. l_mid_all(i) .AND. tot_precip(i) > 0.0) THEN
      kterm_max = kterm_mid(i)
    END IF
    ! Real deep convection index
    IF (l_mid_all(i) .AND. tot_precip(i) > 0.0) THEN
      npossdd = npossdd +1
      index_possdd(npossdd) = i
    END IF
  END DO
  ...
  IF (npossdd > 0) THEN
    ...
    CALL dd_evap_ctl(npnts, npossdd, nlev, trlev, ntra, kterm_max,       &
                     index_possdd, lcbase, kterm_mid, freeze_lev,        &
  ...
  END IF  ! real mid-level convective points
...
IF (nmid >  0) THEN
  ...
  IF (l_cv_conserve_check) THEN
    CALL cor_engy_6a(npnts, nmid, nlev, index1,                       &
  ...
  DO j = 1,nmid
    ...
    IF (temp1(j)  <   0.0 .AND.                                       &
      ...
      WRITE(umMessage,'(a19,i6,a9,g26.18,a7,g26.18)')                        &
        ' negative q mid, i:', i,' q after ',temp1(j),' dq/dt ',dqbydt(i,k)
      CALL umPrint(umMessage,src='mid_conv_mod-6a')
      WRITE(umMessage,'(3A26)') 'q inc','qcl inc ','qcf inc '
      CALL umPrint(umMessage,src='mid_conv_mod-6a')
      DO kk=1,nlev-1
        WRITE(umMessage,'(3G26.18)') dqbydt(i,kk),dqclbydt(i,kk),         &
                    dqcfbydt(i,kk)
        CALL umPrint(umMessage,src='mid_conv_mod-6a')
      END DO
    END IF

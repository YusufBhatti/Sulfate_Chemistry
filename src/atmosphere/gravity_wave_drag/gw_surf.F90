...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'GW_SURF_MOD'
CONTAINS
...
SUBROUTINE gw_surf(                                               &
   ...
   k_top,k_top_max,lift,l_drag,fr,frc,nsigma,                     &
...
USE c_gwave_mod, ONLY: amplitude_saturation,                      &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='GW_SURF'
...
!---------------------------------------------------------------------
DO i=1,points
  IF (sd_orog(i)  <=  0.0 .OR. kay == 0.0 ) THEN
    l_drag(i) = .FALSE.
    l_cont(i) = .FALSE.
    k_top(i)  = 2
  END IF
...
DO k=2,levels-1
  DO i=1,points
    IF ( l_cont(i) ) THEN
      ...
      n_squared = 2.0*g*(theta(i,k)-theta(i,k-1) )                 &
...
DO i=1,points
  ...
  IF ( nsq_s(i) >  0.0 .AND. speed >  0.0 .AND. l_drag(i) ) THEN
    ...
    IF ( l_taus_scale ) THEN
      ...
      IF ( fr_for_diag(i) <= 1.0 ) THEN
        taus_scale(i) = 1.6*fr_for_diag(i)
      ELSE IF ( fr_for_diag(i) <= 2.0 ) THEN
    ...
    IF (s_wake_stress  >   s_wake_limit) THEN
      ...
      s_y_wake_stress(i) = s_y_wake_stress(i) * s_wake_limit      &
                                              /s_wake_stress
      num_lim(i) = 100.0
      num_fac(i) = 100.0 * (s_wake_stress - s_wake_limit )         &
                          / s_wake_stress
    END IF
...
IF ( fr_d_on ) THEN
  DO i=1,points
    ...
    IF (fr_d(i)  >   1000.0) fr_d(i) = 1000.0
  END DO
END IF

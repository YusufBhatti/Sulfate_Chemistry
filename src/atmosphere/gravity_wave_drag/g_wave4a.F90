...
IMPLICIT NONE
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='G_WAVE_4A_MOD'
...
SUBROUTINE g_wave_4a(                                             &
  ...
  land_index,land_points,timestep,kay,frc,nsigma,r_u,r_v,T_inc,   &
...
USE gw_surf_mod, ONLY: gw_surf
USE gw_vert_mod, ONLY: gw_vert
IMPLICIT NONE
!
! Description:
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='G_WAVE_4A'
...
!$OMP SHARED( udims_s, pdims, levels, at_extremity, work_u, vdims_s,   &
!$OMP         work_v, u, v, up_land, vp_land, land_index, land_points, &
!$OMP         row_length )                                             &
!$OMP PRIVATE( i, j, k, l )
CALL u_to_p(u,                                                    &
...
!$OMP& r_rho_levels,recip_a2) PRIVATE(i,j,k,l)
  DO k=1,levels
    DO l=1,land_points
      j=(land_index(l)-1)/row_length + 1
      i=land_index(l) - (j-1)*row_length
...
! Calling with levels +1 as work_halo starts at zero for kth dim
CALL swap_bounds(                                                 &
     work_halo,                                                   &
     row_length, rows,                                            &
     levels+1, off_x, off_y, fld_type_p, swap_field_is_scalar,    &
     do_west_arg=.TRUE., do_east_arg= .FALSE.)


!$OMP PARALLEL DEFAULT(NONE)                                           &
!$OMP SHARED( work_halo, pdims_s, udims, levels, work_on_u_grid,       &
!$OMP         land_points, land_index, row_length, dv_dt )             &
!$OMP PRIVATE( i, j, k, l )
...
! ENDGAME needs a south swap - ND a north
! Calling with levels +1 as work_halo starts at zero for kth dim
CALL swap_bounds(                                                 &
     work_halo,                                                   &
     row_length, rows,                                            &
...
!$OMP PARALLEL DEFAULT(NONE)                                           &
!$OMP SHARED( work_halo, pdims_s, vdims, levels, work_on_v_grid )
...
IF (stress_ud_on .OR. stress_ud_p_on) THEN  ! stashflag for this diagnostic
  ...
  ! ENDGAME needs a west swap - ND an east
  ! Calling with levels +1 as work_halo starts at zero for kth dim
  CALL swap_bounds(                                                 &
       work_halo,                                                   &
       row_length, rows,                                            &
...
IF (stress_ud_satn_on) THEN  ! stashflag for this diagnostic
  ...
  ! ENDGame requires a west swap - ND an east
  ! Calling with levels +1 as work_halo starts at zero for kth dim
  CALL swap_bounds(                                                 &
       work_halo,                                                   &
       row_length, rows,                                            &
...
IF (tausx_d_on) THEN  ! stashflag for this diagnostic
  ...
  CALL swap_bounds(                                                 &
       work_halo,                                                   &
       row_length, rows,                                            &
       1, off_x, off_y, fld_type_p, swap_field_is_scalar,           &
       do_west_arg=.TRUE., do_east_arg= .FALSE.)

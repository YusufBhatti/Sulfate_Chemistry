...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='EG_POFIL_VATP_MOD'
...
USE turb_diff_mod, ONLY: l_diff_wind_ew_only
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_POFIL_VATP'
...
IF ( L_diff ) THEN
  ...
  IF ( j_begin < 0 ) THEN
    j_begin = begin(0)
    j_end = END(0)
  ELSE IF ( j_end < END(0) ) THEN
    j_end = END(0)
  ELSE IF ( j_begin > begin(0) ) THEN
    j_begin = begin(0)
  END IF !  j_begin < 0
END IF ! L_diff
...
IF ( L_diff ) THEN
  ...
  IF ( L_u_ns_diff .AND. .NOT. l_diff_wind_ew_only) THEN
    DO k = 1, active_levels

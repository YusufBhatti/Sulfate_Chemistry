...
REAL, INTENT(IN) ::                                                     &
  field (udims_s%i_start:udims_s%i_end,                                 &
         udims_s%j_start:udims_s%j_end, model_levels )

! Arguments with Intent IN/OUT. ie: Input variables changed on Output.

REAL, INTENT(INOUT) ::                                                  &
  field_star (udims_s%i_start:udims_s%i_end,                            &
              udims_s%j_start:udims_s%j_end, model_levels )
...
, diff_term(udims%i_start:udims%i_end,                                  &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_TURB_DIFF_U'
...
!$OMP& SHARED(udims,field,delta_z,coeff_th,csxi2_p,xi1_u,xi2_p, &
!$OMP& xi1_p,coeff_centre,csxi2_v,field_star,timestep,recip_r_squared_delz, &
...
! difference between u-point i-1 -> i is on p-point i
      temp(i,j) = ( field(i,j,k) - field(i-1,j,k) ) *                   &
                                                 delta_z(i,j,k) *       &
                                                 coeff_th(i,j,k) /      &
                                         ( csxi2_p(j) * csxi2_p(j) *    &
                                          (xi1_u(i) - xi1_u(i-1)) )
...
! difference between p-point i -> i+1 is on u-point i
    DO i = udims%i_start, udims%i_end
      diff_term(i,j) = ( temp(i+1,j) - temp(i,j) ) /                    &
                         ( xi1_p(i+1) - xi1_p(i) )
    END DO
  END DO
  ...
  DO j = udims%j_start-1, udims%j_end
    DO i = udims%i_start, udims%i_end
! difference between u-point j -> j+1 is on v-point j
...
! difference between v-point j-1 -> j is on u-point j
  DO j = udims%j_start, udims%j_end
    DO i = udims%i_start, udims%i_end

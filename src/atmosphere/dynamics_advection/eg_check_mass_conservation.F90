...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='EG_CHECK_CONSERV_MOD'

CONTAINS
SUBROUTINE eg_check_mass_conservation_fix(rho_n, rho_np1, qs_n, qs_np1,    &
                                      qs_s, number_qs, mype, chara_str )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_CHECK_MASS_CONSERVATION_FIX'
...
REAL :: global_sums(0:number_qs,2)
REAL :: err_mass(0:number_qs)
REAL :: qsmin(0:number_qs)
REAL :: qsmax(0:number_qs)
...
DO k = pdims%k_start, pdims%k_end
  DO j = pdims%j_start, pdims%j_end
    DO i = pdims%i_start, pdims%i_end
      local_sums(i,j,0,1)  = local_sums(i,j,0,1)             &
                              + rho_n(i,j,k) * ec_vol(i,j,k)
      local_sums(i,j,0,2)  = local_sums(i,j,0,2)             &
                              + rho_np1(i,j,k) * ec_vol(i,j,k)
    END DO
  END DO
END DO
...
DO kk = 0, number_qs
  ...
  IF ( mype == 0 .AND. PrintStatus > PrStatus_Normal ) THEN
    WRITE(umMessage,'(A,I4,2x,E23.15)')                                  &
         chara_str//' species / mass conservation error  = ',kk, err_mass(kk)
    CALL umPrint(umMessage,src='eg_check_mass_conservation')
  END IF
END DO

qsmin(0) = MINVAL(rho_np1)
qsmax(0) = MAXVAL(rho_np1)
DO kk = 1, number_qs
  qsmin(kk) = MINVAL(qs_np1( tdims%i_start:tdims%i_end,     &
                             tdims%j_start:tdims%j_end,     &
                             tdims%k_start+1:tdims%k_end,kk))
  qsmax(kk) = MAXVAL(qs_np1( tdims%i_start:tdims%i_end,     &
                             tdims%j_start:tdims%j_end,     &
                             tdims%k_start+1:tdims%k_end,kk))
END DO

CALL gc_rmin(number_qs+1,nproc,istat1,qsmin)
CALL gc_rmax(number_qs+1,nproc,istat2,qsmax)

IF ( mype == 0 .AND. PrintStatus > PrStatus_Normal ) THEN
  DO kk = 0, number_qs
    WRITE(umMessage,'(A,I4,2(1x,E12.5),E23.15)')  &
         chara_str // ' species/min/max/total =',        &
         kk, qsmin(kk),qsmax(kk), global_sums(kk,2)
    CALL umPrint(umMessage,src='eg_check_mass_conservation')
  END DO
END IF
...
RETURN
END SUBROUTINE eg_check_mass_conservation_fix

!======================================================================
!
! This version is used if l_fix_conserv = .FALSE.
!
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_CHECK_MASS_CONSERVATION'
...
CALL gc_rmin(number_qs,nproc,istat1,qsmin)
CALL gc_rmax(number_qs,nproc,istat2,qsmax)

IF ( mype == 0 .AND. PrintStatus > PrStatus_Normal ) THEN
  DO kk = 1, number_qs
    WRITE(umMessage,'(A,I4,2(1x,E12.5))') chara_str//' species/min/max =', &

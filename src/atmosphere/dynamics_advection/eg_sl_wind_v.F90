...
IMPLICIT NONE
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='EG_SL_WIND_V_MOD'

CONTAINS
SUBROUTINE eg_sl_wind_v(g_i_pe, depart_scheme, l_rk_dps,              &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_SL_WIND_V'
...
dep_row_len = vdims%i_len
dep_rows    = vdims%j_len
...
IF ( .NOT. cartesian_grid ) THEN
  CALL eg_interpolation_eta_pmf(                                      &
                     ...
                     l_high, l_mono, depart_xi3_v, depart_xi1_v,      &
...
IF ( .NOT. (cartesian_grid .OR. l_shallow) ) THEN
  CALL eg_interpolation_eta_pmf(                                      &
                     eta_theta_levels,                                &

...
SUBROUTINE eg_vert_weights_eta (                                      &
                        ...
                        l_high, k_int_linear, k_out, eta_in, eta_out, &
                        coeff_z )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_VERT_WEIGHTS_ETA'
...
REAL :: rq3(dim_k_in,-1:2), rq5(dim_k_in,-2:3), rdeta(dim_k_in)
...
!$OMP PARALLEL DO PRIVATE(i,j,k,weight_eta, e_here_minus2,e_here_minus, &
!$OMP&  e_here,e_here_plus, e_here_plus2, e_here_plus3,                 &
!$OMP&  numer_minus2,numer_minus,numer,numer_plus,numer_plus2,          &
!$OMP&  numer_plus3)  SCHEDULE(STATIC) DEFAULT(SHARED)
  DO k = 1, dim_k_out
    DO j = 1, dim_j_out
      DO i = 1, dim_i_out
        ...
        IF ( k_out(i,j,k) <= k_int_linear .OR.                        &
          ...
          coeff_z(i,j,k,-2) = 0.0
          coeff_z(i,j,k,-1) = 0.0
          coeff_z(i,j,k,0)  = 1.0 - weight_eta
        ...
        ELSE IF (k_out(i,j,k) == 2 .OR.                               &
          ...
          coeff_z(i,j,k,0) = (numer_minus*numer_plus*numer_plus2 )    &
                            *rq5(k_out(i,j,k),0)

          coeff_z(i,j,k,1) = (numer_minus*numer*numer_plus2 )         &
                            *rq5(k_out(i,j,k),1)
        ...
        ELSE
          ...
          coeff_z(i,j,k,0) = ( numer_minus2*numer_minus*numer_plus    &
                              *numer_plus2*numer_plus3 )              &
                            *rq5(k_out(i,j,k),0)

          coeff_z(i,j,k,1) = ( numer_minus2*numer_minus*numer         &
                              *numer_plus2*numer_plus3 )              &
                            *rq5(k_out(i,j,k),1)

          coeff_z(i,j,k,2) = ( numer_minus2*numer_minus*numer         &
                              *numer_plus*numer_plus3 )               &
                            *rq5(k_out(i,j,k),2)

          coeff_z(i,j,k,3)= ( numer_minus2*numer_minus*numer          &
...
!$OMP&         PRIVATE(i,j,k,weight_eta)  SHARED(coeff_z, rdeta,     &
!$OMP&         eta_out, eta_in, k_out, dim_k_out, dim_j_out,         &
!$OMP&         dim_i_out)
  DO k=1, dim_k_out
    DO j = 1, dim_j_out
      DO i = 1, dim_i_out
    ...
    DO j = 1, dim_j_out
      DO i = 1, dim_i_out
        ...
        IF ( kD <= k_int_linear .OR. kD == dim_k_in-1 ) THEN
          ...
          coeff_z(i,j,k,-1) = 0.0
          coeff_z(i,j,k,0)  = 1.0-xi
          coeff_z(i,j,k,1)  = xi
          coeff_z(i,j,k,2)  = 0.0

        ELSE

          coeff_z(i,j,k,-1)=-h3*rmesh(kD)**2/(1.0+rmesh(kD))

          coeff_z(i,j,k,0) = h1 + h3*(rmesh(kD)-1.0) -                &
                             h4*rmesh(kD+1)/(1.0+rmesh(kD+1))

          coeff_z(i,j,k,1) = h2 + h3/(1.0+rmesh(kD)) +                &
                             h4*(rmesh(kD+1)-1.0)/rmesh(kD+1)

          coeff_z(i,j,k,2) = h4/(rmesh(kD+1)*(1.0+rmesh(kD+1)))
    ...
    DO j = 1, dim_j_out
      DO i = 1, dim_i_out
        ...
        ELSE
          ...
          coeff_z(i,j,k,-2) = deta(kD)*h3*omega_prime(-2,kD)

          coeff_z(i,j,k,-1) = deta(kD)*( h3*omega_prime(-1,kD) +               &
                                         h4*omega_prime(-2,kD+1) )

          coeff_z(i,j,k,0) = h1 + deta(kD)*( h3*omega_prime(0,kD) +            &
                                             h4*omega_prime(-1,kD+1) )

          coeff_z(i,j,k,1) = h2 + deta(kD)*( h3*omega_prime(1,kD) +            &
                                             h4*omega_prime(0,kD+1) )

          coeff_z(i,j,k,2) = deta(kD)*( h3*omega_prime(2,kD) +                 &
                                        h4*omega_prime(1,kD+1) )

          coeff_z(i,j,k,3) = deta(kD)*h4*omega_prime(2,kD+1)

...
IMPLICIT NONE
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='EG_SL_WIND_W_MOD'

CONTAINS
SUBROUTINE eg_sl_wind_w(g_i_pe, depart_scheme, l_rk_dps,              &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_SL_WIND_W'
...
IF (model_type /= mt_global) THEN
  CALL eg_dep_pnt_cart_eta(                                         &
             ...
             l_depart_high, l_depart_mono, alpha, eta_rho_ext,      &
...
ELSE
  ...
  CALL departure_point_eta(                                         &
             ...
             l_depart_mono, l_shallow, alpha,                       &
             eta_rho_ext, xi3_at_theta,                             &
...
IF ( .NOT. (l_shallow .OR. cartesian_grid) ) THEN
  CALL eg_interpolation_eta_pmf(                                        &
                       ...
                       row_length, rows, model_levels+2, rows,          &
  ...
  CALL eg_interpolation_eta_pmf(                                        &
                       ...
                       row_length, n_rows, model_levels+2, rows,        &
                       row_length, rows, model_levels+1,                &
                       high_order_scheme, monotone_scheme,              &
                       l_high, l_mono,                                  &

...
CHARACTER(LEN=*), PARAMETER :: RoutineName='POLAR_VECTOR_WIND_N'
...
IF (at_extremity(PNorth)) THEN
  ...
  DO k = 1, model_levels
    ...
    a_np = 2.0 * wrk(2*(k-1)+1) / global_row_length
    b_np = 2.0 * wrk(2*k) / global_row_length

    mag_vector_np(k) = SQRT (a_np * a_np + b_np * b_np)

    IF (a_np  ==  0.0 .AND. b_np  ==  0.0) THEN
      dir_vector_np(k) = 0.0
    ELSE
...
IF (at_extremity(PSouth)) THEN
  ...
  DO k = 1, model_levels
    ...
    a_sp = 2.0 * wrk(2*(k-1)+1) / global_row_length
    b_sp = 2.0 * wrk(2*k) / global_row_length

    mag_vector_sp(k) = SQRT (a_sp * a_sp + b_sp * b_sp)
    IF (a_sp  ==  0.0 .AND. b_sp  ==  0.0) THEN
      dir_vector_sp(k) = 0.0
    ELSE

...
IF (m < 0 .OR. m > n .OR. ABS(x) > 1.0) THEN
  icode = 1
  CALL umPrint('Error: Bad argument values in call to ALF',             &
       src=routinename)
  CALL umPrint('One or more of: m < 0; m > n; ABS(x) > 1',              &
       src=routinename)
  WRITE (cmessage,'(A)') 'ALF: Error in argument values m, n or x'

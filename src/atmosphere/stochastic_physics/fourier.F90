...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='FOURIER_MOD'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='FOURIER'
...
IF ( ierr == 1 ) THEN
  WRITE(umMessage,'(A,I4,A)')                                           &
      'vector length = ', nvex, ', greater than NBLOCK'
  CALL umPrint(umMessage,src='fourier.F90')
ELSE IF ( ierr == 2 ) THEN
  WRITE(umMessage,'(A,I3,A)') 'factor = ', ifac, ' NOT catered for'
  CALL umPrint(umMessage,src='fourier.F90')
ELSE

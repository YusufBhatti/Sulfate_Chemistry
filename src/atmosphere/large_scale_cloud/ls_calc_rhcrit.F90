...
MODULE ls_calc_rhcrit_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'LS_CALC_RHCRIT_MOD'
CONTAINS
...
USE planet_constants_mod, ONLY: planet_radius, lsrcp, cp, r, repsilon
...
!       Ice fraction.
    ...
    p_layer_centres(0:row_length+1,0:rows+1,0:model_levels),            &
!       pressure at all points, on theta model_levels (Pa).
!       NB: Zero level of array is surface pressure.
    q(0:row_length+1,0:rows+1,model_levels),                            &
!       Total water content, Q+QCL (kg per kg)
    qcf(0:row_length+1,0:rows+1,model_levels),                          &
!       Cloud ice content at processed model_levels (kg water per kg air).
    t(0:row_length+1,0:rows+1,model_levels)
...
REAL ::                                                               &
   tl(0:row_length+1,0:rows+1,model_levels),                          &
                                          ! Conserved temperature
!                                                      (P292.1, UMDP29)
     qt(0:row_length+1,0:rows+1,model_levels),                          &
                                            ! Conserved WATER
!                                                      (P292.2, UMDP29)
     qst(0:row_length+1,0:rows+1),                                      &
                                        ! SATURATION VAPOUR PRESSURE
     p_grad(row_length,rows,model_levels),                              &
                                        ! TERM WHICH RELATES
!                                         RH_SD_3 TO RH_SD_1
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='LS_CALC_RHCRIT'
...
h_scale = planet_radius * delta_lambda
...
DO k = 1, model_levels
  ...
  IF ( l_new_qsat_lsc ) THEN
    IF ( l_mr_physics ) THEN
      CALL qsat_mix_new(qst,tl(:,:,k),p_layer_centres(:,:,k),                 &
                        row_length+2,rows+2)
    ELSE
      CALL qsat_new(qst,tl(:,:,k),p_layer_centres(:,:,k),                     &
                    row_length+2,rows+2)
    END IF
  ELSE
    ! DEPENDS ON: qsat_mix
    CALL qsat_mix(qst(0,0),tl(0,0,k),p_layer_centres(0,0,k),                  &
                  (row_length+2)*(rows+2),l_mr_physics)
  END IF
  ...
  DO j = 0, rows+1
    ...
    DO i = 0, row_length+1
      ...
      ELSE
        latht = ls/tl(i,j,k)
      END IF
      al(i,j) = 1.0/ (1.0+latht*latht*ercpr*qst(i,j))
  ...
  DO j = 1, rows
    ...
    DO i = 1, row_length
      ...
      IF (icount(i,j)  >   1) THEN
        two_sigma = 2.0 * SQRT(tot_var/icount(i,j))
      ELSE
      ...
      rhcpt(i,j,k) = 1.0 - (root_6*supsat_sd_1 /(al(i,j)*qst(i,j)))
...
IF (at_extremity(PSouth)) THEN
  ...
  r_row_length = 1.0 / global_row_length
  ...
  CALL global_2d_sums(rhcpt(:,j_start:j_start,:), i_length, 1, 0, 0,  &
                      model_levels, rhcpt_mean, gc_proc_row_group)

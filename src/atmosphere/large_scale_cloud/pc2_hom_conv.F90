...
!      Other quantities for the turbulence
 pc2mixingrate, dbsdtbs1, l_mixing_ratio, l_pc2_prod_qcl_mp)
...
!       Increment in liquid cloud fraction (no units)
   cf_above(      tdims%i_start:tdims%i_end,                            &
                  tdims%j_start:tdims%j_end,                            &
                  nlevels),                                             &
!       Total cloud fraction in layer above (no units)
   cf_below(      tdims%i_start:tdims%i_end,                            &
                  tdims%j_start:tdims%j_end,                            &
                  nlevels)
...
DO k = 1, nlevels
  ...
  DO j = tdims%j_start, tdims%j_end
    ...
    DO i = tdims%i_start, tdims%i_end
      ...
      !===========================================================
      IF (i_pc2_erosion_method == pc2eros_hybrid_allfaces .OR.        &
          i_pc2_erosion_method == pc2eros_hybrid_sidesonly .OR.       &

...
MODULE pc2_pressure_forcing_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName = 'PC2_PRESSURE_FORCING_MOD'
CONTAINS
...
USE planet_constants_mod,  ONLY: recip_kappa, pref
USE timestep_mod,          ONLY: timestep
USE yomhook,               ONLY: lhook, dr_hook
USE parkind1,              ONLY: jprb, jpim
USE atm_fields_bounds_mod, ONLY: pdims,tdims, pdims_s, tdims_s,         &
                                 tdims_l
USE  physics_tendencies_mod, ONLY:                                      &
     init_cld_tendencies, dt_cld, l_retain_cld_tendencies

USE atm_step_local, ONLY:                                              &
    t_inc_pres, q_inc_pres, qcl_inc_pres, qcf_inc_pres, cf_inc_pres,   &
    cfl_inc_pres, cff_inc_pres, t_dini, q_dini,  qcl_dini, qcf_dini,   &
...
!       Potential temperature (K)
   cf(             tdims_l%i_start:tdims_l%i_end,                       &
                   tdims_l%j_start:tdims_l%j_end,                       &
                   tdims_l%k_start:tdims_l%k_end),                      &
!       Total cloud fraction (no units)
   cfl(            tdims_l%i_start:tdims_l%i_end,                       &
                   tdims_l%j_start:tdims_l%j_end,                       &
                   tdims_l%k_start:tdims_l%k_end),                      &
!       Liquid cloud fraction (no units)
   cff(            tdims_l%i_start:tdims_l%i_end,                       &
                   tdims_l%j_start:tdims_l%j_end,                       &
                   tdims_l%k_start:tdims_l%k_end),                      &
...
!  Local arrays which do not contain halos
   p_theta_levels_no_halos(                                             &
                   ...
                   pdims%j_start:pdims%j_end,                           &
                   pdims%k_start:pdims%k_end),                          &
   cf_no_halos(    tdims%i_start:tdims%i_end,                           &
                   tdims%j_start:tdims%j_end,                           &
                               1:tdims%k_end),                          &
   cfl_no_halos(   tdims%i_start:tdims%i_end,                           &
                   tdims%j_start:tdims%j_end,                           &
                               1:tdims%k_end),                          &
   cff_no_halos(   tdims%i_start:tdims%i_end,                           &
                   tdims%j_start:tdims%j_end,                           &
                               1:tdims%k_end),                          &
   q_no_halos(     tdims%i_start:tdims%i_end,                           &
                   tdims%j_start:tdims%j_end,                           &
                               1:tdims%k_end),                          &
   qcl_no_halos(   tdims%i_start:tdims%i_end,                           &
                   tdims%j_start:tdims%j_end,                           &
                               1:tdims%k_end),                          &
   qcf_no_halos(   tdims%i_start:tdims%i_end,                           &
                   tdims%j_start:tdims%j_end,                           &
                               1:tdims%k_end)
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='PC2_PRESSURE_FORCING'
...
DO k = 1, tdims%k_end
  DO j = tdims%j_start, tdims%j_end
    DO i = tdims%i_start, tdims%i_end
      ...
      t_inc_pres  (i,j,k) = t(i,j,k)
      q_inc_pres  (i,j,k) = q(i,j,k)
      qcl_inc_pres(i,j,k) = qcl(i,j,k)
      qcf_inc_pres(i,j,k) = qcf(i,j,k)
      cf_inc_pres (i,j,k) = cf(i,j,k)
      cfl_inc_pres(i,j,k) = cfl(i,j,k)
      cff_inc_pres(i,j,k) = cff(i,j,k)
...
DO k = 1, tdims%k_end
  DO j = tdims%j_start, tdims%j_end
    DO i = tdims%i_start, tdims%i_end
      ...
      t_inc_pres  (i,j,k) = t(i,j,k)   - t_inc_pres(i,j,k)  -  t_dini(i,j,k)

      ! Other increment variables
      q_inc_pres  (i,j,k) = q(i,j,k)   - q_inc_pres(i,j,k)  -  q_dini(i,j,k)
      qcl_inc_pres(i,j,k) = qcl(i,j,k) - qcl_inc_pres(i,j,k)-qcl_dini(i,j,k)
      qcf_inc_pres(i,j,k) = qcf(i,j,k) - qcf_inc_pres(i,j,k)-qcf_dini(i,j,k)
      cf_inc_pres (i,j,k) = cf(i,j,k)  - cf_inc_pres(i,j,k) - cf_dini(i,j,k)
      cfl_inc_pres(i,j,k) = cfl(i,j,k) - cfl_inc_pres(i,j,k)-cfl_dini(i,j,k)
      cff_inc_pres(i,j,k) = cff(i,j,k) - cff_inc_pres(i,j,k)-cff_dini(i,j,k)
...
! Save cloud increments
IF (l_retain_cld_tendencies) THEN
  
  ! Allocate variables first
  CALL init_cld_tendencies()
  
  ! Save temperature
  DO k = 1, tdims%k_end
    DO j = tdims%j_start, tdims%j_end
      DO i = tdims%i_start, tdims%i_end
        dt_cld(i,j,k) = t_inc_pres(i,j,k) + t_dini(i,j,k)
      END DO
    END DO
  END DO

END IF ! end if l_retain_cld_tendencies
...
END MODULE pc2_pressure_forcing_mod

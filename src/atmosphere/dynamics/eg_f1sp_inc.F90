...
IMPLICIT NONE
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='EG_F1SP_INC_MOD'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='EG_F1SP_INC'
...
REAL, INTENT(OUT)   ::  theta_star(tdims%i_start:tdims%i_end,         &
...
REAL ::                      r_m_r (tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                    tdims%k_start:tdims%k_end)

REAL ::                      r_m_gr(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                    tdims%k_start:tdims%k_end)

REAL ::                     r_m_cf2(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                    tdims%k_start:tdims%k_end)

REAL ::                       r_m_v(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                    tdims%k_start:tdims%k_end)

REAL ::                      r_m_cl(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                    tdims%k_start:tdims%k_end)

REAL ::                      r_m_cf(tdims%i_start:tdims%i_end,        &
                                    tdims%j_start:tdims%j_end,        &
                                    tdims%k_start:tdims%k_end)
...
IF ( PrintStatus == PrStatus_Diag) THEN
  ...
  IF ( mype == 0 ) THEN
    ...
    CALL umPrint( '==============================================',&
        src='eg_f1sp_inc')
    CALL umPrint( 'F1SP predictor (max, min):',src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'U^f1sp          :',max_r_u,min_r_u
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'V^f1sp          :',max_r_v,min_r_v
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'W^f1sp          :',max_r_w,min_r_w
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'theta^f1sp      :',max_r_theta,        &
                                                  min_r_theta
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'm_star^f1sp     :',max_m_star,         &
                                                  min_m_star
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'mcl_star^f1sp   :',max_mcl_star,       &
                                                  min_mcl_star
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'mcf_star^f1sp   :',max_mcf_star,       &
                                                  min_mcf_star
    CALL umPrint(umMessage,src='eg_f1sp_inc')

    IF (L_mcr_qcf2) THEN   
      WRITE(umMessage,'(A,2E15.5)')   'mcf2_star^f1sp  :', &
          max_mcf2_star,      &
          min_mcf2_star
      CALL umPrint(umMessage,src='eg_f1sp_inc')
    END IF
    IF (L_mcr_qrain) THEN  
      WRITE(umMessage,'(A,2E15.5)')  'mrain_star^f1sp :',  &
          max_mrain_star,     &
          min_mrain_star
      CALL umPrint(umMessage,src='eg_f1sp_inc')
    END IF
    IF (L_mcr_qgraup) THEN
      WRITE(umMessage,'(A,2E15.5)') 'mgraup_star^f1sp:',   &
          max_mgraup_star,    &
          min_mgraup_star
      CALL umPrint(umMessage,src='eg_f1sp_inc')
    END IF
  ...
  IF ( mype == 0 ) THEN
    ...
    CALL umPrint(umMessage,src='eg_f1sp_inc')
    WRITE(umMessage,'(A,2E15.5)') 'theta           :',max_thetav,         &
                                                  min_thetav
    CALL umPrint(umMessage,src='eg_f1sp_inc')

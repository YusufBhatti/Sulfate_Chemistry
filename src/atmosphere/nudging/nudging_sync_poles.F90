...
!------------------------------------------------------------------
MODULE nudging_sync_poles_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER,           &
               PRIVATE :: ModuleName = 'NUDGING_SYNC_POLES_MOD'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='NUDGING_SYNC_POLES'
...
IF (PrintStatus > PrStatus_Oper) THEN
  WRITE(umMessage,'(A)') ' NUDGING_SYNC_POLES: Entering Routine'
  CALL umPrint(umMessage,src='nudging_sync_poles')
END IF
...
ELSE
  ...
  IF (PrintStatus > PrStatus_Oper) THEN
    WRITE(umMessage,'(A)')                                                     &
       ' NUDGING_SYNC_POLES:  Not at the Poles, so do nothing'
    CALL umPrint(umMessage,src='nudging_sync_poles')
  END IF
...
IF (PrintStatus > PrStatus_Diag) THEN
  WRITE(umMessage,'(A,5E12.5)')'NUDGING_SYNC_POLES: Sums equal',               &
...
IF (PrintStatus > PrStatus_Oper) THEN
  WRITE(umMessage,*) ' NUDGING_SYNC_POLES: Leaving Routine'
  CALL umPrint(umMessage,src='nudging_sync_poles')
END IF
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='NUDGING_SYNC_POLE_SUM'
...
END MODULE nudging_sync_poles_mod

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='UKCA_STRAT_UPDATE'
...
REAL :: real_time   ! time in seconds

INTEGER(KIND=jpim), PARAMETER :: zhook_in  = 0
INTEGER(KIND=jpim), PARAMETER :: zhook_out = 1
REAL(KIND=jprb)               :: zhook_handle

CHARACTER(LEN=*), PARAMETER :: RoutineName='UKCA_STRAT_PHOTOL'
...
! Calculate time in seconds since start of month. If within 1 hour of 
! start of month, recalculate table if solar cycle is selected
real_time = 60.*REAL(i_minute) + 3660.*REAL(i_hour) +             &
            86400.*REAL(i_day-1)

! work out logic of whether to fill the tables. If at the beginning of 
! the month and the tables haven't been filled, and the solar cycle is 
! activated, then fill the tables.
IF (tables_filled .AND. (real_time >= secs_per_step) .AND.        &
   i_ukca_solcyc>0) THEN 
! Reset tables filled indicator so that the tables will be re-filled at
! the beginning of the next month.
  tables_filled = .FALSE.
END IF
      
! modify this with solar cycle variable when defined
IF ((firstcall) .OR. (.NOT. tables_filled .AND.                   &
        (real_time < secs_per_step))) THEN 
...
IF (z_top_of_model > 38500.0 .AND. z_top_of_model < 39500.0) THEN
  ozonecol = ozonecol + ozcol_39km
ELSE IF (z_top_of_model > 84000.0 .AND. z_top_of_model < 85500.0)                &
  THEN
...
ELSE
  ...
  errcode = 1
  CALL umPrint( cmessage,src='ukca_strat_update')
  WRITE(umMessage,'(A,E12.5)') 'z_top_of_model: ',z_top_of_model
  CALL umPrint(umMessage,src='ukca_strat_update')
...
USE ukca_option_mod, ONLY: jpctr, l_ukca_h2o_feedback, l_ukca_conserve_h
USE um_parvars
USE ereport_mod,        ONLY: ereport
USE errormessagelength_mod, ONLY: errormessagelength
...
IF (direction) THEN
  ...
  IF (l_ukca_h2o_feedback .AND. l_ukca_conserve_h) THEN
...
REAL, PARAMETER :: maxpres = 30.0
REAL, PARAMETER :: minpres = 20.0
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='UKCA_RELAX_OZONE'

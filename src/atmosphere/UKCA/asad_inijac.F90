...
USE asad_mod,        ONLY: advt, ctype, jpfm, jppjac, jpmsp, jpna,     &
                           madvtr, moffam, n_in_ss,                    &
    ...
    IF ( IS < 0 ) THEN
      DO je = 1, ABS(IS)
        ...
        IF ( ifn(ifam(1)) >  2*jpnr ) THEN
          WRITE(umMessage,*) 'INTERNAL ASAD ERROR: array too small ', &
...
DO jc = 1, jpctr
  njx = ifn(jc) / 3
  IF ( njx > jpnjcx3 ) THEN
    WRITE(umMessage,*) 'FATAL ASAD ERROR: array njacx3 too ',         &
...
DO jc = 1, jpctr
  ...
  IF ( ifp(jc) > jppjac ) THEN
    WRITE(umMessage,*) 'FATAL ASAD ERROR: array npjac1 too ',            &

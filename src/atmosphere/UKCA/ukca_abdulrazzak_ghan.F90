...
REAL :: zndtop3(kbdim,klev,nwbins)  ! top line integral for calc pcdncact3
...
!$OMP         zrc, zsm, zsmax, zsum, zw, zwpwdw, zxi, zxv)             &
!$OMP SHARED(cp, cthomi, g, kbdim, klev, lc_sq, mode, nwbins,          &
!$OMP        papm1, pesw, pn, pqm1, prdry, prho, Printstatus,          &
!$OMP        psmax, ptm1, pwarr, pwbin, pwpdf,                         &
!$OMP        za, zb, zcdnc, zeps,                                      &
!$OMP        zndbot, zndbotm, zndtop, zndtop3, zndtopm,                &
!$OMP        zsigmaln)
  ...
  DO jk=1, klev
    DO jl=1, kbdim
      ...
      IF ( (zw > zeps) .AND. (pqm1(jl,jk) > zeps) .AND.                &
        ...
        zka=(5.69+0.017*(ptm1(jl,jk)-273.15))*1.0e-5
  ...
  DO jk=1, klev
    DO jl=1, kbdim
      ...
      IF (zwpwdw < zeps .AND. Printstatus == prstatus_diag) THEN
        WRITE(umMessage,*) 'RW: zwpwdw(',jl,',',jk,')= ',  zwpwdw
        CALL umPrint(umMessage,src='ukca_abdulrazzak_ghan')
        WRITE(umMessage,*) 'RW: pwpdf(',jl,',',jk,',',jw,')=  ', pwpdf(jl,jk,jw)
        CALL umPrint(umMessage,src='ukca_abdulrazzak_ghan')
        WRITE(umMessage,*) 'RW: pwbin(',jl,',',jk,',',jw,')=  ', pwbin(jl,jk,jw)
        CALL umPrint(umMessage,src='ukca_abdulrazzak_ghan')
        WRITE(umMessage,*) 'RW: pwarr(',jl,',',jk,',',jw,')=  ', pwarr(jl,jk,jw)
        CALL umPrint(umMessage,src='ukca_abdulrazzak_ghan')
      END IF
      DO jmod=1, nmodes
        IF (mode(jmod)) THEN
      ...
      IF (zcdnc(jl,jk,jw) > zeps) THEN
        zndtop3(jl,jk,jw) = (zcdnc(jl,jk,jw)**(-1.00/3.00))*zpwdw
      END IF
    END DO ! jl
  END DO ! jk
...
!$OMP DO SCHEDULE(STATIC) 
  DO jk=1, klev
    DO jl=1, kbdim
      ...
      zndtop3(jl,jk,1) = zndtop3(jl,jk,1) + zndtop3(jl,jk,jw)
    END DO
  END DO
!$OMP END DO NOWAIT
END DO
...
DO jk=1, klev
  DO jl=1, kbdim
    ...
    IF (zndbot(jl,jk,1) > zeps) THEN
      ...
      pcdncact3(jl,jk)=zndtop3(jl,jk,1)/zndbot(jl,jk,1)
    ELSE
      IF (PrintStatus == Prstatus_diag) THEN

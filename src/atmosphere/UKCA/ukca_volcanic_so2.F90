...
REAL, PARAMETER :: lat_volc(nerupt) =                             &
                (/  46.0,   17.3,  4.89,  15.1,  -46.0,  0.0/)
! longitude / degrees of volcanoes
REAL, PARAMETER :: lon_volc(nerupt) =                             &
                (/ 238.0,  266.8,284.63, 120.4,  287.0,  0.0/)
! year of eruption
...
REAL, PARAMETER :: magnitude(nerupt)  =                           &
                (/0.6e9, 13.4e9, .66e9, 20.0e9, 1.5e9, 10.0e9/)
...
REAL, PARAMETER :: height_top(nerupt) =                           &
                (/22000.0,32000.0,26000.0,29000.0,22000.0,30000.0/)
...
DO k=1,nerupt
  ...
  IF ((lat_index(k) > 0) .AND. (year == year_volc(k))) THEN
    ...
    IF ( trigger_calc .OR. (i_day_number > start_volc(k) .AND.  &
      ...
      j = lat_index(k)
      l1 = 1
      DO WHILE (r_theta_levels(i,j,l1) - planet_radius <= 19000.0)

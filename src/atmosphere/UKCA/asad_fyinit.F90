...
USE asad_mod,             ONLY: ctype, fch4, fco2, f, fh2, fn2, fo2,   &
                                ilcf, ilct, ilftr, ilss,               &
                                jpcf, jpco, jpif, jpna, jpsp, jsubs,   &
                                madvtr, moffam, nlmajmin, peps,        &
                                speci, tnd, y
USE ukca_diurnal_oxidant, ONLY: ukca_set_diurnal_ox, ukca_set_diurnal_ox_col 
USE ukca_option_mod, ONLY: jpspec, l_ukca_offline, l_ukca_offline_be, &
                           l_ukca_asad_columns
...
IF ( ofirst ) THEN
  ...
  DO j = 1, ict
    ...
    ELSE IF ( speci(js) == 'CO2       ' ) THEN
      zcnst = fco2            ! non-interactive CO2
...
IF (ofirst .AND. (l_ukca_offline .OR. l_ukca_offline_be)) THEN
  DO j = 1, icf
     js = ilcf(j)
     IF (L_ukca_asad_columns) THEN

...
! start point for each atm pe
INTEGER, POINTER     :: offset_map(:,:,:)=>NULL()
...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='IOS_MODEL_GEOMETRY'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='IOS_DUMP_INIT'
...
IF (model_rank==0) THEN
  DO server=1,IOS_Server_Groups
    DO sub_task=1,IOS_Metadata_Receivers()
...
!$OMP CRITICAL(internal_write)
        WRITE(IOS_message,'(A,I4,A,I4)')                                   &
            'Info: Sending landmask to server ',                           &
            server,', @ global rank ',io_servers(server,sub_task)
!$OMP END CRITICAL(internal_write)
        CALL IOS_print(IOS_message,src='ios_model_geometry')
      END IF
    END DO
  END DO
END IF
...
INTEGER, INTENT(IN)       :: numpoints
INTEGER, POINTER          :: mask(:) ! Note this is really logical data
REAL(KIND=jprb)           :: zhook_handle

CHARACTER(LEN=*), PARAMETER :: RoutineName='IOS_DUMPINITMODEL'
...
!$OMP CRITICAL(internal_write)
WRITE(IOS_message,'(A,I8)')'Allocated Global Land Mask on IOS Points=',    &
    numpoints
!$OMP END CRITICAL(internal_write)
CALL IOS_print(IOS_message,src='ios_model_geometry')
IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_out,zhook_handle)
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='IOS_CLIENT_GEOMETRY_INIT'
...
!$OMP CRITICAL(internal_write)
        WRITE(IOS_message,'(A,I4,A,I4)')                                   &
            'Info: Sending model geometry to server ',                     &
            server,', @ global rank ',io_servers(server,sub_task)
!$OMP END CRITICAL(internal_write)
        CALL IOS_print(IOS_message,src='ios_model_geometry')
      END IF
...
!$OMP CRITICAL(internal_write)
  WRITE(IOS_message,'(A,I2)')                                              &
      'Info: Asynchronous message target rank: ',                          &
      target_ios_rank
!$OMP END CRITICAL(internal_write)
  CALL IOS_print(IOS_message,src='ios_model_geometry')
END IF
...
IF (IOS_Verbosity >= IOS_PrStatus_Diag) THEN
  ...
  CALL IOS_print(IOS_message,src='ios_model_geometry')
  CALL IOS_print                                                           &
      ('^ North      IOS Geometry initisialisation',                       &
      src='ios_model_geometry')
  CALL IOS_print                                                           &
      ('|            -----------------------------',                       &
      src='ios_model_geometry')
  CALL IOS_print('|',src='ios_model_geometry')
  CALL IOS_print('+-----> East',src='ios_model_geometry')
  CALL IOS_print('',src='ios_model_geometry')
  ...
  DO fld_type=1,nfld_types
    ...
    CALL IOS_print(IOS_message,src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A,I2)')'Field Type ',fld_type
!$OMP END CRITICAL(internal_write)
    CALL IOS_print(IOS_message,src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A,I5)')'  CPU =',model_rank
!$OMP END CRITICAL(internal_write)
    CALL IOS_print(IOS_message,src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A,I5,A,I5)')'  Grid=',                             &
        atm_global_points(1,fld_type),'x',                                 &
        atm_global_points(2,fld_type)
!$OMP END CRITICAL(internal_write)
    CALL IOS_print(IOS_message,src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A,I5,A,I5)')' LSize=',                             &
        size_map(1,fld_type,model_rank),'x',                               &
        size_map(2,fld_type,model_rank)
!$OMP END CRITICAL(internal_write)
    CALL IOS_print(IOS_message,src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A,i5,A,i5)')                                       &
...
!$OMP END CRITICAL(internal_write)
    ...
    CALL IOS_print                                                         &
        ('         |          |         ',src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A,I5,A)')'         |    ',model_rank,' |         '
!$OMP END CRITICAL(internal_write)
    CALL IOS_print(IOS_message,src='ios_model_geometry')
    CALL IOS_print                                                         &
        ('         |          |         ',src='ios_model_geometry')
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='COMPUTE_OFFSETS'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='IOS_SERVER_GEOMETRY_INIT'
...
!$OMP CRITICAL(internal_write)
  WRITE(geom_message,'(A,I0,A,I0)')'Data buffer wrong size: Expected ',    &
...
!$OMP CRITICAL(internal_write)
    WRITE(geom_message,'(A,I0)')'table error: reverse lookup gave cpu ',   &
        newproc
!$OMP END CRITICAL(internal_write)
...
IF (IOS_Verbosity >= IOS_PrStatus_Diag) THEN
  ...
  WRITE(IOS_message,*)
  CALL IOS_print(IOS_message,src='ios_model_geometry')
  WRITE(IOS_message,'(A3,1x,A3,1x,A4,1x,A4,1x,A4,1x,A4,1x,A4,1x,A4)')      &
  ...
  DO fld_type=1,nfld_types
    DO proc=0,atm_numprocs-1
!$OMP CRITICAL(internal_write)
      WRITE(IOS_message,'(I3,1x,I3,1x,I4,1x,I4,1x,I4,1x,I4,1x,I4,1x,I4)')  &
          proc,fld_type,                                                   &
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SETMAXFIELDDOMAIN'
...
!$OMP CRITICAL(internal_write)
WRITE(IOS_message,'(A,I7,A)')'IOS_Model_Geometry: Info: Max Field Domain=',&
    MaxFieldDomain,' words'
!$OMP END CRITICAL(internal_write)
CALL IOS_print(IOS_message,src='ios_model_geometry')
!$OMP CRITICAL(internal_write)
WRITE(IOS_message,'(A,I6,A,I2)')'IOS_Model_Geometry: Info: for CPU=',mcpu, &
    ' Field type=',mfld
!$OMP END CRITICAL(internal_write)
CALL IOS_print(IOS_message,src='ios_model_geometry')

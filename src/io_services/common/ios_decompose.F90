...
IMPLICIT NONE
INTEGER :: IOS_Decompose_Plan = 0
INTEGER :: IOS_Decompose_Max_Plan = 1
...
IF ( IOS_Decompose_Plan == 0) THEN
  ! Allocate imbalance over the lowest bins
  IF ( local_bin < remainder ) local_items=local_items+1
  num_below=MIN(remainder,local_bin)
ELSE IF ( IOS_Decompose_Plan == 1) THEN
  ! Allocate imbalance over the highest bins
  IF ( bins-local_bin-1 < remainder ) local_items=local_items+1
  num_below=MAX(remainder-bins+local_bin,0)
END IF

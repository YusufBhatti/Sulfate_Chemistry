...
MODULE ios_dump
USE ios_stash
USE cppxref_mod, ONLY: ppx_atm_tzonal,ppx_atm_uzonal, ppx_atm_scalar
USE um_types
IMPLICIT NONE
...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='IOS_DUMP'
...
CHARACTER (LEN=errormessagelength)              :: IOS_dmp_message
CHARACTER (LEN=*), PARAMETER     :: RoutineName = 'IOS_DUMP_PACK_DATA'
...
! Correction for scalar field
IF (arg_grid_type  ==  ppx_atm_scalar) THEN
  domainBox%e = 1
  domainBox%w = 1
  domainBox%n = 1
  domainBox%s = 1
...
!$OMP CRITICAL(internal_write)
  WRITE(IOS_dmp_message,'(A,I0,A,I0,A)')                                   &
...
REAL(KIND=jprb)                    :: zhook_handle
CHARACTER (LEN=*), PARAMETER       :: RoutineName = 'IOS_DUMP_WRITE_DATA'

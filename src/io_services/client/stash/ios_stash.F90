...
REAL                   :: IOS_async_stash_stall_time = 0.0
INTEGER, PARAMETER     :: async_tag_base             = 100000
INTEGER, PARAMETER     :: IOS_async_max_handle       = 499
! params for dr_hook
INTEGER(KIND=jpim), PARAMETER, PRIVATE :: zhook_in   = 0
INTEGER(KIND=jpim), PARAMETER, PRIVATE :: zhook_out  = 1
CHARACTER (LEN=errormessagelength), PRIVATE           :: IOS_st_message

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='IOS_STASH'
...
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_STASH_CLIENT_INIT'
...
USE mpl, ONLY: mpl_status_size, mpl_real, mpl_max
IMPLICIT NONE
INTEGER              :: indx
LOGICAL              :: msg
INTEGER              :: ierror
...
REAL(KIND=jprb)      :: zhook_handle
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_STASH_CLIENT_FINI'
...
IF (IOS_Verbosity>=IOS_prstatus_oper) THEN
  ...
  CALL IOS_print(IOS_message,src='ios_stash')
END IF
...
!$OMP CRITICAL(internal_write)
      WRITE(IOS_Message,'(A,I4,A,I3,A,I4,A,I3,A,I3)')                      &
          'Unused slot: ',indx,                                            &
          ' unit=',slot(indx)%UNIT,                                        &
...
WRITE(IOS_message,'(A,F8.3)')                                              &
    'Info: IOS_Stash: Time waiting for all requests to complete: ',t2-t1
CALL IOS_print(IOS_message,src='ios_stash',level=IOS_prstatus_oper)

WRITE(IOS_message,'(A,F8.3,A)')                                            &
    'Info: IOS_Stash: Stalls waiting for async buffer took ',              &
    IOS_async_stash_stall_time,' seconds'
CALL IOS_print(IOS_message,src='ios_stash',level=IOS_prstatus_oper)

CALL MPL_Reduce(IOS_async_stash_stall_time,t1,1,mpl_real,                  &
    mpl_max,0,model_comm,ierror)

IF (model_rank==0) THEN
  WRITE(IOS_message,'(A,A,F10.3,A)')                                        &
      'Info: IOS_Stash: ','     Maximum was ',t1,' seconds'
  CALL IOS_print(IOS_message,src='ios_stash',level=IOS_prstatus_oper)
END IF
...
INTEGER              :: mplStat(mpl_status_size)
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_STASH_NEXT_BUFFER'

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)

msg=(IOS_Verbosity>=IOS_prstatus_debug)

IF (msg) THEN
!$OMP CRITICAL(internal_write)
  WRITE(IOS_Message,'(A,A,I3)')'Info: ios_stash: ',                        &
...
!$OMP CRITICAL(internal_write)
      WRITE(IOS_message,'(A,I3)')'Next buffer request for unit=',UNIT
!$OMP END CRITICAL(internal_write)
      CALL IOS_print(IOS_message,src='ios_stash')
!$OMP CRITICAL(internal_write)
      WRITE(IOS_message,'(A,I4,A)')                                        &
          'But an undispatched slot ',indx,' for this unit'
!$OMP END CRITICAL(internal_write)
      CALL IOS_print(IOS_message,src='ios_stash')
      CALL IOS_print('exists!',src='ios_stash')
...
!$OMP END CRITICAL(internal_write)
    ...
    IF (need_timer) THEN
      ...
      IF (t2-t1 > IOS_Timeout) THEN
        CALL IOS_print('Timeout value tripped waiting for stash')
  ...
  IF (need_timer) THEN
    t2=Get_Wallclock_time()
    IF (IOS_Verbosity>=IOS_prstatus_oper) THEN
      WRITE(IOS_Message,'(A,A,F10.3,A,F10.3)')                              &
          'Info: IOS_Stash: ',                                             &
...
!$OMP CRITICAL(internal_write)
    WRITE(IOS_Message,'(A,A,I3,A)')'Info: ios_stash: ',                    &
        'Slot ',s,' is now free.'
!$OMP END CRITICAL(internal_write)
    CALL IOS_print(IOS_message,src='ios_stash')
  END IF
...
ELSE
  ...
  IF (IOS_Verbosity>=IOS_prstatus_normal) THEN
    WRITE(IOS_Message,'(A,A)')'Info: IOS_stash: ',                         &
        'No slot could be freed, something needs dispatching'
    CALL IOS_print(IOS_message,src='ios_stash')
    CALL IOS_print('Info: IOS_stash:   or more slots allocating')
  END IF
...
REAL(KIND=jprb)     :: zhook_handle
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_STASH_PACK_PP_DATA'
...
!$OMP END CRITICAL(internal_write)
  CALL IOS_print(IOS_st_message,src='ios_stash')
  WRITE(IOS_Message,'(A,A)')                                               &
...
! We used to pack level here, but it will break the repeat field check.
...
CALL IOS_pack_async_buffer(s,subdomain,local_len,control,subdomain_type)
...
SUBROUTINE IOS_pack_async_buffer( s , array, arrayLen, cont, domType)
...
INTEGER                             :: clen     !metadata length
REAL(KIND=jprb)                     :: zhook_handle
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_PACK_ASYNC_BUFFER'
...
IF (IOS_isRepeatRecord(slot(s),cont)) THEN

  ! Check we have space...
  CALL IOS_assertSpace(slot(s),1)
  ...
  slot(s)%control(slot(s)%control_offset+loc_record_len_control)=          &
      -1! The user control block length is 1 for a repeat record.
#if defined(IOS_DEBUG)
  slot(s)%control(slot(s)%control_offset+loc_record_handle)=               &
      -1*slot(s)%handle
  slot(s)%control(slot(s)%control_offset+loc_record_record)=               &
      -1*slot(s)%num_records_in_pack
  slot(s)%control(slot(s)%control_offset+loc_record_len_data)=             &
      -arrayLen! record data len
  slot(s)%control(slot(s)%control_offset+loc_record_slot)=                 &
      -s! source slot number
#endif
...
ELSE
  ...
  ! loc_record_start is "1" by definition - ie the 1st word in control
  ! for the new item
  slot(s)%lastRecord=slot(s)%control_offset+loc_record_start

  ! Check we have space...
  CALL IOS_assertSpace(slot(s),clen)

  ! Fill out the mandatory bit of the metadata (metametadata)
  slot(s)%control(slot(s)%control_offset+loc_record_start)=                &
      IOS_stash_record_start
  slot(s)%control(slot(s)%control_offset+loc_record_len_control)=          &
      -clen ! record user control block length
#if defined(IOS_DEBUG)
  slot(s)%control(slot(s)%control_offset+loc_record_handle)=               &
      -1*slot(s)%handle
  slot(s)%control(slot(s)%control_offset+loc_record_record)=               &
      -1*slot(s)%num_records_in_pack
  slot(s)%control(slot(s)%control_offset+loc_record_len_data)=             &
      -arrayLen ! record data length
  slot(s)%control(slot(s)%control_offset+loc_record_len_slot)=             &
      -s  ! source slot number
#endif
...
IF (DomType /= no_intersection .OR. IOS_AsyncSendNull) THEN
  ...
  CALL IOS_assertDataSpace(slot(s),arrayLen)
...
INTEGER(KIND=ios_tukind),  POINTER    :: sendBuffer(:)
REAL(KIND=jprb)                       :: zhook_handle
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_STASH_WRITE_PP_DATA'
...
IF (model_rank==0) THEN
  ...
  DO targetRank=1,IOS_Metadata_Receivers()
    qHandle = ios_init_md(                                                 &
        slot(s)%UNIT,-1,                                                   &
        IOS_Action_StashWritePPData,                                       &
        dataSize=slot(s)%control_offset,targetRank=targetRank)
...
REAL(KIND=jprb)            :: zhook_handle
CHARACTER (LEN=*), PARAMETER                                               &
    :: RoutineName = 'IOS_STASH_DISPATCH'
...
!$OMP CRITICAL(internal_write)
  WRITE(IOS_Message,'(A,A,I3,A,I3,A,I4,A,I3,A,I4)')                        &
      'Info: IOS_stash: ',                                                 &

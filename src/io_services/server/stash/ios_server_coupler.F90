...
INTEGER :: proc
INTEGER :: errCode
CHARACTER (LEN=errormessagelength)               :: errorMessage

IF (IOS_Verbosity>=IOS_PrStatus_Oper) THEN
  CALL IOS_print('',src='ios_server_coupler')
  CALL IOS_print('Coupling relationship: This IOS rank owns',              &
      src='ios_server_coupler')
  CALL IOS_print('',src='ios_server_coupler')
  WRITE(IOS_message,'(A15,A12,A10,A10,A10)')'Item','Field Type','From',    &
      'To','Total'
  CALL IOS_print(IOS_message,src='ios_server_coupler')
  WRITE(IOS_message,'(A15,A12,A10,A10,A10)')'----','----------','----',    &
      '--','-----'
  CALL IOS_print(IOS_message,src='ios_server_coupler')
END IF
...
IF (IOS_Verbosity>=IOS_PrStatus_Oper) THEN
!$OMP CRITICAL(internal_write)
  WRITE(IOS_message,'(A15,A12,I10,I10,I10)')'processors','   ',            &
      proc_start,proc_end,proc_end-proc_start+1
!$OMP END CRITICAL(internal_write)
  CALL IOS_print(IOS_message,src='ios_server_coupler')
!$OMP CRITICAL(internal_write)
  WRITE(IOS_message,'(A15,A12,I10,I10,I10)')'processor rows','   ',        &
      proc_row_start,proc_row_end,                                         &
...
DO fld_type=1,nfld_types
  ...
  IF (IOS_Verbosity>=IOS_PrStatus_Oper) THEN
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A15,I12,I10,I10,I10)')'grid rows',fld_type,        &
...
!$OMP END CRITICAL(internal_write)
  ...
  IF (IOS_Verbosity>=IOS_PrStatus_Oper) THEN
!$OMP CRITICAL(internal_write)
    WRITE(IOS_message,'(A15,I12,I10,I10,I10)')'grid points',fld_type,      &

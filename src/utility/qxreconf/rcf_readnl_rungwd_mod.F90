...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_READNL_RUNGWD_MOD'
...
USE g_wave_input_mod, ONLY:                                              &
  run_gwd, l_gwd, l_use_ussp, print_nlist_run_gwd, read_nml_run_gwd
...
! Read the namelist
CALL read_nml_run_gwd(nft)

IF ( PrintStatus >= PrStatus_Oper .AND. mype == 0) THEN
  CALL print_nlist_run_gwd()
END IF

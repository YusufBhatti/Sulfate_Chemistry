...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_INIT_CANOPY_WATER_MOD'
...
USE jules_surface_types_mod, ONLY: &
    npft, urban, urban_canyon, urban_roof
...
CHARACTER (LEN=*), PARAMETER  :: RoutineName='RCF_INIT_CANOPY_WATER'
...
IF (mype == 0 .AND. PrintStatus >= PrStatus_Normal ) THEN
  CALL umPrint( 'Initialising tile canopy '//&
...
!----------------------------------------------------------------------
! Interpolate mean_canopyw and copy result to vegetation and urban
! tiles. On other tiles set canopy_water to 0.
!----------------------------------------------------------------------
...
ELSE
  ...
  DO i = npft+1, canopy_water % levels
    IF ( i == urban .OR. i == urban_canyon .OR. i == urban_roof ) THEN
      canopy_water % DATA(:,i) = mean_canopyw_out % DATA(:,1)
    ELSE

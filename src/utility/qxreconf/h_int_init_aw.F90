...
SUBROUTINE h_int_init_aw(icof,idim2,max_field_out                 &
...
USE Rcf_HeadAddress_Mod, ONLY:                                    &
    ...
    FH_RowDepCStart,  FH_CoordSystem,   FH_CoordSystem_Cartesian, &
    FH_CoordSystem_RegLonLat
...
INTEGER  ::  idim2             !Second dimension of index arrays
...
INTEGER  ::  aw_index_targ_lhs(row_length_out+1,idim2)
                              !Index of source box overlapping
                              !lhs of target grid-box
INTEGER  ::  aw_index_targ_top(max_rows_out+1,idim2)
                              !Index of source box overlapping
                              !top of target grid-box
INTEGER  ::  bl_index_b_l(max_field_out,idim2)
                              !Gather index for bottom l.h.c of
                              !source grid box. 1=P-pts; 2=UV-pts
INTEGER  ::  bl_index_b_r(max_field_out,idim2)
                              !Gather index for bottom r.h.c of
                              !source grid box. 1=P-pts; 2=UV-pts
INTEGER  ::  bl_index_t_l(max_field_out,idim2)
                              !Gather index for top l.h.c of
                              !source grid box. 1=P-pts; 2=UV-pts
INTEGER  ::  bl_index_t_r(max_field_out,idim2)
                              !Gather index for top r.h.c of
                              !source grid box. 1=P-pts; 2=UV-pts
REAL     ::  aw_area_box(idim2) !area of grid box in sq units of
                              !  source grid
REAL     ::  aw_colat_t(max_rows_out+1,idim2)
                              !Colatitude of top of target grd-box
                              ! (in units of DELTA_LAT_SRCE)
REAL     ::  aw_long_l(row_length_out+1,idim2)
                              !Left longitude of target grid-box
                              ! (in units of DELTA_LONG_SRCE)
REAL     ::  weight_t_r(max_field_out,idim2) ! Weights for bilinear
REAL     ::  weight_b_r(max_field_out,idim2) !\horizontal interpolatn
REAL     ::  weight_t_l(max_field_out,idim2) !/ 1=P-pts; 2=U-pts;
REAL     ::  weight_b_l(max_field_out,idim2) ! 3=V-pts;4=zonal mean


INTEGER  ::  ErrorStatus          ! Error flag (0 = OK)

! Local parameters:
CHARACTER (LEN=*), PARAMETER :: RoutineName='H_INT_INIT_AW'
...
IF (FixHd_out( FH_HorizGrid ) == 0 .AND.                                &
  ...
  CALL umPrint( ' *ERROR* Trying to interpolate from a hemispheric' //  &
      ' or LAM to a global domain',src='h_int_init_aw')
...
IF (FixHd_out( FH_HorizGrid ) <  3 .AND.                             &
  ...
  CALL umPrint( ' *ERROR* Trying to interpolate from a limited ' //      &
      'area domain to a global or hemispheric domain',src='h_int_init_aw')
...
IF (FixHd_out( FH_CoordSystem ) == FH_CoordSystem_Cartesian) THEN
  ! Cartesian output grid: use the original LAM co-ordinates from lam_config:

  IF (FixHd_in( FH_CoordSystem ) == FH_CoordSystem_Cartesian) THEN
    ! Input cartesian grid -> output Cartesian grid
    ! Area weighted option not supported at present

    CALL umPrint( ' *ERROR* Trying to use area weighting for ' //      &
      'Cartesian to Cartesian',src='h_int_init_aw')
    Cmessage = 'Cartesian to Cartesian area weighting not coded'
    ErrorStatus = 5
    CALL Ereport( RoutineName, ErrorStatus, Cmessage )

  ELSE  ! Input dump lat-lon
  ! Cartesian output grid: use the original LAM co-ordinates from lam_config:
  ...
  ! 2.1a: Grid spacing
    delta_lat_target=target_delta_lat
    delta_lon_target=target_delta_lon
  
    ! 2.2a: Coordinates of north pole on grid
    npole_lat_target=RealHd_out( RC_PoleLat )
    npole_lon_target=RealHd_out( RC_PoleLong )
  
    ! 2.3a: Coordinates of top left hand p-point on grid
    start_lat_target=target_frstlata
    start_lon_target=target_frstlona
  END IF

ELSE

  ! Non-Cartesian output grid: use the domain stored in the real header:
  ! 2.1b: Grid spacing
  delta_lat_target=RealHd_out( RC_LatSpacing )
  delta_lon_target=RealHd_out( RC_LongSpacing )
  
  ! 2.2b: Coordinates of north pole on grid
  npole_lat_target=RealHd_out( RC_PoleLat )
  npole_lon_target=RealHd_out( RC_PoleLong )
  
  ! 2.3b: Coordinates of top left hand p-point on grid
...
! 2.2c: Coordinates of north pole on grid
npole_lat_source=RealHd_in( RC_PoleLat )
npole_lon_source=RealHd_in( RC_PoleLong )

! 2.3c: Coordinates of top left hand p-point on grid
...
IF (npole_lat_source /= npole_lat_target .AND.                      &
  ...
  CALL umPrint( 'Source and target grids have different poles')
  WRITE(umMessage,'(A,A)') 'Reconfigure onto a grid with the same pole as'      &
,                  'target grid'
  CALL umPrint(umMessage,src='h_int_init_aw')
  CALL umPrint( 'before attempting area weighted interpolation')
...
CALL f_shum_horizontal_field_bi_lin_interp_get_coeffs                      &
                      (bl_index_b_l(1,1),bl_index_b_r(1,1),                &
                       ...
                       row_length_in,max_rows_in,max_field_out,            &

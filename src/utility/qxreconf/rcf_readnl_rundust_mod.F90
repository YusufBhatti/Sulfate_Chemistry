...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_READNL_RUNDUST_MOD'
...
USE dust_parameters_mod, ONLY:                                           &
  RUN_Dust, dust_parameters_load, print_nlist_run_dust,                  &
  read_nml_run_dust
...
! Read the namelist
CALL read_nml_run_dust(nft)

IF ( PrintStatus >= PrStatus_Oper .AND. mype == 0) THEN
  CALL print_nlist_run_dust()
END IF

...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_CLOUD_FRAC_CHK_MOD'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='RCF_CLOUD_FRAC_CHK'
  ...
  IF (qcf     % interp == interp_done .OR.   &
    ...
    DO k = 1, qcf % levels
      ...
      IF (PrintStatus >= PrStatus_Diag .AND. mype == 0) THEN
        WRITE(umMessage,'(A,I4,A,I0)') ' Level ',k,                            &
        ' Cloud fracs (frozen) reset to zero ',COUNT
        CALL umPrint(umMessage,src='rcf_cloud_frac_chk_mod')
      END IF

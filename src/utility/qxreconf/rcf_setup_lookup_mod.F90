...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_SETUP_LOOKUP_MOD'
...
USE stparam_mod, ONLY: &
    st_levels_model_rho, st_levels_model_theta

USE umPrintMgr, ONLY:      &
    umPrint,                &
    umMessage

! Required for JULES flexible tiles
USE jules_surface_types_mod, ONLY: ntype, npft, nnvg
...
USE packing_codes_mod, ONLY: PC_No_Packing, PC_BitMask_CompressType,           &
    PC_LandMask_Compression, PC_FieldIDX_CompressType
    ...
    DO WHILE (ASSOCIATED(recondat_node % recondat_info))
      ...
      DO  k=k_out,k_out+n_levels*n_plevels-1
        ...
        IF (n_levels > 1) THEN
          ...
          IF ( Output_Grid % height_gen_method == height_gen_smooth        &
            .OR. Output_Grid % height_gen_method == height_gen_linear ) THEN

            IF (STM_record % lbvc_code == 9 .OR.                           &
                STM_record % lbvc_code ==65 ) THEN  ! Hybrid/Eta levels

              IF (STM_record % lv_code == st_levels_model_theta) THEN
                ...
                ELSE IF ( lev > Output_Grid % model_levels ) THEN
                  ...
                  Lookup( bulev, lev )  = TRANSFER( 0.0, int_val )
                  Lookup( blev, lev )   = TRANSFER( 0.0, int_val )
                  Lookup( brlev, lev )  = TRANSFER( 0.0, int_val )
                  Lookup( bhulev, lev ) = TRANSFER( 0.0, int_val )
                  Lookup( bhlev, lev )  = TRANSFER( 0.0, int_val )
                  Lookup( bhrlev, lev ) = TRANSFER( 0.0, int_val )
              ...
              ELSE IF (STM_record % lv_code == st_levels_model_rho ) THEN
          ...
          ELSE IF (Output_Grid % height_gen_method ==                      &
            ...
            IF (STM_record % lbvc_code == 9 .OR.                           &
              ...
              IF (STM_record % lv_code == st_levels_model_theta) THEN
                ...
                IF ( lev == Output_Grid % model_levels ) THEN ! Top level
                  ...
                  Lookup( brlev, k ) = TRANSFER(                           &
                      Output_Grid % eta_rho_levels  ( Lev), int_val )
                ELSE IF ( lev > Output_Grid % model_levels ) THEN
                  Lookup( bulev, k ) = TRANSFER( 0.0, int_val )
                  Lookup( blev, k  ) = TRANSFER( 0.0, int_val )
                  Lookup( brlev,k )  = TRANSFER( 0.0, int_val )
                ELSE
                  Lookup( bulev, k ) = TRANSFER(                           &
              ...
              ELSE IF ( STM_record % lv_code == st_levels_model_rho ) THEN
                ...
                END IF
              ELSE
                WRITE(umMessage,*) 'ERROR:- lv_code=',STM_record % lv_code
                CALL umPrint(umMessage,src='rcf_setup_lookup_mod')
        ...
        IF (STM_Record % pt_code > 0) THEN
          ...
          ! Flexible surface tiles - copy id numbers to lookup
          ! A user-defined selection of surface types is allowed
          ! Tile configuration (surface_type_ids) or multi-layer
          ! snow ids (ml_snow_type_ids) copied to lbplev.
          ! Input fields are checked against tile configuration
          ! elsewhere in the rcf.
          IF (STM_Record % pt_code == 9 .AND. .NOT. l_aggregate) THEN
            IF (STM_record % pl_code == 11  ) THEN
              Lookup(lbplev,k) = ml_snow_type_ids(k-k_out+1)
            ELSE
              Lookup(lbplev,k) = surface_type_ids(k-k_out+1)
            END IF
          END IF
...
DO k=1, Hdr_Out % Len2Lookup
  ...
  Lookup( lbsrce,k ) = um_version_int*10000 + model_id

  IF (Lookup( data_type,k )  ==  0) THEN
    Lookup( data_type,k ) = STM_Record % data_type
  END IF

  IF (Lookup( lbpack,k )  ==  PC_No_Packing) THEN
...
DO k=1, Hdr_Out % Len2Lookup
  ...
  IF (AREA_Expand(k) == 1) THEN
    ...
    IF (n2 == PC_BitMask_CompressType .AND. n3 == PC_LandMask_Compression) THEN
      Lookup( lbrow,k ) = 0
      Lookup( lbnpt,k ) = 0
    ELSE IF (n2 == PC_FieldIDX_CompressType                                    &
             .AND. n3 == PC_LandMask_Compression) THEN
      Lookup( lbrow,k ) = 0
      Lookup( lbnpt,k ) = 0
    ELSE

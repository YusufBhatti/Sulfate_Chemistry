...
CHARACTER(LEN=*), PARAMETER  :: RoutineName='BOX_BND'
...
IF ( printstatus >= prstatus_diag ) THEN
  WRITE(umMessage,FMT='(A,F9.4)')"Latitude increment of source grid = ",      &
       delta_lat_srce
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Latitude increment of target grid  = ",     &
       delta_lat
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Longitude increment of source grid = ",     &
       delta_long_srce
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Longitude increment of target grid = ",     &
       delta_long
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Start latitude of centre of first grid " // &
       "box in target area = ", start_lat
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Start latitude of centre of first grid " // &
       "box in source area = ", start_lat_srce
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Start longitude of centre of first "     // &
       "grid in target area = ", start_long
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,FMT='(A,F9.4)')"Start longitude of centre of first "     // &
       "grid in source area = ", start_long_srce
  CALL umPrint(umMessage,src='box_bnd')
END IF
! The definition of colat is that it is 0.0 at the south pole and
! increasing to source_rows at the north pole.
! This is due to changing grid from north to south
start_long_box = start_long - 0.5*delta_long
start_colat_box = (start_lat + 90.0) - 0.5*delta_lat
start_long_box_srce = start_long_srce - 0.5*delta_long_srce
start_colat_box_srce = (start_lat_srce + 90.0) -0.5*delta_lat_srce

IF (global) THEN
  IF (igrid_srce == 1 .AND. near_equal_real(start_lat_srce,-90.0)) THEN
    cmessage = ' BOX_BND: source grid not global'
    errorstatus = 1
    CALL ereport(routinename, errorstatus, cmessage)
  END IF
  IF (igrid == 1 .AND. near_equal_real(start_lat,-90.0)) THEN
...
ELSE
  WRITE(umMessage,*) 'BOX_BND'
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,*) 'start_colat_box      ',start_colat_box
  CALL umPrint(umMessage,src='box_bnd')
  WRITE(umMessage,*) 'start_colat_box_srce ',start_colat_box_srce
  CALL umPrint(umMessage,src='box_bnd')
  IF (near_equal_real(start_colat_box,start_colat_box_srce)) THEN
...
IF (.NOT. global) THEN
  IF (long_offset <  0.0) THEN
    WRITE(umMessage,*) ' LONG_OFFSET = ',long_offset,' ; Reset to 0.0'
    CALL umPrint(umMessage,src='box_bnd')
    long_offset = 0.0
  END IF
  IF (colat_offset <  0.0) THEN
    WRITE(umMessage,*) ' COLAT_OFFSET = ',colat_offset,' ; Reset to 0.0'
    CALL umPrint(umMessage,src='box_bnd')
    colat_offset = 0.0
  END IF
END IF
...
j_t(1) = FLOOR(colat_t(1) + 1.0)
DO j=2,rows+1
  colat_t(j) = colat_offset  + (j-1)*ns_box
  j_t(j) = FLOOR(colat_t(j) + 1.0)
END DO

...
! *****************************COPYRIGHT*******************************
!  Idealised baroclinic setup

MODULE rcf_ideal_baroclinic_mod

IMPLICIT NONE

CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_IDEAL_BAROCLINIC_MOD'

CONTAINS

! Subroutine rcf_ideal_baroclinic
!
! Description:
...
SUBROUTINE rcf_ideal_baroclinic ( hdr_out, xi3_at_theta, xi3_at_rho, xi3_at_u, &
...
USE rcf_ideal_baro_pert_mod, ONLY: &
    baro_pert
...
CHARACTER (LEN=*),  PARAMETER :: routinename='RCF_IDEAL_BAROCLINIC'
...
! Add perturbation
   IF (l_baro_perturbed) THEN
    u_out % data(i,k) = u_out % data(i,k) + baro_pert(xi1_u_1d(i), xi2_p_1d(i))
   END IF
  END DO
END DO
...
RETURN
END SUBROUTINE rcf_ideal_baroclinic
END MODULE rcf_ideal_baroclinic_mod

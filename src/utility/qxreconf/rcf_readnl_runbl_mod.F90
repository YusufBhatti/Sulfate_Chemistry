...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_READNL_RUNBL_MOD'

CONTAINS

SUBROUTINE rcf_readnl_runbl (nft)

USE bl_option_mod, ONLY: run_bl, print_nlist_run_bl, read_nml_run_bl
...
! Read the namelist
CALL read_nml_run_bl(nft)

IF ( PrintStatus >= PrStatus_Oper .AND. mype == 0) THEN
  CALL print_nlist_run_bl()
END IF

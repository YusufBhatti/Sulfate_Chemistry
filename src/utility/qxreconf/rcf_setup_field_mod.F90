...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_SETUP_FIELD_MOD'
...
CHARACTER (LEN=*), PARAMETER :: RoutineName = 'RCF_SETUP_FIELD'
...
DO i = 1, hdr % Len2Lookup
  ...
  IF (title == 'Input grid' .OR.                                           &
      title == 'Auxillary File' .OR.                                       &
      title == 'Output grid') THEN
  ...
  CASE (ppx_atm_tall, ppx_atm_tsea )      ! Theta points
  ...
  CASE DEFAULT
    ...
    IF (section /= stashcode_prog_sec                                      &
        .AND. section /= stashcode_tracer_sec                              &
        .AND. section /= stashcode_ukca_sec                                &
        .AND. .NOT. var_recon                                              &
        .AND. select_output_fields == tstmsk_to_decide) THEN
    ...
    ELSE
      ...
      ! unknown in section 0 or free tracer or ukca sections or
      ! especially for VAR and when interpolating input fields only, we need
      ! to know about it.

      WRITE(umMessage,*) 'Unsupported Grid-type ',                         &
          field( field_count ) % stashmaster % grid_type
      CALL umPrint(umMessage,src='rcf_setup_field_mod')
...
ELSE
  ...
  IF (mype == 0 .AND. PrintStatus >= PrStatus_Normal ) THEN
    CALL umPrint('',src='rcf_setup_field_mod')
    CALL umPrint('',src='rcf_setup_field_mod')
    CALL umPrint(title,src='rcf_setup_field_mod')
    CALL umPrint('',src='rcf_setup_field_mod')
    ...
    DO k = 1, field_count
      ...
      i = i + field(k) % levels
      WRITE(umMessage,'('' '',I4,I5,I8,I4,3I6,2x,A36)')                    &
...
IF ( (fld_lookup(lbcode) /= lbcode_reg_lat_lon       .AND.               &
    ...
    .NOT. var_recon .AND. select_output_fields == tstmsk_to_decide)) THEN
  IF (mype == 0 .AND. PrintStatus >= PrStatus_Diag ) THEN
    CALL umPrint(                                                       &
        "INFO: Detected incompatible field in is_valid_field.")
    WRITE(umMessage,'(A,3I7,A)') &
        "INFO: Disabling (item_code,lbcode,lbproc): (", &
...
ELSE IF ( .NOT. ASSOCIATED(stm_entry) ) THEN
  IF (mype == 0 .AND. PrintStatus >= PrStatus_Diag ) THEN
    CALL umPrint( "INFO: No STASHmaster available in is_valid_field.", &
        src='rcf_setup_field_mod')
    WRITE(umMessage,'(A,I7)') "INFO: Disabling item_code ", &
        fld_lookup(item_code)
    CALL umPrint(umMessage,src='rcf_setup_field_mod')
  END IF

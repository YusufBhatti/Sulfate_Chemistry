...
!
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_GRIB_CONTROL_MOD'
...
USE io, ONLY: setpos8, file_open, readableWords, File_Close, buffin

USE io_constants, ONLY:                                                   &
    ioNameProvided, ioOpenReadWrite, ioNoDelete, ioNameInEnv, ioOpenReadOnly
...
CALL assign_file_unit(ainitial, Hdr_Dmy % UnitNum, handler="portio")

CALL File_Open( Hdr_Dmy % UnitNum, ainitial, read_write=ioOpenReadOnly, &
                name_in_environ=ioNameProvided, error=errstat)

IF ( errstat /= 0 ) THEN
  Cmessage = 'Failed to Open GRIB file'
  ErrorStatus = 10
  CALL EReport( RoutineName, ErrorStatus, Cmessage )
ELSE
  IF ( PrintStatus >= PrStatus_Diag  ) THEN
    IF ( mype == 0 ) THEN
      CALL umPrint( "Opened Grib Data File",src='rcf_grib_control_mod')
    END IF
  END IF
END IF
...
Reading: DO                      ! Endless loop until exit supplied
  ...
  IF ( errstat /= 0 ) THEN
    WRITE(Cmessage,'(A,I2)')   'Failed trying to SetPos to ',              &
        pos_in_file
    ErrorStatus = 20
    CALL EReport( RoutineName, ErrorStatus, Cmessage )
  END IF
  ...
  IF ( rerrstat > 0 ) THEN
    Cmessage   = 'Failed trying to read short block from GRIB file'
    ErrorStatus = 30
    CALL EReport( RoutineName, ErrorStatus, Cmessage )
  END IF
  ...
  IF (i == 0) THEN               ! Index didn't find GRIB in the buffer
    ...
    IF (PrintStatus >= PrStatus_Diag) THEN
      IF ( mype == 0 ) THEN
        WRITE(umMessage,*) "Buffer block read contained no 'GRIB' indicator"
        CALL umPrint(umMessage,src='rcf_grib_control_mod')
      END IF
    END IF
    ...
    IF (len_max_c > readable_io_len) THEN
      IF (PrintStatus >= PrStatus_Diag) THEN
        IF ( mype == 0 ) THEN
          WRITE(umMessage,'(2(A,I3),A)') 'len_max_c =', len_max_c,         &
...
DO i = 1, grib_max_fields
  ...
  IF (ASSOCIATED(Lists(i) % Begin) .AND.                                   &
    ...
    CASE (grib_U_field,                                                    &
      ...
      WRITE(cMessage,cFormat) "Re-ordering ",                              &
  ...
  ELSE
    WRITE(cMessage,'(A,I2,A,I2,A)') "List ", i ,                           &
  ...
  IF (PrintStatus >= PrStatus_Diag) THEN
    IF ( mype == 0 ) THEN
      CALL umPrint( cMessage,src='rcf_grib_control_mod')
    END IF
  END IF
...
IF ( PrintStatus >= PrStatus_Diag  ) THEN
  IF ( mype == 0 ) THEN
    WRITE(umMessage,*) "Opening Intermediary Dump"
    CALL umPrint(umMessage,src='rcf_grib_control_mod')
  END IF
END IF
...
CALL assign_file_unit( DumpName, Hdr_Itm % UnitNum, handler="portio")

CALL File_Open( Hdr_Itm % UnitNum, DumpName, &
                read_write=ioOpenReadWrite, error=errstat)

IF ( errstat /= 0 ) THEN
  Cmessage    = 'Failed to Open Intermediary Dump file'
  ErrorStatus = 99
  CALL EReport( RoutineName, ErrorStatus, Cmessage )
END IF
...
DO i = 1, grib_max_fields
  ...
  IF (ASSOCIATED(Lists(i) % Begin) ) THEN
    ...
    DO WHILE (ASSOCIATED(Current))
      ...
      ELSE
        Cmessage = 'Failed to write to temp dump :Unknown Data Type'
        ErrorStatus = 99
        CALL EReport( RoutineName, ErrorStatus, Cmessage )
      END IF
...
CALL File_Close ( Hdr_Dmy % UnitNum, ainitial,                       &
                  delete=ioNoDelete, error=errstat)
    
CALL release_file_unit( Hdr_Dmy % UnitNum, handler="portio")

CALL File_Close ( Hdr_Itm % UnitNum, DumpName, &
                  delete=ioNoDelete, error=errstat)

CALL release_file_unit( Hdr_Itm % UnitNum, handler="portio")

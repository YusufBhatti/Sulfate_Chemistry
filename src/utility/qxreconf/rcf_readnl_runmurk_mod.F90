...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_READNL_RUNMURK_MOD'
...
USE murk_inputs_mod, ONLY:                                               &
  RUN_Murk, l_murk, l_murk_source, print_nlist_run_murk,                 &
  read_nml_run_murk
...
! Read the namelist
CALL read_nml_run_murk(nft)

IF ( PrintStatus >= PrStatus_Oper .AND. mype == 0) THEN
  CALL print_nlist_run_murk()
END IF

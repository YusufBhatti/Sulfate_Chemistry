...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RCF_GRIB_SPCL_CTL_MOD'
...
CASE (stashcode_orog)
  IF ( mype == 0 ) THEN
    CALL umPrint( "Special treatment for orography", &
        src='rcf_grib_spcl_ctl_mod')
  END IF
  CALL Rcf_Grib_Spcl_Orog(Current,FieldData)

CASE (stashcode_lsm)
  IF ( mype == 0 ) THEN
    CALL umPrint( "Special treatment for landsea mask", &
        src='rcf_grib_spcl_ctl_mod')
  END IF
...
CASE (stashcode_exner)
  IF ( mype == 0 ) THEN
    CALL umPrint( "Special treatment for exner pressure", &
        src='rcf_grib_spcl_ctl_mod')
  END IF
...
CASE (stashcode_pstar)
  IF (Current % Block_1(p_Param_ID) == EID_Log_Surf_Press) THEN
    IF ( mype == 0 ) THEN
      CALL umPrint( "Special treatment for log surface pressure", &
          src='rcf_grib_spcl_ctl_mod')
    END IF
...
CASE (stashcode_soil_moist)
  IF ( mype == 0 ) THEN
    CALL umPrint( "Special treatment for soil moisture", &
        src='rcf_grib_spcl_ctl_mod')
  END IF
...
CASE (stashcode_mean_snow)
  IF ( mype == 0 ) THEN
    CALL umPrint( "Special treatment for snow amount", &
        src='rcf_grib_spcl_ctl_mod')
  END IF
...
IF ( Current % Block_2(p_LatGrdPnt1) >                                     &
  ...
  IF ( PrintStatus >= PrStatus_Diag   ) THEN
    IF ( mype == 0 ) THEN
      WRITE(umMessage,'(2A)')"About to reverse latitudes of ",Current % Desc
      CALL umPrint(umMessage,src='rcf_grib_spcl_ctl_mod')
    END IF
  END IF
  ...
  CASE (Grb_Data_Int)
    ...
    Cmessage = 'Tried to Flip Integer Data Type without any code'
    ErrorStatus = 10
...
CASE (stashcode_lsm)
  ...
  IF ( mype == 0 ) THEN
    CALL umPrint( "Store Land/Sea Mask for landpacking", &
        src='rcf_grib_spcl_ctl_mod')
  END IF
...
CASE (stashcode_soil_temp,                                                 &
  ...
  IF (.NOT. l_gotlsm) THEN
    Cmessage = 'LSM not available for land packing'
    ErrorStatus = 10
    CALL Ereport( RoutineName, ErrorStatus, Cmessage )
  END IF

  IF ( mype == 0 ) THEN
    WRITE(umMessage,'(A,I5)') "Performing land-packing for stashcode ",    &
        Current % Stashcode
    CALL umPrint(umMessage,src='rcf_grib_spcl_ctl_mod')
  END IF

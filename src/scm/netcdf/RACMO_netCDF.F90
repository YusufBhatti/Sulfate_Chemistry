...
USE planet_constants_mod,  ONLY: r, kappa, p_zero, recip_kappa, g
...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='RACMO_NETCDF'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ALLOC_RACMO_NC'
...
IF (STATUS /= nf90_noerr) THEN
  ...
  CALL umPrint(umMessage,src='RACMO_netCDF')
END IF
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEALLOC_RACMO_NC'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='READ_RACMO_NC'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='RACMO_NC_VAR_TO_SCM'
...
file_qadv(:,:) =  file_qadv(:,:)*rsec_per_day
file_tadv(:,:) =  file_tadv(:,:)*rsec_per_day
file_uadv(:,:) =  file_uadv(:,:)*rsec_per_day
file_vadv(:,:) =  file_vadv(:,:)*rsec_per_day
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='RACMO_NC_GRID_TO_SCM'

IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)

ALLOCATE( exner_th(rw_lng,rw,model_levels)   &
  ...
  DO j=1, rw
    DO i=1, rw_lng
      ...
      CALL interp1d(file_z_full_prf, file_w(kk,:),                 &
                    z_th,                                          &
                    w_bg_nc(i,j,kk-obs_t0+1,1:model_levels))
...
DO kk=obs_t0, nt_nc-1
  w_inc_nc(:,:,kk-obs_t0+1,:) = (rsec_per_day/obs_pd_nc)     &
                              * ( w_bg_nc(:,:,kk-obs_t0+2,:) &
                                - w_bg_nc(:,:,kk-obs_t0+1,:))
END DO
...
l_w_inc  = .TRUE.
WRITE(umMessage,*) &
    ' No LS W-tendency in driver file, Forcing calculated from w-field.'
CALL umPrint(umMessage,src='RACMO_netCDF')

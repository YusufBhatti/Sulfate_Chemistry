...
, mx_mod_lv, mx_st_lv, mx_tr_lv, mx_tr_vars

USE scm_utils, ONLY:                                      &
  rmdi, imdi, rw_lng, rw, nlnd, ntile, nml_nmod_lv        &
, st_lv, ntr_lv, ntr_var                                  &
...
, qi     (mx_rw_lng*mx_rw*mx_mod_lv)      = rmdi &! Spec. humid (kg/kg)
...
, ccai_dp(mx_rw_lng*mx_rw)                = rmdi &
, ccai_md(mx_rw_lng*mx_rw)                = rmdi &
, ccai_sh(mx_rw_lng*mx_rw)                = rmdi
...
, i_rgrainl, ccai_dp, ccai_md, ccai_sh
...
!=============================================================================
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='S_INPROF'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='READ_INPROF'
...
IF (istatus /= 0) THEN
  ...
  CALL umPrint(umMessage,src='s_inprof')
...
IF (nml_nmod_lv /= imdi) THEN
  ...
  !=====================================================
  IF (nml_nmod_lv /= model_levels) THEN
    interpolate = .TRUE.
  ELSE
    ...
    ELSE
      ...
      IF (z_rh(model_levels+1) /= nml_z_th(model_levels+1)) THEN
        interpolate = .TRUE.
      END IF
    END IF ! kill_interp
  END IF
...
IF (ccai_dp(1) /= rmdi) scm_ccai_dp  = RESHAPE( ccai_dp,  (/rw_lng,rw/) )
IF (ccai_md(1) /= rmdi) scm_ccai_md  = RESHAPE( ccai_md,  (/rw_lng,rw/) )
IF (ccai_sh(1) /= rmdi) scm_ccai_sh  = RESHAPE( ccai_sh,  (/rw_lng,rw/) )
...
ELSE
  ...
  IF (theta(1)  /= rmdi)                                                  &
    scm_theta  = RESHAPE( theta,  (/rw_lng,rw,model_levels/) )

  IF (ui(1)     /= rmdi)                                                  &
    scm_ui     = RESHAPE( ui,     (/rw_lng,rw,model_levels/) )

  IF (vi(1)     /= rmdi)                                                  &
    scm_vi     = RESHAPE( vi,     (/rw_lng,rw,model_levels/) )

  IF (qi(1)     /= rmdi)                                                  &
    scm_qi     = RESHAPE( qi,     (/rw_lng,rw,model_levels/) )

  IF (wi(1)     /= rmdi)                                                  &
    scm_wi     = RESHAPE( wi,     (/rw_lng,rw,model_levels+1/) )

  IF (w_advi(1) /= rmdi)                                                  &
    scm_w_advi = RESHAPE( w_advi, (/rw_lng,rw,model_levels+1/) )

  IF (p_in(1)   /= rmdi)                                                  &
    scm_p_in   = RESHAPE( p_in,   (/rw_lng,rw,model_levels+1/) )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='INTERP_INPROF'
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th,        scm_qi(i,j,1:model_levels) )
    END DO
  END DO
...
IF (ui(1) /= rmdi) THEN
  ...
  DO j=1, rw
    DO i=1, rw_lng
      ...
      CALL interp1d                                                       &
        ( rdum1d,          rdum1db                                        &
        , z_rh(1:model_levels), scm_ui(i,j,1:model_levels) )
    END DO
  END DO
...
IF (vi(1) /= rmdi) THEN
  ...
  DO j=1, rw
    DO i=1, rw_lng
      ...
      CALL interp1d                                                       &
        ( rdum1d,          rdum1db                                        &
        , z_rh(1:model_levels), scm_vi(i,j,1:model_levels) )
    END DO
  END DO
...
IF ( wi(1) /= rmdi) THEN
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ( rdum1d, nml_wi(i,j,:)                                           &
        , z_th,   scm_wi(i,j,1:model_levels))
    END DO
  END DO
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ALLOC_INPROF'
...
ALLOCATE                                         &
  ...
  , exner_rh     (rw_lng,rw,    model_levels+1)  &
  , exner_th     (rw_lng,rw,    model_levels)    &
  , ti_rh        (rw_lng,rw,    model_levels+1)  &
  , ti_th        (rw_lng,rw,    model_levels) )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEALLOC_INPROF'

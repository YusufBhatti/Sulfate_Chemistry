...
USE jules_snow_mod, ONLY: nsmax, r0, rho_snow_const


USE s_maxdim,  ONLY:                                                        &
  mx_mod_lv, mx_st_lv, mx_sm_lv, mx_tr_lv, mx_tr_vars                       &
, dim_cs1, dim_cs2, mx_nobs

USE scm_utils, ONLY:                                                        &
  rmdi, imdi, rw_lng, rw, nobs, nbl_lv, nlnd, ntile                         &
, o3_lv, st_lv, sm_lv, npft, ntr_lv, ntr_var, ntype
...
, l_flux_bc       &! Now redundent - use flux_bc_opt switch in run_bl
...
, tsurf_elev_surft (:,:)   &! Temperature of elevated subsurface tiles (K)
...
, ustar_forcing(:,:,:)   ! ... friction velocity (m/s)
...
, ccai_dp(:,:)          &! Initial deep conv cloud amount (decimal frac.)
, ccai_md(:,:)          &! Initial mid-level conv cloud amount (decimal frac.)
, ccai_sh(:,:)          &! Initial shallow conv cloud amount (decimal frac.)
...
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='S_MAIN_FORCE'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='SET_FORCING_DEFAULTS'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ALLOC_FORCING'
...
ALLOCATE                                &
  ...
  , so2        (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , so4_aitken (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , so4_accu   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , so4_diss   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , nh3        (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , dms        (rw_lng,rw,model_levels) &
  , aerosol    (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , aerosol_em (rw_lng,rw,model_levels) &
  , bmass_new  (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , bmass_aged (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , bmass_cld  (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , soot_new   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , soot_aged  (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , soot_cld   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , ocff_new   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , ocff_aged  (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , ocff_cld   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , nitr_acc   (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , nitr_diss  (rw_lng,rw,tdims%k_start:tdims%k_end) &
  , co2        (rw_lng,rw,tdims%k_start:tdims%k_end) )
...
ALLOCATE                                       &
  ...
  , ice_fract     (rw_lng,rw)                  &
  , qi            (rw_lng,rw,model_levels)     &
  , theta         (rw_lng,rw,model_levels)     &
  , ui            (rw_lng,rw,model_levels)     &
  , vi            (rw_lng,rw,model_levels)     &
  , wi            (rw_lng,rw,0:model_levels)   &
  , p_in          (rw_lng,rw,model_levels+1)   &
  , w_advi        (rw_lng,rw,0:model_levels)   &
  , free_tracers  (rw_lng,rw,ntr_lv,ntr_var)   &
  , bl_w_var_i    (rw_lng,rw) )


! INOBSFOR
ALLOCATE                                         &
  ( q_star        (rw_lng,rw,nobs,model_levels)  &
  , q_bg          (rw_lng,rw,nobs,model_levels)  &
  , t_inc         (rw_lng,rw,nobs,model_levels)  &
  , u_inc         (rw_lng,rw,nobs,model_levels)  &
  , v_inc         (rw_lng,rw,nobs,model_levels)  &
  , t_bg          (rw_lng,rw,nobs,model_levels)  &
  , u_bg          (rw_lng,rw,nobs,model_levels)  &
  , v_bg          (rw_lng,rw,nobs,model_levels)  &
  , w_inc         (rw_lng,rw,nobs,0:model_levels)&
  , w_bg          (rw_lng,rw,nobs,0:model_levels)&
  , flux_h        (rw_lng,rw,nobs)               &
  , flux_e        (rw_lng,rw,nobs)               &
  , tstar_forcing (rw_lng,rw,nobs)               &
  , ustar_forcing (rw_lng,rw,nobs) )
...
! INGEOFOR
ALLOCATE                                       &
  ( ug (rw_lng,rw,nobs,model_levels)           &
  , vg (rw_lng,rw,nobs,model_levels) )
...
ALLOCATE                                       &
  ...
  , qcl_rad         (rw_lng,rw,model_levels)   &
  , qcf_rad         (rw_lng,rw,model_levels)   &
  , layer_cloud_rad (rw_lng,rw,model_levels) )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEALLOC_FORCING'
...
DEALLOCATE                                                                &
  ...
  , i_ds,              i_rgrainl,        ccai_sh,         ccai_md         &
  , ccai_dp )

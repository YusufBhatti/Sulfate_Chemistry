...
, mx_mod_lv, mx_st_lv, mx_tr_lv, mx_tr_vars, mx_nobs

USE scm_utils,  ONLY:                                               &
  rmdi, imdi, rw_lng, rw, nml_nmod_lv, nobs                         &
...
, scm_ustar_forcing => ustar_forcing            &
, scm_q_star        => q_star                   &
, scm_t_inc         => t_inc                    &
, scm_u_inc         => u_inc                    &
, scm_v_inc         => v_inc                    &
...
, ustar_forcing (mx_rw_lng*mx_rw*mx_nobs)               = rmdi &
...
, q_bg          (mx_rw_lng*mx_rw*mx_nobs*mx_mod_lv)     = rmdi &
...
, q_star        (mx_rw_lng*mx_rw*mx_nobs*mx_mod_lv)     = rmdi &
...
, flux_h, flux_e, tstar_forcing, ustar_forcing
...
!=============================================================================
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='S_INOBSFOR'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='READ_INOBSFOR'
...
IF (istatus /= 0) THEN
  ...
  CALL umPrint(umMessage,src='s_inobsfor')
...
IF (ustar_forcing(1) /= rmdi)                                             &
  scm_ustar_forcing = RESHAPE( ustar_forcing ,(/rw_lng,rw,nobs/))
...
ELSE
  ...
  IF (nobs /= imdi) THEN
    ...
    IF (t_inc(1)  /= rmdi)                                                &
      scm_t_inc  = RESHAPE( t_inc  ,(/rw_lng,rw,nobs,model_levels/) )

    IF (q_star(1) /= rmdi)                                                &
      scm_q_star = RESHAPE( q_star ,(/rw_lng,rw,nobs,model_levels/) )

    IF (u_inc(1)  /= rmdi)                                                &
      scm_u_inc  = RESHAPE( u_inc  ,(/rw_lng,rw,nobs,model_levels/) )

    IF (v_inc(1)  /= rmdi)                                                &
      scm_v_inc  = RESHAPE( v_inc  ,(/rw_lng,rw,nobs,model_levels/) )

    IF (w_inc(1)  /= rmdi)                                                &
      scm_w_inc  = RESHAPE( w_inc  ,(/rw_lng,rw,nobs,model_levels+1/) )




    IF (t_bg(1)   /= rmdi)                                                &
      scm_t_bg   = RESHAPE( t_bg   ,(/rw_lng,rw,nobs,model_levels/) )

    IF (q_bg(1)   /= rmdi)                                                &
      scm_q_bg   = RESHAPE( q_bg   ,(/rw_lng,rw,nobs,model_levels/) )

    IF (u_bg(1)   /= rmdi)                                                &
      scm_u_bg   = RESHAPE( u_bg   ,(/rw_lng,rw,nobs,model_levels/) )

    IF (v_bg(1)   /= rmdi)                                                &
      scm_v_bg   = RESHAPE( v_bg   ,(/rw_lng,rw,nobs,model_levels/) )

    IF (w_bg(1)   /= rmdi)                                                &
      scm_w_bg   = RESHAPE( w_bg   ,(/rw_lng,rw,nobs,model_levels+1/) )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='INTERP_INOBSFOR'
  ...
  DO kk=1, nobs
    DO j=1, rw
      DO i=1, rw_lng
        ...
        CALL interp1d                                                     &
          ( rdum1d,          rdum1db                                      &
          , z_rh(1:model_levels), scm_u_inc(i,j,kk,1:model_levels) )
      END DO
    END DO
  END DO
  ...
  DO kk=1, nobs
    DO j=1, rw
      DO i=1, rw_lng
        ...
        CALL interp1d                                                     &
          ( rdum1d,          rdum1db                                      &
          , z_rh(1:model_levels), scm_v_inc(i,j,kk,1:model_levels) )
      END DO
    END DO
  END DO
    ...
    DO j=1, rw
      DO i=1, rw_lng
        CALL interp1d                                                     &
          ...
          , z_th,   scm_w_inc(i,j,kk,1:model_levels) )
      END DO
    END DO
  END DO
  ...
  DO kk=1, nobs
    DO j=1, rw
      DO i=1, rw_lng
        ...
        CALL interp1d                                                     &
          ( rdum1d,          rdum1db                                      &
          , z_rh(1:model_levels), scm_u_bg(i,j,kk,1:model_levels) )
      END DO
    END DO
  END DO
  ...
  DO kk=1, nobs
    DO j=1, rw
      DO i=1, rw_lng
        ...
        CALL interp1d                                                     &
          ( rdum1d,          rdum1db                                      &
          , z_rh(1:model_levels), scm_v_bg(i,j,kk,1:model_levels) )
      END DO
    END DO
  END DO
    ...
    DO j=1, rw
      DO i=1, rw_lng
        CALL interp1d                                                     &
          ...
          , z_th,   scm_w_bg(i,j,kk,1:model_levels) )
      END DO
    END DO
  END DO
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ALLOC_INOBSFOR'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEALLOC_INOBSFOR'

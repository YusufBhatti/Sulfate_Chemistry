...
, mx_nlnd, mx_mod_lv, mx_o3_lv, mx_ntile

USE scm_utils, ONLY:                                                        &
  rmdi, imdi, rw_lng, rw, nlnd, nml_nmod_lv, ntr_lv                         &
...
REAL, PRIVATE ::                                 &
  so2         (mx_rw_lng*mx_rw*mx_mod_lv) = rmdi &
, so4_aitken  (mx_rw_lng*mx_rw*mx_mod_lv) = rmdi &
, so4_accu    (mx_rw_lng*mx_rw*mx_mod_lv) = rmdi &
, so4_diss    (mx_rw_lng*mx_rw*mx_mod_lv) = rmdi &
...
!=============================================================================
CHARACTER(LEN=*), PARAMETER, PRIVATE :: ModuleName='S_RUNDATA'
...
INTEGER :: istatus
INTEGER :: icode
CHARACTER(LEN=errormessagelength) :: cmessage, iomessage
CHARACTER(LEN=*), PARAMETER :: RoutineName='READ_RUNDATA'
...
IF (istatus /= 0) THEN
  ...
  CALL umPrint(umMessage,src='s_rundata')
...
! Prevent scaling albedo on tiles to agree with obs when no obs provided
!==========================================================================
IF ( l_albedo_obs .AND. nlnd > 0 ) THEN

  IF (l_spec_albedo) THEN

    IF ( ANY(albobs_vis(1:nlnd)== rmdi) .OR.                                  & 
         ANY(albobs_nir(1:nlnd)== rmdi) ) THEN
    ! Check if obs were read in from namelist

      icode=501
      cmessage =                                                     newline//&
        'Attempting to scale albedo on tiles to obs but'//           newline//&
        'no albedo observations given in namelist!'
      CALL ereport (routinename, icode, cmessage)
    END IF 

  ELSE 

    IF ( ANY(albobs_sw(1:nlnd)== rmdi) )  THEN
    ! Check if obs were read in from namelist
...
ELSE
  ...
  IF (co2(1)   /= rmdi)                                                   &
    scm_co2    = RESHAPE( co2,   (/rw_lng,rw,model_levels/) )

  IF (so2(1)   /= rmdi)                                                   &
    scm_so2    = RESHAPE( so2,   (/rw_lng,rw,model_levels/) )

  IF (dms(1)   /= rmdi)                                                   &
    scm_dms    = RESHAPE( dms,   (/rw_lng,rw,model_levels/) )

  IF (nh3(1)   /= rmdi)                                                   &
    scm_nh3    = RESHAPE( nh3,   (/rw_lng,rw,model_levels/) )

  IF (so4_aitken(1) /= rmdi)                                              &
    scm_so4_aitken = RESHAPE( so4_aitken, (/rw_lng,rw,model_levels/) )

  IF (so4_accu(1)   /= rmdi)                                              &
    scm_so4_accu   = RESHAPE( so4_accu,   (/rw_lng,rw,model_levels/) )

  IF (so4_diss(1)   /= rmdi)                                              &
    scm_so4_diss   = RESHAPE( so4_diss,   (/rw_lng,rw,model_levels/) )

  IF (aerosol(1)    /= rmdi)                                              &
    scm_aerosol    = RESHAPE( aerosol,    (/rw_lng,rw,model_levels/) )

  IF (aerosol_em(1) /= rmdi)                                              &
    scm_aerosol_em = RESHAPE( aerosol_em, (/rw_lng,rw,model_levels/) )

  IF (free_tracers(1) /= rmdi)                                            &
    scm_free_tracers = RESHAPE(free_tracers,(/rw_lng,rw,ntr_lv,ntr_var/) )

  IF (soot_new(1)   /= rmdi)                                              &
    scm_soot_new   = RESHAPE( soot_new,   (/rw_lng,rw,model_levels/) )

  IF (soot_aged(1)  /= rmdi)                                              &
    scm_soot_aged  = RESHAPE( soot_aged,  (/rw_lng,rw,model_levels/) )

  IF (soot_cld(1)   /= rmdi)                                              &
    scm_soot_cld   = RESHAPE( soot_cld,   (/rw_lng,rw,model_levels/) )

  IF (bmass_new(1)  /= rmdi)                                              &
    scm_bmass_new  = RESHAPE( bmass_new,  (/rw_lng,rw,model_levels/) )

  IF (bmass_aged(1) /= rmdi)                                              &
    scm_bmass_aged = RESHAPE( bmass_aged, (/rw_lng,rw,model_levels/) )

  IF (bmass_cld(1)  /= rmdi)                                              &
    scm_bmass_cld  = RESHAPE( bmass_cld,  (/rw_lng,rw,model_levels/) )

  IF (ocff_new(1)   /= rmdi)                                              &
    scm_ocff_new   = RESHAPE( ocff_new,   (/rw_lng,rw,model_levels/) )

  IF (ocff_aged(1)  /= rmdi)                                              &
    scm_ocff_aged  = RESHAPE( ocff_aged,  (/rw_lng,rw,model_levels/) )

  IF (ocff_cld(1)   /= rmdi)                                              &
    scm_ocff_cld   = RESHAPE( ocff_cld,   (/rw_lng,rw,model_levels/) )

  IF (nitr_acc(1)   /= rmdi)                                              &
    scm_nitr_acc   = RESHAPE( nitr_acc,   (/rw_lng,rw,model_levels/) )

  IF (nitr_diss(1)  /= rmdi)                                              &
    scm_nitr_diss  = RESHAPE( nitr_diss,  (/rw_lng,rw,model_levels/) )
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='INTERP_RUNDATA'
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_co2(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_so2(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_dms(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_nh3(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_so4_aitken(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_so4_accu(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_so4_diss(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_aerosol(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_aerosol_em(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_soot_new(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_soot_aged(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_soot_cld(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_bmass_new(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_bmass_aged(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_bmass_cld(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_ocff_new(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_ocff_aged(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_ocff_cld(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_nitr_acc(i,j,1:model_levels) )
    END DO
  END DO
  ...
  DO j=1, rw
    DO i=1, rw_lng
      CALL interp1d                                                       &
        ...
        , z_th(1:model_levels), scm_nitr_diss(i,j,1:model_levels) )
    END DO
  END DO
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='ALLOC_RUNDATA'
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='DEALLOC_RUNDATA'

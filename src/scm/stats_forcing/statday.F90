...
SUBROUTINE statday                                                            &
  ( row_length, rows, ntrop, atime, btime, dayno, deltan                      &
...
, at(row_length,rows,model_levels-1)   &! Out Variable a in eqn 2.22
, ad(row_length,rows,model_levels-1)   &!     used to calculate mean of
                                        !     random variable for
                                        !     temp. and dew point depression
, atime, btime                         &! In  Constants for calculating
                                        !     annual cycle in eqn. 2.33
, avn(row_length,rows,model_levels-1)  &! Out Variable a in eqn 2.22
...
REAL ::                                &
  cdbar(row_length,rows,model_levels)  &! Out Mean and SD of random variable
, cdsd(row_length,rows,model_levels)   &!     for dew pt. depression (eqns
                                        !     2.22 and 2.23)
, ctbar(row_length,rows,model_levels)  &! Out Mean and SD of random
, ctsd(row_length,rows,model_levels)   &!     variable for temp. (eqns 2.22
                                        !     and 2.23)
, cvnbar(row_length,rows,model_levels) &! Out Mean and SD of random variable
, cvnsd(row_length,rows,model_levels)  &!     for velocity VN (eqns 2.22
                                        !     and 2.23)
, cwbar(row_length,rows,ntrop)         &! Out Mean and SD of random variable
, cwsd(row_length,rows,ntrop)          &!     for vertical velocity (eqns 2.22
                                        !     and 2.23)
, dbar(row_length,rows,model_levels)   &! Out Mean and SD dewpoint
, dsd(row_length,rows,model_levels)    &!     depression at daynumber relative
                                        !     to winter solstice (K)
, dbara(row_length,rows,model_levels)  &! In  Amplitude and mean of seasonal
, dbarb(row_length,rows,model_levels)   !     variation of mean dew pt.
                                        !     depression (K)

REAL ::                                 &
  ddash(row_length,rows,model_levels)   &! Out Dew pt. corrections (K)
, deltan(row_length,rows)               &! In  Radius of area (m)
, dgrada(row_length,rows,model_levels)  &! In  Amplitude and mean of seasonal
, dgradb(row_length,rows,model_levels)  &!     variation of dew pt. depression
                                         !     gradient (K/km)
, p(row_length,rows,model_levels+1)     &! Out Pressure on rho levels (Pa)
...
, tbar(row_length,rows,model_levels)    &! Out Mean and SD temperature at
, tsd(row_length,rows,model_levels)     &!     daycount days from winter
                                         !     solstice (K)
, tbara(row_length,rows,model_levels)   &! In  Amplitude and mean of
, tbarb(row_length,rows,model_levels)   &!     seasonal variation of temp. (K)
, tgrada(row_length,rows,model_levels)  &! In  Amplitude and mean of seasonal
, tgradb(row_length,rows,model_levels)  &!     variation of temp. gradient
                                         !     (K/km)
, tsda(row_length,rows,model_levels)    &! In  Amplitude and mean of seasonal
, tsdb(row_length,rows,model_levels)    &!     variation of SD of temp. (K)
, vnbar(row_length,rows,model_levels)   &! Out Mean and SD velocity VN at
, vnsd(row_length,rows,model_levels)    &!     daycount days from
                                         !     winter solstice (m/s)
, vpbar(row_length,rows,model_levels)   &! Out Mean  velocity VP at
                                         !     daycount days from
                                         !     winter solstice (m/s)
, vnbara(row_length,rows,model_levels)  &! In  Amplitude and mean of seasonal
, vnbarb(row_length,rows,model_levels)  &!     variation of velocity VN
                                         !     (m/s)
, vnsda(row_length,rows,model_levels)   &! In  Amplitude and mean of seasonal
, vnsdb(row_length,rows,model_levels)   &!     variation of SD of velocity VN
                                         !     (m/s)
, vpbara(row_length,rows,model_levels)  &! In  Amplitude and mean of seasonal
, vpbarb(row_length,rows,model_levels)  &!     variation of velocity VP
                                         !     (m/s)
...
! DEPENDS ON: xnew
CALL xnew (tbar, tbara, tbarb, row_length, rows, model_levels, xt)

! DEPENDS ON: xnew
CALL xnew (tsd, tsda, tsdb, row_length, rows, model_levels, xt)

! DEPENDS ON: xnew
CALL xnew (dbar, dbara, dbarb, row_length, rows, model_levels, xt)

! DEPENDS ON: xnew
CALL xnew (vnbar, vnbara, vnbarb, row_length, rows, model_levels, xt)

! DEPENDS ON: xnew
CALL xnew (vnsd, vnsda, vnsdb, row_length, rows, model_levels, xt)

! DEPENDS ON: xnew
CALL xnew (vpbar, vpbara, vpbarb, row_length, rows, model_levels, xt)
...
! DEPENDS ON: acinit
CALL acinit (tbar, tsd, at, ctbar, ctsd, cort, model_levels, row_length, rows)

! DEPENDS ON: acinit
CALL acinit (dbar, dsd, ad, cdbar, cdsd, cord, model_levels, row_length, rows)

! DEPENDS ON: acinit
CALL acinit (vnbar, vnsd, avn, cvnbar, cvnsd, corvn, model_levels, row_length, &
             rows)

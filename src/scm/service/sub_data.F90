...
USE rad_input_mod, ONLY: a_sw_radstep_diag
USE s_main_force,  ONLY: ntrad1, conv_mode
USE scm_utils,     ONLY: time_string, daycount
...
, nlevsrows, nlevscount  &! model_levels, nsoil elements
, nwetrows, nwetcount    &
...
IF ( conv_mode == 1      .AND.                                     &
     istep     >= ntrad1 .AND.                                     &
     MOD(istep-ntrad1, a_sw_radstep_diag) == 0) THEN


  DO k=1, model_levels
    DO j=1, rows
      DO i=1, row_length
        t(i,j,k) = theta(i,j,k)*exner_th(i,j,k)
      END DO
    END DO
  END DO
  ...
  !   Write out headings
  WRITE (22,c6fmt)
  WRITE (22,c4fmt) daycount, istep, title1
  WRITE (22,c3fmt) ayear, aday, time_string
  WRITE (22,c5fmt)
  WRITE (22,c7fmt)
  WRITE (22,c5fmt)
    ...
    DO k=1, row_length
      ...
      DO nlevscount=1, nlevsrows
        ...
        ELSE
          ...
          WRITE (22,c1fmt) (exner_rh(k,l, i + element), i = 1, lastrow)
        END IF
      END DO
      WRITE (22,c5fmt)
      WRITE (22,c10fmt)

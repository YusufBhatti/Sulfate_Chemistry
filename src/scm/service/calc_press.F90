...
exner_theta_levels(:,:,0) = 0.0 ! Otherwise uninitialised
...
CALL calc_exner_at_theta                                                    &
  ...
  , model_levels, 0, 0, 0, 0, exner_theta_levels, l_include_halos)

! calculate p at theta_levels
! DEPENDS ON: calc_p_from_exner
CALL calc_p_from_exner                                                      &
  ( p_theta_levels, row_length, rows, tdims%k_len, 0, 0                    &
...
IF (l_calc_rho) THEN
  ...
  CALL calc_rho                                                             &
    ...
    , exner_rho_levels, exner_theta_levels(:,:,1:)                          &
    , theta, p, rho )
END IF

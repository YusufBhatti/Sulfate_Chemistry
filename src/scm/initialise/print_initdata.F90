...
SUBROUTINE print_initdata                                                     &
  ( row_length,rows, land_pts, nozone, nfor, nbl_levs                         &
...
, ccwpin_rad, layer_cloud_rad, qcl_rad, ozone, ug_opt, vg_opt, ustar_forcing
...
REAL ::                                  &
  tls(row_length,rows,nfor,model_levels) &! Temperature
, qls(row_length,rows,nfor,model_levels) &! Specific humidity
, uls(row_length,rows,nfor,model_levels) &! Zonal wind
, vls(row_length,rows,nfor,model_levels) &! Meridional wind
, wls(row_length,rows,nfor,0:model_levels)  ! Vertical velocity


REAL ::                                  &
  ti(row_length,rows,model_levels)        ! Initial temp. profile  (K)
...
, multnl,resnl             &! Counters for printing model_levels variables
...
CHARACTER (LEN=100) :: c101fmt, c102fmt, c103fmt, c104fmt
...
CHARACTER(LEN=*), PARAMETER :: RoutineName='PRINT_INITDATA'
...
c104fmt = '('                                                     &
 //'" * Surface friction velocity                 ",f27.1," *")'
...
DO l=1, rows          ! loop on points
  DO k=1, row_length
    ...
    IF (nout(numnout)  /=  0) THEN
      ...
      WRITE (nout(numnout),c15fmt) model_levels
      WRITE (nout(numnout),c16fmt) model_levels
      ...
      IF (obs) THEN
        ...
        ! ustar
        WRITE (nout(numnout),c104fmt)
        IF (multnf  /=  0) THEN
          DO m=1, multnf
            WRITE (nout(numnout),'(5f10.5)')                      &
              (ustar_forcing(k,l,i),i=(m-1)*5+1,5*m)
          END DO
          WRITE (nout(numnout),'(5f10.5)')                        &
            (ustar_forcing(k,l,i), i = (m-1)*5+1, (m-1)*5+resnf)
        ELSE
          WRITE (nout(numnout),'(5f10.5)')                        &
           (ustar_forcing(k,l,i),i=1,resnf)
        END IF               ! multnf
      ...
      IF (noforce .OR.  obs .OR. altdat .OR. geoforce) THEN
        multnl = INT((model_levels+1)/10)
        resnl = MOD((model_levels+1),10)
        IF (noforce .OR. obs                                      &
        ...
        multnl = INT(model_levels/10)
        resnl = MOD(model_levels,10)
        ...
        multnl = INT(model_levels/5)
        resnl = MOD(model_levels,5)
        IF (multnl  /=  0) THEN
          DO j=1, multnl
            WRITE (nout(numnout),'("  ",5e12.5)')                 &
      ...
      IF (radcloud_fixed) THEN
        ...
        multcl = INT(model_levels/5)
        rescl = MOD(model_levels,5)

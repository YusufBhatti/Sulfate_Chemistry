...
def tstmsk(config, modl, isec, item, h_vers, exppxi, space_code_flag):
    ...
    l_mcr_qcf2 = False # Hard codded in mphys_inputs_mod.f90
                       # But can be set for CASIM (see below)
    ...
    if l_casim:
        casim_moments_choice = _get_var(config, "namelist:run_precip",
                         "casim_moments_choice", type_="integer")
    else:
        casim_moments_choice = None

# The following CASIM-specific switches are hard coded to False initially
...
# Set CASIM switches based on the value of the casim_moments_choice switch.
# This should mirror the Fortran routine casim_set_dependent_switches.
    if l_casim:
        l_mcr_qcf2 = True # This is always true for present CASIM
                          # configuration

        # casim_moments_choice can only be set to 1 and 8 in the GUI
        # at present.
        if casim_moments_choice == 1:
            # Set switches for double moment configuration
            l_mp_cloudnumber = True
            l_mp_rainnumber  = True
            l_mp_icenumber   = True
            l_mp_snownumber  = True
            l_mp_graupnumber = True
        elif casim_moments_choice == 8:
            # Set switches for triple moment configuration
            l_mp_cloudnumber = True
            l_mp_rainnumber  = True
            l_mp_icenumber   = True
            l_mp_snownumber  = True
...
# ( end of CASIM options )
    ...
    cnv_cold_pools = _get_var(config, "namelist:run_convection",
                     "cnv_cold_pools", default=0, type_="integer")
    ...
    l_urban2t = _get_var(config, "namelist:jules_surface",
    ...
    l_glomap_mode_clim =  _get_var(config, "namelist:run_glomap_aeroclim",
                                   ...
                                   type_="logical")
    # EasyAerosol
    l_easyaerosol_sw = _get_var(config, "namelist:easyaerosol", "l_easyaerosol_sw",
                                default=False, type_="logical")

    l_easyaerosol_lw = _get_var(config, "namelist:easyaerosol", "l_easyaerosol_lw",
                                default=False, type_="logical")

    # Leonard terms
    l_leonard_term = _get_var(config, "namelist:run_diffusion", "l_leonard_term",
                            default=False, type_="logical")
    ...
    if (nsec == 0 or nsec == 31 or  nsec == 33 or
        ...
        if sum_iopn != 0:
            ...
            if (n2n1 == 99 and nsec != 33 and nsec != 36 and
                ...
                elif n8 == 2 and iscrntdiag != 2 and iscrntdiag != 3:
                    # diagnosis of screen temperature
                    l_mask = False
                    msg = _form_msg("n8", n8, "iscrntdiag != 2 and iscrntdiag != 3")
                ...
                elif n13 == 4 and cnv_cold_pools <= 0:
                    # Convective Cold Pools
                    l_mask = False
                    msg = _form_msg("n13", n13, "Convective Cold Pools")
    ...
    # UKCA
    if nsec == 34 or nsec == 38 or nsec == 51 or nsec == 50 or nsec == 52:
    ...
    if nsec == 1:
        if sum_iopn != 0:
            ...
            elif n9 == 1 and not l_easyaerosol_sw:
                l_mask = False
                msg = _form_msg("n9", n9, "not l_easyaerosol_sw")
    ...
    # Long wave radiation
        ...
        if sum_iopn != 0:
            ...
            if n8 == 1 and nice_use != nice:
                l_mask = False
                msg = _form_msg("n8", n8, "nice_use != nice")
            if n9 == 1 and not l_easyaerosol_lw:
                l_mask = False
                msg = _form_msg("n9", n9, "not l_easyaerosol_lw")
    ...
    # Boundary layer
        ...
        if sum_iopn != 0:
            ...
            if n5 == 1 and not l_leonard_term:
                l_mask = False
                msg = _form_msg("n5", n5, "not l_leonard_term")
    ...
    # Large-scale precipitation
        ...
        if sum_iopn != 0:
            ...
            elif n8 == 2 and ( ( not l_casim ) or
                               ( not l_mp_cloudnumber ) or 
                               ( not l_mp_icenumber ) ):
                l_mask = False
                msg = _form_msg("n8", n8, 
                                "not l_casim or not l_mp_cloudnumber or not l_mp_icenumber")
            elif n8 == 3 and ( ( not l_casim ) or
                               ( not l_mp_rainnumber ) or
                               ( not l_mp_icenumber ) ):
                l_mask = False
                msg = _form_msg("n8", n8, 
                                "not l_casim or not l_mp_rainnumber or not l_mp_icenumber")
            elif n8 == 4 and ( ( not l_casim ) or 
                               ( not l_mp_icenumber ) ):
                l_mask = False
                msg = _form_msg("n8", n8, "not l_casim or not l_mp_icenumber")
            elif n8 == 5 and ( ( not l_casim ) or
                               ( not l_mp_snownumber ) ):
                l_mask = False
                msg = _form_msg("n8", n8, "not l_casim or not l_mp_snownumber")
            elif n8 == 6 and ( ( not l_casim ) or 
                               ( not l_mp_graupnumber )):
                l_mask = False
                msg = _form_msg("n8", n8, "not l_casim or not l_mp_graupnumber")
    ...
    # PWS Diagnostics
        ...
        if sum_iopn != 0:
            if n1 == 1 and not l_param_conv:
                # Diagnostic not available when convection is turned off
                l_mask = False
                msg = _form_msg("n1", n1, "not l_param_conv")
...
def _check_chem(config, noption):
    ...
    # If n is 3 then it is currently unavailable.
    elif noption == 3:
        raise OptionNotFoundError("_check_chem Invalid option code {0}:"
                                  .format(noption))
...
def _check_glomapclim(config, noption):
    ...
    # If n is 3 then it is on if ACTIVATE is on
        ...
        l_mask = _get_var(config, "namelist:run_glomap_aeroclim",
                          "l_glomap_clim_arg_act",default=False,
                          type_="logical")
    
    # If n is 4 then it is on if aie1 or aie2 is on or ACTIVATE is on
    elif noption == 4:
        l_glomap_clim_arg_act= _get_var(config, "namelist:run_glomap_aeroclim",
                                         "l_glomap_clim_arg_act",
                                         default=False, type_="logical")
        l_glomap_clim_aie1   = _get_var(config, "namelist:run_glomap_aeroclim",
                                        "l_glomap_clim_aie1", default=False, 
                                        type_="logical")
        l_glomap_clim_aie2   = _get_var(config, "namelist:run_glomap_aeroclim",
                                        "l_glomap_clim_aie2", default=False, 
                                        type_="logical")
        
        l_mask = l_glomap_clim_arg_act or l_glomap_clim_aie1 or l_glomap_clim_aie2
    ...
    # i.e. which aerosol scheme is in use
        ...
        if not l_ukca_mode:
            ...
            if i_mode_setup == 1:
                ...
                nstr = 'n8'
                n = n8
            elif i_mode_setup == 10:
                nstr = 'n10'
                n = n10
                
            # currently only support values of i_mode_setup 1,2,6,8 and 10
            else:
                l_mask = False

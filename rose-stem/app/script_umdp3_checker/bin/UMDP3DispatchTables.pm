...
my %dispatch_table_file_c = (
  ...
  'File missing crown copyright statement or agreement reference' => \&UMDP3::check_crown_copyright,
  'File missing correct code owner comment' => \&UMDP3::check_code_owner,
  'Used an _OPENMP if-def without also testing against UM_USE_C_OPENMP_VIA_THREAD_UTILS. (Or _OPENMP does not come first in the test.)' => \&UMDP3::c_openmp_define_pair_thread_utils,
  'Used an _OPENMP && UM_USE_C_OPENMP_VIA_THREAD_UTILS if-def test in a logical combination with a third macro' => \&UMDP3::c_openmp_define_no_combine,
...
sub get_file_dispatch_table_fortran {
  return %dispatch_table_file_fortran;
}

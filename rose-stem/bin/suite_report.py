...
RESOURCE_MONITORING_JOBS = {
    'meto': ['atmos_meto_xc40_seukv_eg_4x9', ],
    'nci': [],
    'uoe': [],
    'niwa': [],
    'kma': [],
...
ROSE_BUSH_URL = {
    'meto': 'http://fcm1/rose-bush',
    'nci': 'http://accessdev.nci.org.au/rose-bush',
    'uoe': 'Unavailable',
    'niwa':  'http://wrh-1.hpcf.niwa.co.nz:8080',
    'kma': 'Unavailable',
    'vm': 'http://localhost/rose-bush',
    'mss': 'Unavailable',
    'ncas': 'http://puma.nerc.ac.uk/rose-bush',
    'Unknown': 'Unavailable',
}
...
class SuiteReport(object):
    ...
    def __init__(self, suite_path, log_path=None, omit_housekeeping=True,
        ...
        (self.site, self.groups, self.trustzone, self.rose, self.cylc, 
         self.fcm, self.required_comparisons) = self.parse_rose_suite_run(
                                                    self.suite_path)
    ...
    def parse_rose_suite_run(suite_dir):
        ...
        required_comparisons = (compare_output == 'true' and 
    ...
    def gen_resources_table(self):
        ...
        for job in RESOURCE_MONITORING_JOBS[self.site]:
            filename = os.path.join(self.suite_path, 'log', 'job', '1',
                                    job, 'NN', 'job.out')
            if os.path.isfile(filename):
                wallclock, memory = self.get_wallclock_and_memory(filename)
                if wallclock and memory:
                    if found_nothing:
                        lines = ["", " || '''Task''' || '''Wallclock''' || " +
                                 "'''Total Memory''' ||"]
                        found_nothing = False
                    lines.append(" || {0:s} || {1:} || {2:} || ".format(job,
                                 wallclock, memory))
        lines.append("")
        return lines
    ...
    def print_report(self):
        ...
        try:
            ...
            if not self.required_comparisons:
                trac_log.append("")
                trac_log.append("-----")
                trac_log.append(" = WARNING !!! = ")
                trac_log.append("This rose-stem suite did not run the " +
                        "required comparisons (COMPARE_OUTPUT and/or " +
                        "COMPARE_WALLCLOCK are not true) and is therefore " +
                        "'''not valid''' for review")
                trac_log.append("-----")
                trac_log.append("")
            ...
            trac_log.extend(self.gen_resources_table())
            trac_log.append("")
